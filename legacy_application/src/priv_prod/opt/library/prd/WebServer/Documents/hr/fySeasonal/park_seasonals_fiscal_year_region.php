...
if(@$next_request!="" AND $level>4)
	...
	where fiscal_year='$fiscal_year'"; //echo "$sql";
	$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql");
	}
	...
	if($parkList[0]!="")
		...
		echo "<td><form><select name=\"center_code_1\" onChange=\"MM_jumpMenu('parent',this,0)\"><option selected=''>Select Park</option>";
			foreach($parkList as $k=>$v)
				...
				if($v==$_SESSION['hr']['select']){$s="selected";}else{$s="";}
				echo "<option value='$con1' $s>$v</option>";
				}
			  echo "</select></td></form>";
		}
		else
...
$flds_1="t1.center,t1.center_code,t1.osbm_title,t1.beacon_posnum,t1.ncas_account,t1.start_date, t1.budget_hrs_a, t1.budget_weeks_a, t1.month_11, t1.aca, t1.avg_rate_new, t1.budget_dollars_a, t1.district,t1.park_comments,t1.park_approve";
...
$pass_header="";
	...
	//	if($fld=="budget_dollars_b"){continue;}
		...
		if($fld=="budget_hrs_a")
			{
			$th=" bgcolor='white'";
			$header_fld="<font color='purple' size='+1'>Hours</font><br />Jun 16 2016<br />Jun 16 2017";
			$header_fld.="<br /><font color='red'>required</font>";
			}
		if($fld=="budget_weeks_a")
			{
			$th=" bgcolor='white'";
			$header_fld="<font color='purple' size='+1'>Weeks</font><br />Jun 16 2016<br />Jun 16 2017";
			$header_fld.="<br /><font color='red'>required</font>";
			}
		
		if($fld=="budget_dollars_a")
			{
		//	$header_fld="Dollars<br />Prev&nbsp;FY&nbsp;1 <br />Prev_FY_2 <br />This_FY_1 ";
...
// 			}
	...
	foreach($date_array as $number=>$fields)
		...
		echo "<tr>
<td colspan='5'>2016 <a href='http://www.osc.nc.gov/BEST/support/payroll/PY_Calendar_Jan_Dec__Ind_2016.pdf' target='_blank'>BEACON seasonal payroll calendar link</a></td>
