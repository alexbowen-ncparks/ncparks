...
// echo "20<pre>"; print_r($_SESSION); echo "</pre>";  //exit;	
...
// 	$com=mysqli_real_escape_string($connection, $_POST['comments']);
	if(empty($_POST['comments']) or empty($_POST['park_code']))
		...
		if(empty($_POST['comments']))
			{		
			echo "No comments were entered. This order was not processed."; 
			}
...
if(empty($rep))
	...
	if(!empty($park_order_list))
		...
		echo "<th colspan='2'>
		<form action='order_placed.php'>View Order Placed for Center: <select name='park_code' onchange=\"this.form.submit()\">
...
while($row=mysqli_fetch_assoc($result))
	...
	if(empty($rep))
		...
		<input type='submit' name='credit_submit' value=\"Enter Credit\">
		</td></tr>";
		echo "
		<script>
		$(function()
