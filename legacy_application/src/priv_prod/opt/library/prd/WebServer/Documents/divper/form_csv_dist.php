...
foreach($diffArray as $k=>$v)
	{
	//$divA.=$posArray[$i]."<br>";
// 	$sql = "SELECT  vacant.*,position.park, format(t3.`budgeted_amount`,0) as 'budgeted_salary',  format(t3.`salaried_amount`,0) as 'salaried_amount', format(t3.`difference_budget_amt_and_salary_amt`,0) as 'actual_budgeted_salary', B0149.position_desc as posTitle
// 
// From vacant
// 
...
// where position.beacon_num=$v";
	...
	$sql = "SELECT  vacant.*,position.park, format(replace(t3.`budgeted_amount`,',',''),0) as 'budgeted_salary',  format(replace(t3.`salaried_amount`,',',''),0) as 'salaried_amount', format(replace(t3.`difference_budget_amt_and_salary_amt`,',',''),0) as 'actual_budgeted_salary', t3.position_desc as posTitle

From vacant

LEFT JOIN position on position.beacon_num=vacant.beacon_num

LEFT JOIN B0149 as t3 on position.beacon_num=t3.position
...
// 	echo "<pre>"; print_r($temp_header_array); echo "</pre>";  exit;
	$rename_header=array("beacon_num"=>"BEACON Number", "posTitle"=>"Position", "payGrade"=>"Pay Grade/Comp Level", "park"=>"Park", "reg"=>"District", "budgeted_salary"=>"Budgeted Salary", "dateVac"=>"Date Vacated", "temp_calendar"=>"Calendar Days", "temp_business"=>"Business Days", "lastEmp"=>"Last Employee", "flow_date"=>"Flow Date", "approval_flow"=>"Approval Flow", "hireMan"=>"Hiring Manager", "comments"=>"Comments", "status"=>"Status", "year_end"=>"Days to EoY","start_date"=>"Start Date","actual_budgeted_salary"=>"Actual Budgeted Salary","salaried_amount"=>"Salaried Amount");
