...
if ($is_minimum_common == FALSE) {
                            ...
                            $exit = TRUE)
    {
        ...
        if (!$error_message) {
            $error_message = PMA_mysqli_error();
        }
...
function PMA_safe_db_list($only_db_check, $dbh, $dblist_cnt, $rs, $userlink, $cfg, $dblist) {
    ...
    if ($only_db_check == FALSE) {
        ...
        if (PMA_MYSQL_INT_VERSION >= 40002) {
            ...
            $rs               = PMA_mysqli_query($connection, $local_query, $dbh); // Debug: or PMA_mysqlDie('', $local_query, FALSE);
        ...
        if (!$dblist_cnt) {
            ...
            $rs           = PMA_mysqli_query($connection, $auth_query, $dbh); // Debug: or PMA_mysqlDie('', $auth_query, FALSE);
        } // end
    }
...
if ($is_minimum_common == FALSE) {
        ...
        if ($dblist_cnt) {
            ...
            for ($i = 0; $i < $dblist_cnt; $i++) {
                ...
                if ($is_show_dbs && ereg('(^|[^\])(_|%)', $dblist[$i])) {
                    ...
                    $rs          = PMA_mysqli_query($connection, $local_query, $dbh);
                    // "SHOW DATABASES" statement is disabled
                    if ($i == 0
                        && (PMA_mysqli_error() && mysql_errno() == 1045)) {
                        $true_dblist[] = str_replace('\\_', '_', str_replace('\\%', '%', $dblist[$i]));
                        $is_show_dbs   = FALSE;
                    }
                    // Debug
                    // else if (PMA_mysqli_error()) {
                    //    PMA_mysqlDie('', $local_query, FALSE);
                    // }
                    while ($row = @PMA_mysqli_fetch_row($rs)) {
                        $true_dblist[] = $row[0];
                    } // end while
                    if ($rs) {
                        mysql_free_result($rs);
    ...
    function PMA_countRecords($db, $table, $ret = FALSE)
    {
        ...
        $result       = PMA_mysqli_query($connection, $local_query) or PMA_mysqlDie('', $local_query, '', $err_url);
        ...
        if (!isset($num)) {
            $result = PMA_mysqli_query($connection, 'SELECT COUNT(*) AS num FROM ' . PMA_backquote($db) . '.' . PMA_backquote($table)) or PMA_mysqlDie('', $local_query, '', $err_url);
...
//-->
        ...
        // Corrects the tooltip text via JS if required
            ...
            $result = @PMA_mysqli_query($connection, 'SHOW TABLE STATUS FROM ' . PMA_backquote($GLOBALS['db']) . ' LIKE \'' . PMA_sqlAddslashes($GLOBALS['table'], TRUE) . '\'');
            if ($result) {
...
//-->
        ...
        if (isset($GLOBALS['table']) && isset($GLOBALS['sql_query'])
            ...
            if (isset($tbl_status) && (int) $tbl_status['Index_length'] > 1024) {
                @PMA_mysqli_query($connection, 'REPAIR TABLE ' . PMA_backquote($GLOBALS['table']));
            }
        }
        unset($tbl_status);
