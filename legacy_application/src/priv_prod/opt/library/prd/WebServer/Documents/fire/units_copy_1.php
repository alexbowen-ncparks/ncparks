...
unset($ARRAY);
$sql="SELECT t1.*, t2.unit_prescription, t2.file_name,t2.id as rx_id, t3.unit_map, t3.map_name, t3.id as map_id, t3.map_annotate
from $table as t1
...
//echo "<pre>"; print_r($ARRAY); echo "</pre>";
$skip=array("unit_id","park_code","map_annotate","prescription","file_name","rx_id");
...
foreach($ARRAY as $index=>$array)
	...
	foreach($array as $fld=>$value)
		...
		if($fld=="unit_prescription")
			{
			$input="";
			foreach($ARRAY as $k_index=>$v_array)
				{
				foreach($v_array as $k=>$value)
				{
				if($k=="unit_prescription")
					{
					if(!empty($value))
						{
						$file_name=$ARRAY[$k_index]['file_name'];
						$rx_id=$unit_id.".".$ARRAY[$k_index]['rx_id'];
						$input.="View Rx for Unit <a href='$value' target='_blank'>$unit_name</a> <==> <a href='burn_unit_upload.php?del_rx=$rx_id'>delete</a> $file_name";
						$unit_prescription=$value;
						}
					$input.="<br /><br />Select your file to upload. <font size='-2'>(Should be a Word or PDF file.)</font><br />
					<input type='file' name='file_upload_prescription'  size='40'><br />";
					}
				}
