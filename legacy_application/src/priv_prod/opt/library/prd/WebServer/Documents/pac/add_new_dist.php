...
$sql="SELECT t1.email 
	...
	where t3.beacon_num='60032913' or t3.beacon_num='60033104' or t3.beacon_num='60033019' or t3.beacon_num='60032912'
...
// 	echo "$dist $sql";
	...
	while($row=mysqli_fetch_assoc($result))
		{
		if(array_key_exists($dist,$dist_to_reg))
			{
	...
	*/
	...
	if(!empty($id))
		...
		@$dist_approve=$ARRAY[0]['dist_approve'];
			...
			echo "<tr><td>PASU <a href='mailto:$disu_email?subject=$park_code PAC Nomination for $fn $ln&body=$email_link'>emails RESU</a> to request approval. </td>";

	if($level>1)
		{
		$chop_email="denise.williams@ncparks.gov";
	//	if(!isset($dist_approve)){$dist_approve="";}
		echo "<td>DISU <a href='mailto:$chop_email?subject=$park_code PAC Nomination for $fn $ln'>emails CHOP</a> to request approval on: <input id=\"datepicker1\" type=\"text\" name='dist_approve' value='$dist_approve' size='12'/></td>";
		}
		echo "</tr></table>";
		}
