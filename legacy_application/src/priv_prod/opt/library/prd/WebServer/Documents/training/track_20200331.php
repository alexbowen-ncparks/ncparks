...
if(empty($_POST['rep']))
	...
	if($level==1 and ($_SESSION['training']['working_title']=="Park Superintendent" or $_SESSION['training']['tempID']=="Coffman4471")) // allow ranger acting as PASU
		{
		...
		where $where
		order by t1.Lname, t1.Fname"; //echo "$sql";
...
/*SELECT concat( t3.Lname, ', ', t3.Fname ) AS name, t1.id, t2.program, t2.title, t1.date_completed, group_concat( t4.file_name SEPARATOR '*') AS file_name, t1.comments, t1.hours, t2.hrs_credits, t4.link
FROM track AS t1
...
*/	
	$sql="SELECT concat( t3.Lname, ', ', t3.Fname ) AS name, t1.id, t2.program, t2.title, t1.date_completed, group_concat( t4.file_name SEPARATOR '*') AS file_name, t1.comments, t1.hours, t2.hrs_credits, group_concat( t4.link SEPARATOR '*') AS link, t5.currPark as Park
FROM track AS t1
LEFT JOIN class AS t2 ON t1.class_id = t2.id
LEFT JOIN divper.empinfo AS t3 ON t1.emid = t3.emid
...
if(empty($_POST['rep']))
	{
	echo "<th align='center'>$c</th></tr><tr>";
	}
