...
$skip_flds=array("id","mc_proj_number");
...
//	echo "<pre>"; print_r($_POST); echo "</pre>"; // exit;
	if($_POST['submit']=="Add")
		{include("proj_add.php"); $action_type="Update"; $var_act="Add";}
...
// ************* Form **************
$fld_rename=array("park_code"=>"Park Code(s)", "proj_name"=>"Project&nbsp;Name", "proj_type"=>"Project&nbsp;Type", "summary"=>"Project&nbsp;Summary", "proj_lead"=>"Project&nbsp;Lead", "sco"=>"Does this project require SCO review?", "start_date"=>"Start&nbsp;Date", "duration"=>"Duration", "cost"=>"Cost Estimate", "funding_source"=>"Funding&nbsp;Source", "proj_number"=>"Project Number", "rmr_form"=>"RMR form completed?", "tr_form"=>"Trail Review form completed?", "design"=>"Will design assistance be required?", "gmp"=>"Project identified in GMP?", "pep"=>"Project listed on PEP list?", "pac"=>"Project review with Park Advisory Committee?", "edits"=>"Edited&nbsp;by", "submitted_by"=>"Submitted&nbsp;by", "proj_comments"=>"Project Comments", "how_done"=>"How will work be accommplished?", "proj_status"=>"<font color='orange'>Status</font>");
...
echo "<tr><td colspan='3'><b>NC DENR - DPR Project Review and Approval Form</b> Support material can be uploaded after creation of project.</td></tr>";
foreach($fld_array as $index=>$fld)
	{
	if(in_array($fld,$skip_flds)){continue;}
	if(in_array($fld,$not_required)){$req="";}
	@$value=${$fld};   IF(empty($var_act)){$value="test";}
	...
	if($fld=="how_done")
		...
		$display_input.=" If \"Other\", indicate source in Project Summary.";
		}
	if($fld=="proj_number")
		{
...
Add a Comment</a> <div id=\"next_com\" style=\"display: none\"><textarea name='next_comment' cols='111' rows='5'></textarea></div>";
		}
		
	if($fld=="submitted_by" and !empty($submitter_email))
		{
