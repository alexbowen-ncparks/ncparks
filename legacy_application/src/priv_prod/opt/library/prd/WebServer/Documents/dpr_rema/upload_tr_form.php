...
if(isset($_FILES['tr_form']['tmp_name']))
	{
/*
...
*/
	$error=$_FILES['tr_form']['error'];
	if($error==0){
			$temp_name=$_FILES['tr_form']['tmp_name'];
			if($temp_name==""){continue;}
			$name=$_FILES['tr_form']['name'];
			$name=str_replace("'","",$name);
			$name=addslashes($name);
		extract($_POST);
		$form_name="DPR_tr_form_".$proj_number."_".time();
		$e=explode(".",$_FILES['tr_form']['name']);
		$ext=array_pop($e);
		$file_name = $form_name.".".$ext;

//echo " $file_name"; exit;

		$uploaddir = "upload_tr_form"; // make sure www has r/w permissions on this folder
		...
		$sql="INSERT INTO upload_tr_form SET proj_id='$proj_id', proj_number='$proj_number', file_name='$name', link='$uploadfile' ";
