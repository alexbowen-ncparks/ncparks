...
if(isset($del))
	{
	$sql="SELECT pr63_id, link FROM le_images_pio where id='$del'";
...
//echo "<pre>"; print_r($path_parts); echo "</pre>";  exit;
	...
	$sql="DELETE FROM le_images_pio where id='$del'";
//		echo "$sql";exit;
		$result = @mysqli_query($connection,$sql) or die("$sql Error 1#". mysqli_errno($connection) . ": " . mysqli_error($connection));
		
		header("Location: pr63_form_pio.php?id=$pr63_id");
	exit;
	}
...
if($file_type!="pdf")
	...
	$img->destroy();
		...
		$sql="INSERT le_images_pio SET pr63_id='$id', parkcode='$parkcode', link='$uploadfile', image_name='$in', imagesize='$size'";
	//	echo "$sql<br />";//exit;
		$result = mysqli_query($connection,$sql) or die("$sql Error 1#". mysqli_errno($connection) . ": " . mysqli_error($connection));
			}
		}

	header("Location: pr63_form_pio.php?id=$id&file=$mime");
	}
?>