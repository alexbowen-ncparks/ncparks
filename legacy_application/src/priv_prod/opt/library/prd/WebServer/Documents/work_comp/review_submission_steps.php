...
//	echo "<pre>"; print_r($_POST); echo "</pre>";  exit;
	...
	if(!empty($delete))
		...
		$sql="DELETE FROM witness_statement where wc_id='$wc_id'";
		$result = mysqli_query($connection,$sql) or die ("Couldn't execute query Update. $sql ".mysqli_error($connection));
		...
		echo "Deletion was successful.<br />";
		exit;
		}
...
$rename=array("form_19_link"=>"Step 1: Injury Data Collection Form","date_of_injury"=>"Date of Injury","wc_auth_link"=>"Step 3: Workers' Comp Authorization","incident_investigate_link"=>"Step 4: Injury Data Collection form for Supervisors","emp_statement_link"=>"Step 6: Employee's Statement","wit_statement_link"=>"Step 5: Witness Statement","release_info_link"=>"Step 8: Release of Information","park_code"=>"Park Code","wc_id"=>"Submission ID","timestamp"=>"Submission Timestamp","leave_options_link"=>"Step 7: Leave Options","park_comments"=>"Park Comments","hr_comments"=>"HR Comments","wc_approved"=>"Entered into Corvel","employee_status"=>"Employee Status","refuse_treatment_link"=>"Step 2: Refuse Treatment","tick_log_link"=>"Tick Log","type_of_injury"=>"Injury Info","body_part"=>"Body Part", "hr_rep_comments"=>"HR Rep Comments");

$relink_action=array("form_19_link"=>"new_wc_request_1.php","refuse_treatment_link"=>"refuse_treatment_2.php","wc_auth_link"=>"wc_authorization_3.php","incident_investigate_link"=>"incident_investigate_4.php","wit_statement_link"=>"witness_statement_5.php","emp_statement_link"=>"employee_statement_6.php","park_code"=>"Park Code","wc_id"=>"Submission ID","timestamp"=>"Submission Timestamp", "leave_options_link"=>"leave_options_7.php","release_info_link"=>"release_info_8.php","tick_log_link"=>"tick_related_9.php");

// $relink_action=array("form_19_link"=>"new_wc_request.php","wc_auth_link"=>"wc_authorization.php","incident_investigate_link"=>"incident_investigate.php","emp_statement_link"=>"employee_statement.php","release_info_link"=>"release_info.php","park_code"=>"Park Code","wc_id"=>"Submission ID","timestamp"=>"Submission Timestamp", "leave_options_link"=>"leave_options.php","refuse_treatment_link"=>"refuse_treatment.php","tick_log_link"=>"tick_related.php");
...
foreach($ARRAY AS $index=>$array)
	...
	foreach($array as $fld=>$value)
		...
		if($fld=="park_comments")
			{
			echo "<form action='review_submission_steps.php' method='POST'>"; // start form to capture comments and approval
			}
		...
		if($fld=="employee_status")
			...
			$value.="</td><td>Email DPR  <a href=\"mailto:$email?cc=$cc&subject=$park_code Worker Comp for $full_name&body=https://10.35.152.9/work_comp/review_submission_steps.php?wc_id=$wc_id You will need to login to the Worker's Comp db application to view. 
			$value.="</td><td>Email DPR  <a href=\"mailto:$email?cc=$cc&subject=$park_code Worker Comp for $full_name&body=https://10.35.152.9/work_comp/review_submission_steps.php?wc_id=$wc_id You will need to login to the Worker's Comp db application to view. 
\">DPR-HR-Staff & Safety Officer</a>";
		...
		if($fld=="wc_approved")
			...
			$value=$new_value;
			$body="https://10.35.152.9/work_comp/review_submission_steps.php?wc_id=$wc_id  You will need to login to the Worker's Comp db application to view.";
			$body="https://10.35.152.9/work_comp/review_submission_steps.php?wc_id=$wc_id  You will need to login to the Worker's Comp db application to view.";
