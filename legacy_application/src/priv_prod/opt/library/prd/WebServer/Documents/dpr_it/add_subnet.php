...
// echo "<pre>"; print_r($_POST); echo "</pre>";  //exit;
	...
	$_POST['location']=$exp[0];
	$_POST['site_name']=$exp[1];
	$location=substr($_POST['location'],0,4);
	
	IF(in_array($location, $adm_district))
		{
		$_POST['district_section']="DPR ADM";
		if($_POST['location']=="NRC")
			{
			$_POST['district_section']="Admin";
			}
		}
		ELSE
		{
		$_POST['district_section']=$district[$location];
		}
...
$search_computers_dropdown=array("division","district_section","location","type","os","make","model");
$search_subnets_dropdown=array("division", "location", "site_name", "type", "current_service_provider","vlan","site_id");
include("form_arrays.php");

$skip=array("id", "district_section", "region_section", "site_name");
...
// 	if($fld=="division"){$value="DPR";}
	$named_fld=$fld;
	$line="<tr><td><strong>$named_fld</strong></td>
...
// 	if($fld=="division"){$drop_down_array[]="MNS";}
	if($fld=="location"){$named_fld="location * site_name";}
	
		$line="<td><strong>$named_fld</strong></td><td><select name='$fld'><option value=\"\" selected></option>\n";
