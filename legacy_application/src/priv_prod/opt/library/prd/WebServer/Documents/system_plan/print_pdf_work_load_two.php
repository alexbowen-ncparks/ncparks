...
if(empty($_POST))
	{
	$_POST['parkcode'][$parkcode]=$parkcode;
	}
...
foreach($_POST['parkcode'] as $k=>$v)
	{
	PDF_begin_page_ext($pdf, 595, 1842, "");
...
//	echo "$sql"; exit;
	...
	PDF_set_text_pos($pdf, 50, 1800);
	PDF_show($pdf, $text);
$sql = "SELECT COUNT(*) AS `number`, `fac_type` 
FROM facilities.`spo_dpr` 
where park_abbr='$parkcode'
GROUP BY `fac_type` ORDER BY `fac_type`";//echo "$sql";
$result = @MYSQL_QUERY($sql,$connection);
$new_fac[0]="";
	while($row=mysql_fetch_assoc($result))
		{
		$new_fac[0].=$row['fac_type']."-".$row['number'].", ";
		}
		$new_fac[0]=rtrim($new_fac[0],", ");   //echo "$new_fac[0]"; exit;
	 unset($FAC_DISPLAY);
...
$sql="SELECT `Description of Site` as park_description
	from dpr_system.survey_2013
	where park_code='$parkcode' 
	";
	$result = @MYSQL_QUERY($sql,$connection);	
	$row=mysql_fetch_assoc($result);
	if($row){extract($row);}
//	echo "p=$park_description"; exit;
	
	$text="Summary";
	PDF_setfont($pdf, $helvetica_bold, 11.0);
	PDF_set_text_pos($pdf, 50, 1780);
	PDF_show($pdf, $text);

	$width=110;
	$text=$park_description;
...
*/	
	$texty = pdf_get_value($pdf, "texty", 0);
$y=$texty-26;
PDF_set_text_pos($pdf, 50, $y+7);
		PDF_setfont($pdf, $helvetica_bold, 11.0);
		PDF_show($pdf, "State Park Facilities");
	PDF_setfont($pdf, $times, 11.0);

	$textx = pdf_get_value($pdf, "textx", 0);	
	$texty = pdf_get_value($pdf, "texty", 0);	
			PDF_set_text_pos($pdf, $textx, $texty);
			PDF_show($pdf, " - (facility type and number)");
$y=$texty-13;	
if(!isset($no_theme)){$no_theme="";}
$fac_width=100;
foreach($new_fac as $index=>$line)
		{
		$check_width=$fac_width;
		if($no_theme==""){$check_width-=55;}
		if(strlen($line)<$check_width)
			{
			if($no_theme==""){$line=strtoupper($line);}  // Theme Title
					$line=ltrim($line);
			PDF_set_text_pos($pdf, 50, $y);
			...
			foreach($line_array as $i=>$word)
				{
				$para.=$word." ";
				if(strlen($para)>$fac_width OR (strlen($para)+(strlen(@$line_array[$i+1])-4))>$fac_width)
					{
					PDF_set_text_pos($pdf, 50, $y);
					$para=ltrim($para);
					PDF_show($pdf, $para." ");
...
//echo "t=$texty"; exit;
	...
	$textx = pdf_get_value($pdf, "textx", 0);
	$texty = pdf_get_value($pdf, "texty", 0);
		$pass_texty=$texty-$fudge;
	$text="Permanent Staff";

		PDF_setfont($pdf, $helvetica_bold, 11.0);
			PDF_set_text_pos($pdf, 50, $texty-$fudge+7);
			PDF_show($pdf, $text);
		...
		$work_load_texty=pdf_get_value($pdf, "texty", 0);
		
	$text="Seasonal Staff";

//	$texty = pdf_get_value($pdf, "texty", 0);
			PDF_set_text_pos($pdf, 300, $pass_texty+7);
		PDF_setfont($pdf, $helvetica_bold, 11.0);
			PDF_show($pdf, $text);
	...
	if(@$POSITION2!="")
		...
		$work_load_texty_seasonal=pdf_get_value($pdf, "texty", 0);
		...
		// Work load
		if($work_load_texty_seasonal<$work_load_texty)
			{$work_load_texty=$work_load_texty_seasonal;}
		$var=explode("\n", $work_load['justification']);
	//	echo "<pre>"; print_r($var); echo "</pre>";  exit;
		$i=0;
	$para="";
	$y=$work_load_texty-26;
		PDF_setfont($pdf, $helvetica_bold, 11.0);
	PDF_set_text_pos($pdf, 50, $y+5);
				PDF_show($pdf, "Work Load Factors");
	$y=$work_load_texty-36;
	$c=count($var);
		PDF_setfont($pdf, $times, 10.0);
//	echo "<pre>"; print_r($var); echo "</pre>";  exit;
$width=123;
	foreach($var as $index_1=>$line)
		{
		$check_width=$width;
		if($no_theme==""){$check_width-=55;}
			...
			$para="";
			$line_array=explode(" ",$line);
	//		if($level>4){echo "<pre>"; print_r($line_array); echo "</pre>";  exit;}
			foreach($line_array as $i=>$word)
				{
				$para.=$word." ";
				if(strlen($para)>$width OR (strlen($para)+(strlen(@$line_array[$i+1])-3))>$width)
					{
				//	if($level>4 and $i>93){echo "$i $para<pre>"; print_r($line_array); echo "</pre>";  exit;}
					PDF_set_text_pos($pdf, 50, $y);
					$para=ltrim($para);
					PDF_show($pdf, $para." ");
					$y-=11;
			...
			if($index_1>-1)  // last line of all para 
				{
				$test=trim($var[$index_1]);
			
				PDF_set_text_pos($pdf, 50, $y);
				PDF_show($pdf, $para);
				if(!empty($para))
					{
					$y-=11;   // distance between last line and start of next para
					}
				}
		}
	// end work load
