...
if($position!="Park Superintendent" and $level<2 and !in_array($_REQUEST['beacon_num'],$check_supervise))
	{
// 	if($position!="" and $_SESSION['divper']['select']!="CRMO")
// 		{
// 		exit;
// 		}
	exit;
...
$sql = "SELECT * FROM dpr_system.parkcode_names_region";
...
while($row=mysqli_fetch_assoc($result))
	...
	if($row['region']=="CORE"){$arrayCORE[]=$row['park_code'];}
	if($row['region']=="PIRE"){$arrayPIRE[]=$row['park_code'];}
	if($row['region']=="MORE"){$arrayMORE[]=$row['park_code'];}
	}
...
//	echo "<pre>"; print_r($_POST); echo "</pre>"; // exit;
	...
	header("Location: trackPosition_reg.php?beacon_num=$beacon_num&m=1");
	exit;
	}
...
</table><form method='post' action='trackPosition_reg.php'>
$success
<table border='1'><tr><th>BEACON<br>Number</th><th>Park</th><th>Region</th><th>CDL Required</th><th>Hiring Manager<br>Name</th><th>Classification</th></tr>
<tr>

<td align='center'><font color='blue'>$beacon_num</font></td>
<td align='center'><font color='blue'>$park</font></td>
<td align='center'><font color='blue'>$reg</font></td>
