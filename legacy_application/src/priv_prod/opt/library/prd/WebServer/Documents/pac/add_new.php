...
if(!empty($id))
	...
	if(in_array("PAC",$passCode))
		{
		$letterCode="appoint";
		echo "<tr><td colspan='2'>Produce PAC<br /><a href='PAC/nomination_letter.php?id=$id&parkcode=$park' target='_blank'>Nomination</a> Letter</td>";
// 		echo "<tr><td colspan='2'>Produce PAC<br /><a href='PAC/nomination_letter_reg.php?id=$id&parkcode=$park' target='_blank'>Nomination</a> Letter</td>";
...
// 			if(empty($disu_email))
// 				{
// 				$disu_email="Derrick.Evans@ncparks.gov";
...
// 				}
	...
	if($level>1)
		{
// 		$chop_email="Derrick.Evans@ncparks.gov";
	//	if(!isset($dist_approve)){$dist_approve="";}
		echo "<td>DISU <a href='mailto:$chop_email?subject=$park_code PAC Nomination for $fn $ln&body=$email_link'>emails DDOP</a> to request approval on: <input id=\"datepicker1\" type=\"text\" name='dist_approve' value='$dist_approve' size='12'/></td>";
		}
		echo "</tr></table>";
		}
...
//	echo "<pre>"; print_r($ARRAY); echo "</pre>"; // exit;
	...
	if(!empty($ARRAY[0]['file_link']))
		...
		foreach($files as $kf=>$kv){
			$f=explode("/",$kv);
			@$links.="<td align='center'>[<font color='brown'>$f[2]</font>]<br /><a href='$kv' target='_blank'>View</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href='dpr_labels_del_file.php?link=$kv&pass=$park&id=$id'  onclick=\"return confirm('Are you sure you want to delete this File?')\">Delete</a>";
			echo "</td>";
			}
		echo "<tr>$links</tr>";
		}
