...
$count=count($fieldNames);
echo "<form action='pcard_holders_dncr12.php'>";
echo "<table border='1' cellpadding='2'>";
foreach($ARRAY as $k=>$v)
	{// each row
...
foreach($ARRAY as $k=>$v)
	...
	echo "<tr>";
		...
		foreach($v as $k1=>$v1){// each field, e.g., $tempID=$v[tempID];
			...
			if(in_array($k1,$LinkFields)){echo "<td>$v1 <a href='pcard_holders_dncr12.php?edit=y&id=$v1'>Edit</a></td>";} else  {echo "<td>$v1</td>";}
