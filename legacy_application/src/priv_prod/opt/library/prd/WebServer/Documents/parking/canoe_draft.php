...
$edit_array=array("Canoe 1","Canoe 2");
...
$sql="SELECT  * from `canoe_dates` where 1 and date_id = '$_POST[pass_edit]'";  
$result = @mysqli_query($connection,$sql) or die(mysqli_error($connection));
while($row=mysqli_fetch_assoc($result))
	{$existing_dates[$row['space']]=$row;}
echo "<pre>"; print_r($existing_dates); echo "</pre>"; // exit;

		foreach($edit_array as $index=>$var_canoe)
			{
			if(!empty($_POST['park_code'][$var_canoe]) and array_key_exists($var_canoe,$_POST['space']))
				{
				$pc=$_POST['park_code'][$var_canoe];
				if($level<4 and !empty($_POST['space'][$var_canoe]))
					{
					$ck_name=$existing_dates[$var_canoe]['name'];
					if($ck_name!=$log_name){continue;}
					}
					else
					{
					$check_name="";
					}
				$sql="REPLACE canoe_dates 
				SET all_day='$pc',
				 date_id = '$_POST[pass_edit]', space='$var_canoe' $replace_name
				"; 
				echo "$sql"; exit;
				...
				where date_id = '$_POST[pass_edit]' and space='$var_canoe' $check_name
				";  
				$result = @mysqli_query($connection,$sql) or die("$sql".mysqli_error($connection));
				if(!empty($_POST['park_code'][$var_canoe]) and !empty($_POST['space'][$var_canoe]))
					{
					$pc=$_POST['park_code'][$var_canoe];
					$sql="REPLACE canoe_dates 
					SET all_day='$pc',
				...
				header("Location: cal.php?cal_type=canoe");
				exit;
				}
			}
		...
		{
// 			echo "79<pre>"; print_r($_POST); echo "</pre>";  exit;
		foreach($_POST['park_code'] as $index=>$var_canoe)
			{
			$sql="DELETE FROM canoe_dates 
				where date_id = '$_POST[pass_edit]' and space='$index' $check_name
				";  //echo "$sql";
				$result = @mysqli_query($connection,$sql) or die("$sql".mysqli_error($connection));
			}
				header("Location: cal.php?cal_type=canoe");
...
<?php
	...
	$cal_array=array(""=>"","parking"=>"Parking Spaces","van"=>"15 Passenger Van","event"=>"Event Trailer","festival"=>"Festival Trailer","canoe"=>"Canoes");
...
<?php
echo "<table align='center'><tr><td><font color='green'><i><b>The canoes are stationed at the Yorkshire Center 12700 Bayleaf Church Road.</b></i></font><br /><br />If reserving a canoe, it is also your responsibility to coordinate returning the canoe to the Yorkshire Center in a timely manner.</td></tr></table>";
