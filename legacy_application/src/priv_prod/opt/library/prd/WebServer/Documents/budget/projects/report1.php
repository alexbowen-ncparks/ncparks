...
$query4="insert into cid_project_balances(projnum,approved,pm_allocation,expenses)
 select proj_num,'','',sum(amount) from partf_payments 
 where 1  and mid(datenew,1,4)='$calyearS' group by proj_num";
 $result4 = mysqli_query($connection, $query4) or die ("Couldn't execute query 4.  $query4");
} 
...
if($cat_nr=="X") {$where.="projcat='nr' or " ;}	  	 
...
if($fyear_py11=="X"){$where3.="yearfundf='$py11' or " ;}
if($fyear_py12=="X"){$where3.="yearfundf='$py12' or " ;}
if($fyear_py13=="X"){$where3.="yearfundf='$py13' or " ;}
if($fyear_py14=="X"){$where3.="yearfundf='$py14' or " ;}
if($fyear_py15=="X"){$where3.="yearfundf='$py15' or " ;}
if($fyear_py16=="X"){$where3.="yearfundf='$py16' or " ;}
if($fyear_py17=="X"){$where3.="yearfundf='$py17' or " ;}
...
if($parkS!='')
{
$where5=" and park='$parkS' ";
}

if($managerS!='')
{
$where6=" and partf_projects.manager='$managerS' ";
}

if($project_nameS!='')
{
$where7=" and partf_projects.projname like '%$project_nameS%' ";
}
// echo "where statement=$where4"; exit; 
if($park_order=='' and $year_order==''){$park_order='X';}
if($park_order!='' and $year_order!=''){$park_order='X';}
 
if($park_order=='X')
{
$query5="select park,projname,partf_projects.project_center_year_type,partf_projects.center,partf_projects.funding_source,sum(approved) as 'approved',
...
$where4 $where5 $where6 $where7
group by cid_project_balances.projnum order by park,status desc, partf_projects.projnum asc";
}

if($year_order=='X')
{
$query5="select park,projname,partf_projects.project_center_year_type,partf_projects.center,yearfundf,partf_projects.funding_source,sum(approved) as 'approved',
sum(expenses) as 'expenses',sum(approved-expenses) as 'balance', statusper as 'status',manager, 
cid_project_balances.projnum from cid_project_balances 
left join partf_projects on cid_project_balances.projnum=partf_projects.projnum 
$where4 $where5 $where6 $where7
group by cid_project_balances.projnum order by yearfundf asc,park,status desc, partf_projects.projnum asc";
}
...
//if($beacnum=='60033166')
{	
echo "Line 141: query5=$query5"; //exit;
}
...
//echo "<br />Line 106: query5=$query5<br />";
...
echo "<th align=center><font color=brown>Funding</font></th>"; 
...
if($calyearS!='')
{
echo "<th align=center><font color=brown>Expenses_$calyearS</font></th>"; 
}
...
//if($calyearS==''){$exp_header='expenses';}
//if($calyearS!=''){$exp_header='expenses_'.$calyearS;}

//echo "<br />exp_header=$exp_header<br />"; exit;

$sql_XX="CREATE TABLE IF NOT EXISTS`budget`.`XX_TEMP_report1` ( 
`park` VARCHAR(64) NOT NULL ,
`projname` VARCHAR(64) NOT NULL ,
`project_center_year_type` VARCHAR(64) NOT NULL ,
`funding` VARCHAR(64) NOT NULL ,
`approved` VARCHAR(64) NOT NULL ,
`expenses` VARCHAR(64) NOT NULL ,
`balance` VARCHAR(64) NOT NULL ,
`status` VARCHAR(64) NOT NULL ,
`manager` VARCHAR(64) NOT NULL 
) ENGINE = MyISAM;";
$createXX = mysqli_query($connection, $sql_XX) or die ("Couldn't execute query 5.  $sql_XX");


$sql_TRUN="TRUNCATE TABLE XX_TEMP_report1";
$trunXX = mysqli_query($connection, $sql_TRUN) or die ("Couldn't execute query 5.  $sql_TRUN");
...
$sql_insert="INSERT INTO XX_TEMP_report1 set park='$park', projname='$projname', project_center_year_type='$project_center_year_type', funding='$funding_source', approved='$approved', expenses='$expenses', balance='$balance'
, status='$status', manager='$manager'";
$insertXX = mysqli_query($connection, $sql_insert) or die ("Couldn't execute query 5.  $sql_insert");
...
"<tr$t>"; 
        ...
        // echo "<td><a href='reports_one_budget_group_summary_by_center.php?budget_group=$budget_group'>$budget_group</a></td>"; 
		 echo "<td>$park</td> 
           <td>$projname</td> 	
           <td>$project_center_year_type <a href='/budget/partf.php?projstring=$project_center_year_type&Submit=Find' target='_blank'>View</a></td> 	
           <td><a href='report1_funding.php?center=$center' target='_blank'>$funding_source</a></td> 	
           <td>$approved</td> 	
           <td>$expenses</td> 	
           <td>$balance</td> 	
           <td>$status</td> 	
...
echo "<tr><td colspan='6'></td><td colspan='2' align='center'><a href='report1_csv.php'>.csv Export</a></td></tr>";
