...
if(in_array($passedBeacon_num,$superviseThese) and $passedBeacon_num!=""){$accessLevel="viewEdit"; $track=1;}// supervisor
...
if(!empty($_SESSION['divper']['accessPark']))
	...
	if(in_array($_SESSION['parkStemp'],$multi_park_array))
		{$test=$_SESSION['parkStemp'];}
	}
...
if($positionTitle=="Park Superintendent IV" AND $test==$parkS)//Reservoir PASU
	{$accessLevel="viewAll";}
...
if($emid!="")
	...
	$sql = "SELECT position.posTitle,position.beacon_num,emplist.jobTitle,emplist.currPark,empinfo.Nname,empinfo.Fname,empinfo.Mname,empinfo.Lname,empinfo.suffix,empinfo.badge,empinfo.add1,empinfo.add2,empinfo.city,empinfo.state,empinfo.zip,empinfo.countycode,empinfo.email,dprunit.ophone,
	dprunit.fax,
	empinfo.phone,
	empinfo.work_cell,
	empinfo.Hphone,
	empinfo.Mphone,
	empinfo.spouse,empinfo.spouse_contact,empinfo.dbmonth,empinfo.dbday
	From empinfo 
...
if($accessLevel=="viewEdit")
	...
	$countyCodeList[]="Lee";  // People live in counties that don't have a park
	sort($countyCodeList);
//	echo"<pre>";print_r($row);echo "</pre>";
	...
	while (list($key,$val)=each($row))
		...
		if(array_key_exists($key,$fieldArray) and $key!="0")
			...
			if($key!="countycode")
				{
				if($key=="badge"){$exp="<font size='-2'>Commissioned Officers Only</font>";}else{$exp="";}
				if($key=="ophone"||$key=="posTitle"||$key=="currPark"||$key=="fax"||$key=="state")
					{
					if($key=="posTitle")
				...
				{
		//		echo "<pre>"; print_r($countyCodeList); echo "</pre>";
				echo "<tr><td align='right'><b>$size[0]</b></td><td><select name=\"$key\"><option selected></option>";
				while (list($key1,$val1)=each($countyCodeList))
					{
					if($val1==""){continue;}
	...
	echo "<tr><td>&nbsp;</td><td>Items in <font color='red'>red</font> are seen ONLY by your supervisors.&nbsp;&nbsp;
...
if($accessLevel=="viewLimited")
	...
	while (list($key,$val)=each($row))
		...
		if(array_key_exists($key,$fieldArray) and $key!="0")
			...
			$visible=$k[2];
			if($visible=="y"){$p=1;}else{$p="";}
			if($key=="countycode"){@$val=$countyCodeList[$val];}
			if($key=="phone"){if($val){$p=1;}else{$p="";}}
