...
echo "<table  align='center'>";
    ...
    if($flag_type=='FAS')
	{echo "<option selected='FAS'>FAS</option>";} else {echo "<option value='FAS'>FAS</option>";}	
...
{
if($statusC=='pending')
{$status='complete'; $status_message="<font color='brown'>$parkC $flag_typeC</font><font color='green'><b> Complete</b></font>. <font color='brown'>Thanks!</font> ";
...
if($statusC=='complete')
{$status='pending'; $status_message="<font color='brown'>$parkC $flag_typeC</font><font color='red'><b> Pending</b></font>. <font color='brown'>Thanks!</font> ";
	

$query="update flag
        set status='$status',finish_time='0000-00-00' 
		where id='$idC' ";
echo "<br />query=$query<br />";		
$result=mysqli_query($connection,$query) or die ("Couldn't execute query. $query");
...
echo "<td>$scorer</td>";
echo "<td><a href=flag1.php?type=search&flag_search=search&parkS=$parkS&statusS=$statusS&parkC=$park&statusC=$status&flag_typeC=$flag_type&idC=$id>$status</a></td>";
