...
if($level=='5')
{
$query4="select * from gameday where 1 order by game_name asc";
}
else
{
$query4="select * from gameday where 1 and status='show' order by game_name asc";
}
//echo $query4;exit;		 
...
echo "<table><tr><td><font color=brown class='cartRow'>gameday</font></td></tr></table>";
...
echo "<br />";
//echo "<td></td>";
echo "<form method='post' action='game_add.php' name='form2' onsubmit='return validateForm()' >";
//echo "<td><input type='text' name= 'alert_location' placeholder='kela,jord,etc'></input></td>";  
echo "<td><textarea name= 'game_name' rows='1' cols='30' placeholder='Game Name' id='input2'></textarea></td>";            
...
if($file=='y'){$bgc="lightpink";} else {$bgc="lightcyan";}

echo 

"<tr bgcolor='$bgc'>"; 

if($file=='y'){echo "<td><a href='$game_document'>$game_name</a></td>";}
else
echo "<td><a href='gameday.php?comment=y&add_comment=y&folder=community&gid=$gid'>$game_name</a></td>"; 
...
$query4a="select game_name,game_document from gameday where gid='$gid' ";
...
$query4b="select * from gameday_comments where 1 and gid='$gid' $order2 ";
...
extract($row4a);
$game_document=str_replace('  ','&nbsp;&nbsp;',$game_document);
$game_document=nl2br($game_document);

echo "<table>";
echo "<tr>";
echo "<td><font color=brown class='cartRow'><a href='gameday.php?folder=community&menu=1'>gameday</a></font></td>";
...
echo "<table>";
echo "<tr>";
echo "<td><font color=brown class='cartRow'>$game_name</font></td>";
if($level=='5')
{
echo "<td><a href='gameday.php?comment=y&add_comment=y&folder=community&gid=$gid&editP=y'>Edit</a></font></td>";
}
...
$query4c="select game_name,game_document from gameday where gid='$gid' ";
...
echo "<form method='post' action='gameday_edit.php'>";
//echo "<td><input type='text' name= 'alert_location' placeholder='kela,jord,etc'></input></td>";  
echo "<td><textarea name= 'game_document' rows='100' cols='120' >$game_document</textarea></td>";            
      
	  echo "<td><input type='submit' name='submit' value='Update_Game'></td>";
...
echo "<table border='1'><td><a href='gameday.php?comment=y&add_comment=y&folder=community&gid=$gid><font color='brown' $shade_comment>Comments</font></a></td>";
...
echo "<form method='post' action='comment_add_gameday.php' name='form3' onsubmit='return validateForm()'  >";
...
echo "<td><a href='gameday.php?comment=y&add_comment=y&folder=community&pid=$pid&show_order=newest'><font size='3' $shade_newest>Newest on top</font></a></td>";
echo "<td><a href='gameday.php?comment=y&add_comment=y&folder=community&pid=$pid&show_order=oldest'><font size='3' $shade_oldest>Newest on bottom</font></a></td>";
