...
select pid
from(
select park,photoname,link,width,height,pid,width/height as 'aspect_ratio'
         from images 
         where park='haro'
         and cat like '%scenic%') as t
where t.aspect_ratio > '2.50'
...
/*
$query4="
select park,photoname,comment,link,width,height,pid,aspect_ratio
from(
select park,photoname,comment,link,width,height,pid,width/height as 'aspect_ratio'
         from images 
         where 1
		 and dateM >= '20150311' and dateM <= '20150317'
         and cat like '%scenic%') as t
where t.aspect_ratio > '1.20' and t.aspect_ratio < '1.60'  "; 
*/

$query4="
select park,photoname,comment,photog,link,width,height,pid,aspect_ratio
from(
select park,photoname,comment,link,photog,width,height,pid,width/height as 'aspect_ratio'
         from images 
         where 1
		 and dateM >= '20150311' and dateM <= '20150317'
         and cat like '%nrid%') as t
where 1  "; 
...
while ($row4=mysqli_fetch_array($result4))
	...
	$Width[]=$row4['width'];
	$Height[]=$row4['height'];
	$Aspect_Ratio[]=$row4['aspect_ratio'];
	//$Gid[]=$row4['gid'];
...
$aspect_ratio2=round($Aspect_Ratio[$j],2);
...
//aspect ratio of "rendered image" is 1.4  (700/500)
echo "<th align='center'><img src='$photo_src'  width='700' height='500' title='$Comments2[$j]' /><br />$Photo_location[$j]<br />$Comments[$j]<br />$Width[$j]X$Height[$j] $aspect_ratio2</th>";
//echo "<th align='center'>$Qid[$j]</th>";
echo "</tr>";
echo "<tr><th>$Comment[$j] <br />$Photog[$j]</th></tr>";
//echo "<tr>";
...
echo "<td>
<form method='post' action='slideshow_20150321g_haro.php'>";
