...
if(!empty($_SESSION['reg']))
	{
	$_SESSION[$database]['select']=$_SESSION['reg'];  // Convert district to region
	$_SESSION['parkS']=$_SESSION['reg'];  // Convert district to region
	}
...
$test=$_SESSION['parkS'];
if(!isset($_SESSION['parkStemp'])){$_SESSION['parkStemp']="";}
...
if($_SESSION['logname']=="Lequire7043")
	{
// echo "72<pre>";print_r($_SESSION);echo "<pre>";//exit;
	}
...
if($positionTitle=="Parks District Superintendent")
...
//	include("../../include/parkcodesDiv.inc");
	...
	if($var=="unit")
		...
		$sql = "SELECT * From dprunit_district WHERE parkcode='$parkS'";
		$result1 = mysqli_query($connection,$sql) or die ("Couldn't execute query2. $sql");
		$row1=mysqli_fetch_array($result1);
		extract($row1);
...
//	if(empty($ck_county)){$message="You did not specify your County; please do so.";}
	header("Location: contactInfo_dist.php?emid=$emid&message=$message");
	exit;} // end Update
...
if($emid!="")
	...
	$sql = "SELECT position.posTitle,position.beacon_num,emplist.jobTitle, position.park_reg as currPark, empinfo.Nname, empinfo.Fname, empinfo.Mname, empinfo.Lname, empinfo.suffix, empinfo.badge, empinfo.radio_call_number, empinfo.add1, empinfo.add2, empinfo.city, empinfo.state, empinfo.zip, empinfo.countycode, empinfo.email,
	t4.ophone,
	t4.fax,
	empinfo.phone,
	empinfo.work_cell,
	...
	LEFT JOIN dpr_system.dprunit_district as t4 on emplist.currPark=t4.parkcode
...
// 	echo "<pre>"; print_r($_SESSION); echo "</pre>"; // exit;
	$district=$_SESSION['parkR'];
	$text="array".$district; $distArray=${$text};
// 	echo "$currPark<pre>"; print_r($distArray); echo "</pre>"; // exit;
...
echo "<br><br>$accessLevel $positionTitle $parkS  - Click the browser back button to return to list.";//exit;

// ******* Validate User **************
if($accessLevel=="viewEdit")
	{
	include("../../include/parkcountyRCC_reg.inc");
	$sql="select county from divper.nc_counties order by county";
	$result_c = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql ");
...
//	echo"<pre>";print_r($new_countyCodeList);echo "</pre>";
	echo "<form action='contactInfo_dist.php'>";
	while (list($key,$val)=each($row))
		{		
		if(array_key_exists($key,$fieldArray) and $key!="0")
			{
