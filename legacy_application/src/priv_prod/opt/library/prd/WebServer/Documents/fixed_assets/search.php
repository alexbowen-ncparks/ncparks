...
// echo "$sql<pre>"; print_r($park_array); echo "</pre>"; // exit;
...
<td align='center'>Controller's Office Code<br />
...
if($submit=="Search")
	{
	$sql="select *  from $table where asset_description like '%$searchterm%' order by location";
	echo "$sql";
	$result = mysqli_query($connection,$sql) or die ("Couldn't execute query 140. $sql ".mysqli_error($connection));
	while ($row=mysqli_fetch_assoc($result))
		{
