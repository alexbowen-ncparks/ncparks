...
// echo "<pre>"; print_r($_POST); echo "</pre>";  exit;
	...
	if(@$_SESSION['parking']['level']<1)
		...
		echo "While you can view the DPR Festival Trailer database without logging in, you must login in order to make changes. Click <a href='parking.html'>here</a> to login.";
	exit;
		}
	...
	mysqli_select_db($connection,$database); // database 
		...
		if(!EMPTY($_POST['space']))
			{
			$sql="SELECT  all_day from `festival_dates`
...
// 	echo "check record $sql<br />$blank_name<br />";   exit;
			...
			$sql="UPDATE festival_dates 
			SET all_day=''
			where date_id = '$_POST[pass_edit]' and space='$space' $blank_name
			";  
			$result = @mysqli_query($connection,$sql) or die(mysqli_error($connection));
			
			if(@$_POST['submit_form']=="Release Festival Trailer")
				{
				$sql="DELETE FROM festival_dates 
				where date_id = '$_POST[pass_edit]' and space='$space'
				";  
				$result = @mysqli_query($connection,$sql) or die(mysqli_error($connection));
// 				echo "The van will now be available for that date.";
header("Location: festival.php?cal_type=festival");
				exit;
				}
			}
// 		if(!empty($exist))
...
// echo "<pre>"; print_r($_POST); echo "</pre>";  exit;
		...
		$sql="UPDATE ignore festival_dates 
...
//echo "add update2 $sql<br /><br />"; //exit;
			...
			if($num<1)
			{
			$sql="INSERT ignore INTO festival_dates 
		SET all_day='$_POST[all_day]', date_id = '$_POST[pass_edit]', space='$space', tempID='$tempID'";  
...
$sql="SELECT * from festival_dates where date_id like '$check_month%'"; //echo "$sql";
$result = @mysqli_query($connection,$sql) or die(mysqli_error($connection));
while($row=mysqli_fetch_assoc($result))
	{
	$ARRAY[]=$row;
...
include("cal_array.php");  // lists the various items being tracked
	...
	foreach($cal_array as $k=>$v)
		{
		if($cal_type=="festival" and $k=="festival"){$s="selected";}else{$s="";}
...
<td align="center"><font color='green' size='+2'>DPR's Festival Trailer</font>
...
for ($i=0; $i<($maxday+$startday); $i++)
	{
		...
		$display="<a href='cal.php?cal_type=festival&pass_edit=$edit'>$day</a>";
...
<?php
echo "<table align='center'><tr><td><font color='green'><i><b>The Festival Trailer is stationed at the Yorkshire Center 12700 Bayleaf Church Road.</b></i></font><br /><br />If reserving the Festival trailer, it is also your responsibility to coordinate returning the trailer to the Yorkshire Center in a timely manner.</td></tr></table>";
...
$bump_message="NOTICE:  A message indicating any special conditions, etc. can go here";
