...
foreach($_POST['delete'] as $k=>$beacon_posnum)
	...
	if(in_array($beacon_posnum,@$_POST['position']))
		...
		$hours_b=$_POST['budget_hrs_b'][$beacon_posnum];
// 		$hours_c=$_POST['budget_hrs_c'][$beacon_posnum];
		$weeks_a=$_POST['budget_weeks_a'][$beacon_posnum];
		$weeks_b=$_POST['budget_weeks_b'][$beacon_posnum];
...
// 		$park_comments=addslashes($_POST['park_comments'][$beacon_posnum]);
		...
		$sql="UPDATE seasonal_payroll_next_split set budget_hrs_a='$hours_a', budget_weeks_a='$weeks_a',  budget_hrs_b='$hours_b', budget_weeks_b='$weeks_b', park_comments='$park_comments', start_date='$start_date', month_11='$month_11', aca='$aca'
...
foreach($_POST['delete'] as $k=>$beacon_posnum)
	...
	if(@!in_array($beacon_posnum,$_POST['position']) OR $_POST['start_date'][$beacon_posnum]=="")
		{
		$sql="UPDATE seasonal_payroll_next_split set park_approve=''
		where beacon_posnum='$beacon_posnum'";
	//	echo "$sql";exit;
		$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql ");
				continue;
...
$center_code=$_POST['center_code'];
		header("Location: /hr/bSeasonal/park_seasonals_next_split.php?center_code=$center_code");
?>