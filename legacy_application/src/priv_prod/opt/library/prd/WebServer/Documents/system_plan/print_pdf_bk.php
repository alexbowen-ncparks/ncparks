...
from stats_day
where park='$parkcode' and left(year_month_day,4)='$prev_yr'
...
if($text=="")
	{$text="The interpretive themes for this park has not yet been entered.\n\n";}
...
if($land>0)
	{
	$acreage="Acreage: ".number_format($land,0)." land acres";
	}
if($water>0)
	{
	@$acreage.="/ ".number_format($water,0)." water acres";
	}
...
$split1=explode("/",$county);
	if(count($split)>1 OR count($split1)>1)
		{
		$var=implode(", ",$split);
		$varCounty="   Counties: ".$var;
		}
		else
		{$varCounty="   County: ".$county;}
...
$pass_texty=$texty;
		...
		PDF_set_text_pos($pdf, 50, $texty-26);
...
$text="Permanent Staff";
		...
		PDF_set_text_pos($pdf, 300, $pass_texty-26);
		PDF_show($pdf, $text);
