...
// 	@$var_dist_park[$v]=$district[$v];
	@$var_district[]=$district[$v];
	@$var_district_park[$v]=$district[$v];
...
// echo "<pre>"; print_r($var_district); echo "</pre>"; // exit;
// echo "<pre>"; print_r($var_district); echo "</pre>"; // exit;
foreach($var_district as $k=>$v)
	{
	$sql="SELECT t1.email
	FROM empinfo as t1
	left join emplist as t2 on t1.emid=t2.emid
...
// 	echo "$sql"; exit;
	...
	if(mysqli_num_rows($result)>0)
		...
		IF($v=="EADI")
			{
			$disu_email[]="sarah.kendrick@ncparks.gov";
			}
		}
	}
...
// 				}
		...
		$all_disu_email['EADI']="sarah.kendrick@ncparks.gov";
		}
//echo "<pre>"; print_r($disu_email); echo "</pre>"; // exit;
...
if(mysqli_num_rows($result)>0)
	...
	{
	$dedi_email[]="brian.strong@ncparks.gov";
	}
