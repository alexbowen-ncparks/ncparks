...
if ($_FILES['files']['error'][0] == 0)
	...
	$sql="REPLACE form_employee_statement_upload (wc_id,file_name) "."VALUES ('$wc_id','$file_name')";

//	echo "$sql <br />";exit;
	$result = @mysqli_query($connection, $sql) or die("$sql<br />Error #". mysqli_errno($connection) . ": " . mysqli_error($connection));

		$id= mysqli_insert_id($connection);
	// remove any existing file for form_employee_statement_upload
			$sql = "SELECT file_link from form_employee_statement_upload  where wc_id='$wc_id'";
	...
	$year=date("Y");
	$ts=time();
	$uploaddir = "upload_form_employee_statement/".$year; // make sure www has r/w permissions on this folder

	//    echo "$uploaddir"; exit;
	if (!file_exists($uploaddir)) {mkdir ($uploaddir, 0777);}

	$new_name="upload_form_employee_statement_".$wc_id."_".$ts;
	...
	$path_to_file="/opt/library/prd/WebServer/Documents/work_comp/".$uploadfile;
		...
		if(file_exists($path_to_file))
			{
			$sql = "UPDATE form_employee_statement_upload set file_link='$uploadfile' where id='$id'";
			$result = @mysqli_query($connection, $sql) or die("$sql Error 1#". mysqli_errno($connection) . ": " . mysqli_error($connection));
			}
			else
