...
// START: queries necessary to reset to Manager Approval=NO for several TABLES when Editing the LAST "fully approved invoice"
if($reset=='y')
{
$query="update `budget_service_contracts`.`invoices` set `manager_approved`='n',`manager_date`='0000-00-00',`manager`='' where `id`='$eid' ";
$result=mysqli_query($connection,$query) or die ("Couldn't execute query . $query");   
        
$query="update `budget_service_contracts`.`invoices_paylines` set `manager_approved`='n' where `scid`='$scid' and `invoice_num`='$invoice_num' ";
$result=mysqli_query($connection,$query) or die ("Couldn't execute query . $query"); 

$query="update `budget_service_contracts`.`pay_lines` set `cashier_approved`='n',`manager_approved`='n' where `scid`='$scid' and `invoice_num`='$invoice_num'  ";
$result=mysqli_query($connection,$query) or die ("Couldn't execute query . $query"); 
}
// END: queries necessary to reset to Manager Approval=NO for several TABLES when Editing the LAST "fully approved invoice"
...
echo "<tr><th align='center'><font color='brown'>Center</font></th><td><font color='brown'>$center</font></td></tr>";
echo "<tr><th align='center'><font color='brown'>Contractor</font></th><td><font color='brown'>$vendor</font></td></tr>";
echo "<tr><th align='center'><font color='brown'>Remit to Name</font></th><td><font color='brown'>$remit_name</font></td></tr>";
...
echo "<tr><th align='center'><font color='brown'>NCAS#</font></th><td><font color='brown'>$ncas_account</font></td></tr>";
...
echo "<tr>";
//echo "<th>Cashier: $tempid<br />  <font color='green'>Approved</font>:<input type='checkbox' name='cashier_approved' value='y' ></th>";
echo "<th colspan='2'><input type=submit name=submit value=Update></th>";
//echo "<th>lines_overdrawn:$lineS_overdrawn<br />payline_check=$payline_check</th>";
...
if($lineS_overdrawn==0 and $payline_check==0)
{echo "<form action='po_invoice_cashier_approval.php'>";
echo "<th>Cashier: $tempid<br />  <font color='green'>Approved</font>:<input type='checkbox' name='cashier_approved' value='y' ><input type='submit' name='submit' value='Submit'></th>";
echo "<input type='hidden' name='scid' value='$scid'>";
