...
//      echo "<pre>"; print_r($location); echo "</pre>"; // exit;
      for($j=1; $j<11; $j++)
		...
		if($header1=="comments")
			{
			$line="<td><textarea name='$fld_name' cols='25' rows='2'></textarea></td>";
			$line.="<td bgcolor='lightblue'>
			<input type='file' name='images[$j]'/>
			<input type='hidden' name='images[$j]'/>
			</td>";
			}
		...
		if($header1=="condition")
			{
			echo "<td bgcolor='lightblue'><select name='$fld_name'><option value='' selected></option>\n";
