...
echo "<br />year2_start_date=$year2_start_date<br />";
echo "<br />year2_end_date=$year2_end_date<br />";
...
$query3="select account,account_description,budget_group,cash_type,gmp,sum(year1) as 'year1',sum(year2) as 'year2',sum(year1-year2) as 'difference'
         from $table2 where 1 and beacnum='$beacnum'
		 $where_section
		 $where_accounts
		 $where_district
		 group by account,budget_group order by cash_type desc,budget_group asc,account asc ";
...
$query4b="select
         sum(year1-year2) as 'difference_total'		 
         from $table2 where 1 and beacnum='$beacnum'
         $where_section	
         $where_accounts
         $where_district		 
...
extract($row5);
*/
echo "<table align='center'><tr><td>Accounts:<font color='red'> $num3 </font></td></tr></table>";
...
echo "<th align=left><font color=brown>Account<br />Description</font></th>"; 
...
"<tr$t>"; 
        ...
        // echo "<td><a href='reports_one_budget_group_summary_by_center.php?budget_group=$budget_group'>$budget_group</a></td>"; 
		 //echo "<td><a href='budget_group_range_history_accounts.php?report=$report&section=$section&district=$district&accounts=$accounts&budget_group=$budget_group&period=$period&range_year_start=$range_year_start&range_year_start2=$range_year_start2&range_month_start=$range_month_start&range_day_start=$range_day_start&range_year_end=$range_year_end&range_year_end2=$range_year_end2&range_month_end=$range_month_end&range_day_end=$range_day_end&parkcode=$parkcode'>$budget_group</a></td>";
