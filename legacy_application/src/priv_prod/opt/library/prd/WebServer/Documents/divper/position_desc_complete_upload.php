...
if ($submit == "Add File")
	...
	$ts=time();
	$file = $beacon_num."_".$_REQUEST['form_name']."_".$ts.".".$ext;
	
	if($ext=="docx")
		{
		$remove_file = $beacon_num."_".$_REQUEST['form_name'].".doc";
		}
	if($ext=="doc")
		{
		$remove_file = $beacon_num."_".$_REQUEST['form_name'].".docx";
		}
	
//	echo "$remove_file<pre>";print_r($_FILES); print_r($_REQUEST);echo "</pre>"; //exit;
	...
	$folder = "position_desc_files/completed";
	if (!file_exists($folder)) {mkdir ($folder, 0777);}
...
//	echo "$uploadfile";exit;
	...
	$sql = "REPLACE position_desc_complete set beacon_num='$beacon_num', form_name='$form_name', beacon_title='$beacon_title', file_link='$uploadfile'";
	//  echo "$sql";exit;
