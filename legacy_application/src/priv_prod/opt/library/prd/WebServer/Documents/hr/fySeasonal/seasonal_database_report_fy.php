...
$display=array("district","center_code","beacon_posnum","osbm_title","start_date",$pay_rate,"ncas_account","div_app","park_approve","comments","park_comments","budget_weeks_a","budget_hrs_a");
...
FROM seasonal_payroll_fy as t1
WHERE (park_approve='y' AND (district='east' OR  district='north' OR  district='south' OR  district='west')) OR (t1.center_code='OPAD' AND park_approve='y' and start_date!='')"; 
// echo "$sql<br />"; //exit;
...
$sql = "SELECT sum(t1.budget_hrs_".$period."*(budget_weeks_".$period."+($adjust))*avg_rate_new) as total
FROM seasonal_payroll_fy as t1
WHERE (park_approve='y') AND (district!='east' AND  district!='north' AND  district!='south' AND  district!='west') AND t1.center_code!='OPAD'"; 
// echo "$sql"; //exit;
$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql");
$row=mysqli_fetch_assoc($result);{$others=$row['total'];}

$district_array=array();
$sql = "SELECT district,ncas_account, sum(t1.budget_hrs_".$period."*(budget_weeks_".$period."+($adjust))*avg_rate_new) as total
FROM seasonal_payroll_fy as t1
...
if($level>3 AND @$rep==""){
	include("menu_fy.php");
	echo "<div align='center'><form method='POST'><table cellpadding='5'><tr><td>Excel <a href=\"seasonal_database_report_fy.php?submit=Find&rep=excel$passQuery\">export</a></td>";
...
IF(@$rep=="")
	...
	echo "<table border='1' cellpadding='3'><tr><td colspan='11' align='center'>$num positions for <font color='red'>Fiscal Year beginning $new_request_date</font></td></tr><tr><td colspan='11' align='center'>Operations: (east, north, south, west, OPAD) <b>$operations</b> + Others: (ADMI, DEDE, NARA, NRTF, PAR3, USBG) <b>$others</b> = <b>$gt</b> - <font size='-1'>Scroll to bottom for 1311/1312 subtotals</font></td></tr>";
	...
	// Parks	
		...
		echo "</td></tr></table></td><td align='center'><a href='seasonal_database_report_fy.php?rep=excel&scope=centers$aa'>export</a><br />Centers</td></tr></table>";
	}
				...
				if(in_array($fld_name,$display)){
						...
						if($fld_name=="district")
							...
							if(@$track_district!=$value)
								...
								$dist_total=array_sum($district_array[$value]);
