...
if(@!$_POST['reg_app'] AND @!$_POST['park_approve'])
	{
	$_POST['reg_app']="y";$reg_app="y";
...
$display=array("region","center_code","beacon_posnum","osbm_title","start_date",$pay_rate,"ncas_account","reg_app","park_approve","comments","park_comments","budget_weeks_a","budget_hrs_a");
$display_1=array("budget_weeks","budget_hrs");
//echo "<pre>"; print_r($display_1); echo "</pre>"; // exit;
...
FROM seasonal_payroll_fy_reg as t1
WHERE (park_approve='y' AND (region='CORE' OR  region='PIRE' OR  region='MORE')) OR (t1.center_code='OPAD' AND park_approve='y' and start_date!='')";
//  echo "$sql<br />"; //exit;
$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql");
$row=mysqli_fetch_assoc($result);
$operations=$row['total'];

$sql = "SELECT sum(t1.budget_hrs_".$period."*(budget_weeks_".$period."+($adjust))*avg_rate_new) as total
FROM seasonal_payroll_fy_reg as t1
WHERE (park_approve='y') AND (region!='CORE' AND  region!='PIRE' AND  region!='MORE') AND t1.center_code!='OPAD'"; 
//      echo "$sql"; //exit;
$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql");
$row=mysqli_fetch_assoc($result);
$others=$row['total'];
...
$region_array=array();
$sql = "SELECT region,ncas_account, sum(t1.budget_hrs_".$period."*(budget_weeks_".$period."+($adjust))*avg_rate_new) as total
FROM seasonal_payroll_fy_reg as t1
WHERE 1 and park_approve='y'
group by region,t1.ncas_account
order by region"; //echo "$sql"; //exit;
$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql");
while($row=mysqli_fetch_assoc($result))
	{
	$region_array[$row['region']][$row['ncas_account']]=$row['total'];
	}
...
if($submit=="Find")
	...
	//		echo "<pre>"; print_r($_POST); echo "</pre>"; // exit;
		...
		if(@$sort=="")
			{$orderBy="t1.region,t1.center_code,t1.osbm_title";}
		else
...
if($level>3 AND @$rep==""){
// 	include("menu_fy.php");
	include("menu_fiscal_year.php");
	echo "<div align='center'><form method='POST'><table cellpadding='5'><tr><td>Excel <a href=\"seasonal_database_report_fiscal_year_reg.php?submit=Find&rep=excel$passQuery\">export</a></td>";

		
		echo "<td align='center'>region<br /><select name='region'><option select=''></option>";
		foreach($region_array as $k=>$v){
			if($k==@$region){$s="selected";}else{$s="value";}
		...
		echo "<td align='center'>Reg Approve<br /><select name='reg_app'><option select=''></option>";
		foreach($reg_array as $k=>$v){
			if($v==$reg_app){$s="selected";}else{$s="value";}
	...
	//	echo "<td align='center'>Adjust Weeks (+ -)<br /><input type='text' name='adjust' value='$adjust' size='2'></td>";
		...
		if(@$_POST['budget_hrs_'.$period]=="0"){$ck0=" checked";}else{$ck0="";}
...
if($level<4 AND @$rep==""){
	include("menu_fy.php");
	echo "<div align='center'><form method='POST'><table cellpadding='5'><tr>";
	...
	echo "<table border='1' cellpadding='3'><tr><td colspan='11' align='center'>$num positions for <font color='red'>Fiscal Year beginning $new_request_date</font></td></tr>
	<tr><td colspan='11' align='center'>Operations: (CORE, PIRE, MORE, OPAD) <b>$operations</b> + Others: (ADMI, DEDE, NARA, NRTF, PAR3, USBG) <b>$others</b> = <b>$gt</b> - <font size='-1'>Scroll to bottom for 1311/1312 subtotals</font></td></tr>";
	...
	// Parks	
		...
		echo "</td></tr></table></td><td align='center'><a href='seasonal_database_report_fiscal_year_reg.php?rep=excel&scope=centers$aa'>export</a><br />Centers</td></tr></table>";
	}
		...
		echo "<tr>";
			foreach($fields as $fld_name=>$value){
				if(in_array($fld_name,$display)){
						...
						if($fld_name=="region")
							{
							if(@$track_region!=$value)
								{
								$td=" bgcolor='yellow'";
								$track_region=$value;
								$dist_total=array_sum($region_array[$value]);
								$value.="<br />".number_format($dist_total,2);
								}
							}
