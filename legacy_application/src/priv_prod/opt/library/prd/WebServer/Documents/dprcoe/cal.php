<?php
if($_REQUEST['month']=="All year" and $_REQUEST['cal_level']=="STWD_District_Park")
	{
	header("Location: http://www.dpr.ncparks.gov/dprcoe/find.php?Submit=Find&ann_100=x");
	exit;
	}
...
if(count($ARRAY)==1 and $test=="All year")
	{
	$exp=explode("-",$ARRAY[0]['dateE']);
	$cMonth=$exp[1];
	$cYear=$exp[0];
	$test=$cMonth;
	$pass_month=$cMonth;
	$check_month=$cYear."-".$cMonth;
	$limit_month=" and dateE like '$check_month%'";
	$sql="SELECT *
	from dprcoe.event
	where 1 and ann_100='x' $limit_month
...
$ignore_month=array("EADI","NODI","SODI","WEDI");
if(count($ARRAY)>1 and $test=="All year")
	{
	$c=count($ARRAY);
	echo "<table border='1' cellpadding='3'><tr><td colspan='13'>$c 100th Anniversary Events</td></tr>";
	foreach($ARRAY AS $index=>$array)
		...
		foreach($array as $fld=>$value)
			...
			if($fld=="dateE")
				{
				$exp=explode("-",$value);
				$cal_level=$array['park'];
				$value="<a href='cal.php?cal_level=$cal_level&year=$exp[0]&month=$exp[1]'>$value</a>";
				}
			echo "<td>$value</td>";
			}
		echo "</tr>";
...
$month_array[]="All year";
$monthNames[12]="All Year";
//echo "<pre>"; print_r($month_array); echo "</pre>"; // exit;
