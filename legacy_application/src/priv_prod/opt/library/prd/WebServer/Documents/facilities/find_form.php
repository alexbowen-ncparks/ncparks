...
if(@$message)
	...
	$sql = "SELECT distinct park_abbr
		...
		$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql $database");
...
// 			}
		...
		if($fld=="lease_period")
			{
			$fld_comment="<font size='-2'>mm/yyyy</font>";
			}
			
		$var_fld=$fld;
		if($fld=="rent_fee")
			{$var_fld="charged_rent";}
		echo "<td>$var_fld $fld_comment<br />";
...
// 			}
		if($fld=="rent_fee")
			...
			if($level>1)
				...
				{
				$display_input=@number_format($value,2);
				}
			}
		if($fld=="fac_name")
			{
		...
		if($fld=="housing_gis_id")
			{
			$display_input.=" <a href='edit_fac.php?file=park_abbr&gis_id=$gis_id'>Add Photo</a></td><td>";
			if(!empty($fac_photo_array))
				{
				foreach($fac_photo_array as $var_k=>$var_v)
					{
					if($var_v==0){continue;}
					$link="/facilities/get_photo.php?pid=$var_v";
					$sql="SELECT link from photos.images where pid='$var_v'";
		...
		// 	if(!empty($housing_attachment_array))
// 				{
// 				$display_input.="</td><td>";
// 				foreach($housing_attachment_array as $var_k=>$var_array)
// 					{
// 					$link=$var_array[0];
// 					$name=array_pop(explode("/",$link));
// 					if($var_array[1]=="x"){$ha=" housing agreement";}else{$ha="";}
// 					$display_input.="<a href='$link' target='_blank'>$name</a> $ha<br />";
// 					
// 					}
// 				}
			$display_input.="</td><td>";
...
if(!empty($submit_label) and $submit_label!="Find")
	{echo "<td>spo_comment: <textarea name='spo_comment' cols='$cols' rows='$rows'>$spo_comment</textarea></td>";
	}

	
	//$housing_form_array from find.php  query to FIND #600
	// #508 was deleted on 2016-09-22 per Adrian O'Neal since new policy was implemented
	if(!empty($housing_form_array))
		{
		echo "<td colspan='3'>
		<table border='1' cellpadding='5'><tr><td><a href='/find/forum.php?forumID=600&submit=Go' target='_blank'>FIND #600</a> stores the following documents:</td></tr>";
...
if($level>2)
	{	
	echo "<td colspan='2'>List all park residences: <a href='find.php?fac_type=Park Residence&status=Active&submit_label=Go to Find'>link</a></td>";
	}
