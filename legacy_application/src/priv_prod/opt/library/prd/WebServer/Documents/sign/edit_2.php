...
// 2022-05-06: CCooper - adding North:65030652
$dist_beacon_num="t3.beacon_num='60032912' OR t3.beacon_num='60032913' OR t3.beacon_num='60033019' OR t3.beacon_num='65030652'";
...
if($ARRAY[0]['approv_BPA']!="0000-00-00")
	{echo "<body onload='scrollWindow();' bgcolor='beige' class=\"yui-skin-sam\">";}
	else
...
if($level>3)
	{$change="Change <a href='change_category.php?id=$edit'>Category</a> from $cat to something else.";}
	else
	{$change="";}
		...
		foreach($row as $fld=>$value)
			{
				...
				//	if($value){$d="block";}else{$d="none";}
					...
					<br />Enter the text that will appear on the sign. Provide any Special Instructions after the text.<br /><textarea name='$fld' cols='55' rows='4'>$value</textarea>PIO_comment<textarea name='pio_comment' cols='55' rows='2'>$row[pio_comment]</textarea><br />";
//***********************					
					include("uploads.php");
						echo "</div>";
				...
				if($fld=="comments")
					{
					if($value){$d="block";}else{$d="none";}
					if($level==1){$explain=" Comments box is for DISU and CHOP. <br />Park comments can go in the \"Description\" box.";}else{$explain="";}
					$item="<div id=\"$fld\">   ... <a onclick=\"toggleDiv('fieldDetails[$fld]');\" href=\"javascript:void('')\"> toggle &#177</a></div>

					<div id=\"fieldDetails[$fld]\" style=\"display: $d\"><br><textarea name='$fld' cols='55' rows='15' $RO>$value</textarea>$explain</div>";
					}
				...
				foreach($standard_sign_array as $k1=>$v1)
									{
									$k2="3.".$k1;
									$values=explode(".",$ck_db_value);
									if($values[1]=="")
										{$s="value";}
										else
										{
