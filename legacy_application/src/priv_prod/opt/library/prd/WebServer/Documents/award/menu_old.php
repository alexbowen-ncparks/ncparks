...
extract($_REQUEST);
$level=$_SESSION['award']['level'];
$tempID=$_SESSION['award']['tempID'];

if($level<3)
	{
	$pass_park=$_SESSION['award']['select'];
	}

//if($level<1){echo "You must be logged into the Award Database to use it.<br /><br />Login <a href='http://www.dpr.ncparks.gov/sign/'>here</a>."; exit;}
...
function valFrm()
	{
	 if (!checkRadio(\"myForm\",\"category\")) 
	alert(\"You must select an Award Category.\");
		var x=document.forms['myForm']['nom_name'].value;
		if (x==null || x=='')
		  {
		  alert('You must enter an nominee\'s name.');
		  return false;
		  }
		var x=document.forms['myForm']['justification'].value;
		if (x==null || x=='')
		  {
		  alert('You must enter a justification.');
		  return false;
		  }
		var x=document.forms['myForm']['email'].value;
		if (x==null || x=='')
		  {
		  alert('You must include a contact email.');
		  return false;
		  }
	} 
//-->
...
<?php
echo "<title>DPR Award Nomination</title>";
@include("../css/TDnull.inc");
	
//	echo "<pre>"; print_r($_SERVER); echo "</pre>"; // exit;
echo "<body><div align='center'>";

if($_SERVER['PHP_SELF']=="/award/menu.php")
	{//<img src='images/clne.jpg'>
	echo "<font color='brown'><img src='../inc/css/images/dpr_1.jpg'><br /><br />Welcome to the NC DPR Award Nomination Website</font>";
	}

$color_array=array("Track an Award Nomination"=>"lightgreen","Award Forms/Guidelines/Appendix"=>"yellow","Instructions"=>"#C6E2FF","Submit an Award Nomination"=>"#FFE4C4","Portal"=>"#00FFFF","Awards Given"=>"violet");

$path="/award/";

$menu_array['Track an Award Nomination']=$path."track.php";

$menu_array['Submit an Award Nomination']=$path."add_request.php";

$menu_array['Awards Given']=$path."given.php";

$menu_array['Award Forms/Guidelines/Appendix']="http://www.dpr.ncparks.gov/find/forum.php?forumID=396&submit=Go";

$menu_array['Instructions']="http://www.dpr.ncparks.gov/find/forum.php?forumID=291&submit=Go";
...
if($level>4)
	{
	$menu_array['Portal']="../portal.php?database=award";
	}
...
foreach($menu_array as $k=>$v)
	{
		$color=$color_array[$k];
		if($k=="Award Forms/Guidelines/Appendix"||$k=="Instructions"||$k=="Portal")
		{
		$v=htmlentities($v);
