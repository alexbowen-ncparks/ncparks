...
if ($submit == "Add File")
	...
	$folder = "position_desc_files/pdf";
	if (!file_exists($folder)) {mkdir ($folder, 0777);}
...
//	echo "$uploadfile";exit;
	...
	if(!empty($remove_file)){unlink($delete_file);}
	
	move_uploaded_file($file_upload['tmp_name'],$uploadfile);// create file on server
	
	$sql = "REPLACE position_desc_pdf set beacon_num='$beacon_num', form_name='$form_name', beacon_title='$beacon_title', file_link='$uploadfile'";
	//  echo "$sql";exit;
