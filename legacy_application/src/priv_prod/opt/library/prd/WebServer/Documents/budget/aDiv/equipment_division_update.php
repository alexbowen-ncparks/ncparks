...
//echo "hello";
//if($submit != "Add"){$submit="Update"; echo "submit=$submit"; exit;}
...
if($submit=="Update")
	 ...
	 foreach ($newKey as $key => $value)
		{
		 ...
		 foreach ($value as $fld => $val)
			...
			if(($fld=="order_complete"||$fld=="receive_complete"||$fld=="paid_in_full") and $newKey[$key]['ordered_amount']=="0.00")
				{$val="n";}// never allow y 
	...
	if($level>4)
		...
		/*
		$sql="update budget_center_allocations,center
		      set budget_center_allocations.center=center.old_center
			  where budget_center_allocations.center=center.new_center
			  and budget_center_allocations.budget_group='equipment'
			  and allocation_justification='approved_equipment_purchase'
			  and fy_req='$f_year' ";
