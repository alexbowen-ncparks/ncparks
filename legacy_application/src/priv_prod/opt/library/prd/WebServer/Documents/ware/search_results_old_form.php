<?php
	...
	foreach($COLS AS $index=>$fld)
		...
		echo "<tr><td colspan='3'>DPR Warehouse Inventory ";
		if(!empty($c))
		...
		echo "<div id='hidden_value1'><input type='text' name='park_code' value=''></div>";
	...
	if(!empty($_GET['id']))
		...
		$readonly=array("id");
		$sql="select t1.*, group_concat(concat(t2.photo_id,'*',t2.link)) as link 
		from base_inventory as t1
