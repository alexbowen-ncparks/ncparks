...
// $exclude_flds=array("employee_street","employee_city","employee_state","employee_zip","employee_work_phone","employee_sex","employee_sex","employee_dob","location_2","employer_premises_2","supervisor_name_8","county_2","department_2");
$include_flds=array("park_code","date_of_injury","employee_name","employee_home_phone","incident_supervisor","supervisor_phone","incident_classification","treatment_required","return_to_work","hazard_type","witness_names","incident_info","job_activity","hazard_review","activity_training","root_cause","prevent_reoccurrence","supervisor_name","manager_name","supervisor_sig","manager_sig","supervisor_date","manager_date");
...
while($row=mysqli_fetch_assoc($result))
	{
	if(in_array($row['Field'], $include_flds))
		{
		$ARRAY[]=$row['Field'];
		}
	}
...
$rename1=array("park_code"=>"Park Code","date_of_injury"=>"Date of Injury","employee_name"=>"Employee's Name","employee_street"=>"Employee's Street Address","employee_city"=>"Employee's City","employee_state"=>"Employee's State","employee_zip"=>"Employee's Zip","employee_home_phone"=>"Employee's Phone","employee_work_phone"=>"Employee's Work Phone","employee_sex"=>"Employee's Sex","employee_dob"=>"Employee's Date of Birth","incident_supervisor"=>"Incident Supervisor","supervisor_phone"=>"Supervisor Phone","incident_classification"=>"Incident Classification","treatment_required"=>"Employee Treatment","return_to_work"=>"Employee Return to Work","hazard_type"=>"Hazard Type","witness_names"=>"Witness Names","incident_info"=>"Incident Description","job_activity"=>"Job Activity","hazard_review"=>"Hazard Review","activity_training"=>"Activity Training","root_cause"=>"Root Cause","prevent_reoccurrence"=>"Prevent Reoccurrence","supervisor_name"=>"Supervisor Name","manager_name"=>"Manager Name","supervisor_sig"=>"Supervisor Signature","manager_sig"=>"Manager Signature","supervisor_date"=>"Supervisor Date","manager_date"=>"Manager Date");

$rename2=array("location_2"=>"Location where injury occurred","county_2"=>"County","department_2"=>"Department","employer_premises_2"=>"State if employer's premises","date_of_injury_3"=>"Date of Injury","day_of_week_4"=>"Day of Week","hour_of_day_4"=>"Hour of Day","paid_entire_day_5"=>"Paid for Entire Day?","begin_disability_6"=>"Date disability began","knew_of_injury_7"=>"Date you or supervisor<br />know of injury","supervisor_name_8"=>"Supervisor Name");
...
if(!empty($_FILES))
	...
	echo "<p><font size='+1'>You have successfully uploaded the <b>Injury Data Collection Form</b>.</font></p>";
	if(@$_POST['reupload']=="reupload")
		...
		echo "Go to Step 2 <a href='treatment_yes_no_2.php?wc_id=$wc_id'>Form</a>";
...
echo "<instructions><a onclick=\"toggleDisplay('instruction');\" href=\"javascript:void('')\">
    ...
    You are in Step 1: Injury Data - <font color='red'>Click for Instructions</font></a>
...
Instructions:<br />
&nbsp;1.   The paper Injury Data Collection Form should already have been completed and scanned.<br />
...
&nbsp;6.  Respond to each item listed below which is the same information identified and required in yellow on the reference Injury Data Collection Form shown to the right.<br />
...
if(empty($_POST['submit']))
	{
// 	<font color='brown'>highlighted</font> 
	echo "<br />
	You must respond to each item below to successfully submit your WC claim.  For incomplete response(s), you will be prompted to complete before proceeding.</home>";
	}
echo "</home>";
...
if((empty($show_form19_upload) and empty($_POST['submit'])) OR !empty($error_message))
	...
	echo "<img src='step1_1.jpg' width='700'>";
	echo "<br /><img src='step1_2.jpg' width='700'>";
	echo "</form19>";
	}
	else
	{
//	echo "s=$show_form19_upload <pre>"; print_r($_POST); echo "</pre>"; // exit;
	...
	echo "<upload19><font size='+2'>You have indicated that all required Injury Data Collection Form fields are properly completed.<br /><br />You can now upload the signed form that has been scanned and saved as a PDF.</font><br /><br />
	<form method='POST' action='new_wc_request_1.php' enctype='multipart/form-data'>
...
$skip[]="body_part";
$skip[]="osha";
echo "<form action='new_wc_request_1.php' method='POST'>
<br />
...
foreach($ARRAY AS $index=>$fld)
	...
	if($fld=="park_code")
		{
		echo "<td bgcolor='beige'><fieldset><legend><b>Basic Information</b></legend><table>";
		}
	...
	if($fld=="witness_names")
		{
		echo "</table></fieldset><tr>
		<td bgcolor='white'><fieldset><legend><b>Incident Information</b></legend><table>";
		}
	if($fld=="prevent_reoccurrence")
		{
		echo "</table></fieldset><tr>
		<td bgcolor='beige'><fieldset><legend><b>Names and Signatures</b></legend><table>";
		}
