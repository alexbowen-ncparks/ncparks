...
echo "<table align='center' border='1'><tr><th><b>Records: <font color='red'>$num</font></b></th></tr></table>";
...
echo "<table align='center' border='1'>";
echo "<tr>";
echo "<th><b>Records: <font color='red' size='4'>$num</font></b></th>";
if($message=='yes')
{
echo "<th>
<img height='25' width='25' src='/budget/infotrack/icon_photos/green_checkmark1.png' alt='picture of green check mark'></img><br /><font color='green'>Update Successful</font>
</th>";
}
echo "</tr>";
echo "</table>";
echo "<table align='center' border='1'>
<tr><th>center</th><th>project_number</th><th>vendor_name</th><th>system_entry_date</th><th>transaction_date</th><th>transaction_number</th><th>transaction_type</th><th>transaction_amount</th><th>source_id</th></tr>";
$i=0;
while($row=mysqli_fetch_array($result)){
extract($row);
...
$subCenter="";
//echo "<tr><td>$center</td><td>$project_number</td><td>$vendor_name</td><td>$transaction_date</td><td>$transaction_number</td><td>$transaction_type</td><td align='right'>$transaction_amount</td><td>$source_id</td><td><table border='1'><tr><td><a href='post_update.php?transaction_type=$transaction_type&source_id=$source_id&center=$center&projnum=$projnum' target='_blank'>Mark Paid</a></td></tr></table></td></tr>";}
echo "<tr><td>$center</td><td>$project_number</td><td>$vendor_name</td><td>$system_entry_date</td><td>$transaction_date</td><td>$transaction_number</td><td>$transaction_type</td><td align='right'>$transaction_amount</td><td>$source_id</td></tr>";}
...
$subCenterF=number_format($subCenter,2);
$totalF=number_format($total,2);
echo "<tr bgcolor='aliceblue'><td colspan='7' align='right'><b>Subtotal for Center $ckCenter:</b></td><td  align='right'>$subCenterF</td></tr>
<tr><td colspan='7' align='right'><b>Grand Total:</b></td><td  align='right'><b>$totalF</b></td></tr>";
...
//echo "<br />Line 228: parkcode=$parkcode<br />";
...
{
$rep=$_SESSION['budget']['report'];echo "You do not have access privileges for this database [$db] report at $level $posTitle. Contact Database Support database.support@ncparks.gov if you wish to gain access. 271<br><br>budget.php<br> a is ";print_r($a);echo "<br />report is ";print_r($_SESSION['budget']['report']);exit;}
}
...
//2
 $query = "insert into project_unposted1( center, project_number,account, vendor_name, system_entry_date, transaction_date, transaction_number, transaction_type, transaction_amount,source_id,parkcode) select ncas_center, project_number,ncas_account, vendor_name, system_entry_date, datesql, ncas_invoice_number,'cdcs', ncas_invoice_amount, id,'' from cid_vendor_invoice_payments where 1 and post2ncas != 'y' and ncas_credit != 'x' group by id;
";
    $result = @mysqli_query($connection, $query);
if(@$showSQL=="1"){echo "<br><br>$query<br><br>";}
...
//4
 $query = "insert into project_unposted1(center, project_number,account, vendor_name, system_entry_date, transaction_date, transaction_number, transaction_type, transaction_amount,source_id,parkcode  ) select center, projnum,ncasnum, concat('pcard','-',cardholder,'-',vendor_name,'-',trans_date), xtnd_rundate_new, transdate_new, transid_new, 'pcard',sum(amount), id,'' from pcard_unreconciled where 1 and ncas_yn != 'y' group by id;
";
    $result = @mysqli_query($connection, $query);
if(@$showSQL=="1"){echo "<br><br>$query<br><br>";}
...
echo "<br />Line 341: query=$query<br />";
...
";
   $result = @mysqli_query($connection, $query);
if(@$showSQL=="1"){echo "$query<br><br>";}
//echo "<br />Line 338: whereSF=$whereSF<br />";
//echo "<br />showland=$showland<br />";
if($showland=='no'){$whereSF=$whereSF." and projcat != 'la'";}
...
echo "<br />Line 361: query=$query<br /><br />";
   $result = @mysqli_query($connection, $query);
if(@$showSQL=="1"){echo "$query<br><br>";}
//echo "<br />Line347: query=$query<br />"; 
...
if($c==''){$t=" bgcolor='cornsilk' ";$c=1;}else{$t=" bgcolor='lightcyan' ";$c='';}
...
echo "<tr>";
//echo "<td colspan='4' align='right'><b>$total_approved</b></td>";
echo "<td></td><td><td><b>Totals</b></td><td><b>$total_approved</b></td>";
