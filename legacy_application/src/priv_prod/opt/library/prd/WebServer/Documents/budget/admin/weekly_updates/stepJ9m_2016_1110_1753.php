...
CID_partf_fund_trans_Funds_Out_Amount,CID_partf_payments_amount)
SELECT new_fund_out as 'center','', sum( amount ) AS 'CID_partf_fund_trans_Funds_Out_Amount','' FROM `partf_fund_trans` WHERE 1 and new_fund_out != '' and datenew <= '$end_date' GROUP BY new_fund_out; 
";
...
SELECT proj_in as 'project', partf_projects.new_center, sum( amount ) AS 'fundsin','','' FROM partf_fund_trans LEFT JOIN partf_projects ON partf_fund_trans.proj_in = partf_projects.projnum WHERE partf_projects.reportdisplay = 'y' and datenew <= '$end_date' GROUP BY partf_projects.projnum ORDER BY partf_projects.new_center; 
";
...
SELECT proj_out AS 'project', partf_projects.new_center, '',sum( amount ) AS 'fundsout','' FROM partf_fund_trans LEFT JOIN partf_projects ON partf_fund_trans.proj_out = partf_projects.projnum WHERE partf_projects.reportdisplay = 'y' and datenew <= '$end_date' GROUP BY partf_projects.projnum ORDER BY partf_projects.new_center; 
";
