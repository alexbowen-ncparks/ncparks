...
if($sips_edit=='yes' and $update_record!='yes')
{
	
echo "sips_edit==yes"; //exit;	
$report_sips_edit_query="SELECT playstation as 'park',center,account,service,invoice_num,ncas_center,center_change,amount,id
          from sips_phone_bill4
		  where 1 and id='$Eid'  ";


//echo "<br />report_sips_edit_query=$report_sips_edit_query<br />"; //exit;
// The variable $result is a PHP resource containing the results of the MySQL query. Its contents can only be used by PHP.
$result_report_sips_edit_query = mysqli_query($connection, $report_sips_edit_query) or die ("Couldn't execute report_sips_edit_query.  $report_sips_edit_query");
//$num3=mysqli_num_rows($result3);
echo "<br /><br />";
echo "<form>";

echo "<table align='center' border='1' cellspacing='3'>";
echo "<tr><td>park</td><td>center</td><td>account</td><td>service</td><td>invoice_num</td><td>ncas_center</td><td>amount</td><td>id</td></tr>";
while ($row_report_sips_edit=mysqli_fetch_array($result_report_sips_edit_query))
	{	
	
extract($row_report_sips_edit);	
echo "<tr>";
echo "<td>$park</td><td>$center</td><td>$account</td><td>$service</td><td>$invoice_num</td><td>$ncas_center</td><td><input type='text' name='sips_edit_amount' value='$amount'</td><td>$id</td><td>";

echo "<input type='hidden' name='update_record' value='yes'>";
echo "<input type='hidden' name='sips_edit' value='yes'>";
...
echo "<input type='submit' name='submit' value='update_sips'>";
echo "</form>";
echo "</td>";
echo "</tr>";
}
...
if($sips_edit=='yes' and $update_record=='yes')
{
$query_update_record="update sips_phone_bill4
         set amount='$sips_edit_amount',ncas_invoice_amount='$sips_edit_amount',invoice_total='$sips_edit_amount'
		 where id='$Eid' ";
		 
echo "query_update_record=$query_update_record<br />";		 

$result_update_record = mysqli_query($connection, $query_update_record) or die ("Couldn't execute query_update_record.  $query_update_record");	
...
$query_center_change="update sips_phone_bill4  set ncas_center='$center_change',center_change='y'  where id='$center_change_id' ";
//echo "query_center_change=$query_center_change";
//echo "<br />"; //exit;
mysqli_query($connection, $query_center_change) or die ("Couldn't execute query center change. $query_center_change") ;


$query_center_change2="update sips_phone_bill4,center
                       set sips_phone_bill4.ncas_company=center.company,
                       sips_phone_bill4.ncas_budget_code=center.budcode
                       where sips_phone_bill4.id='$center_change_id'
                       and sips_phone_bill4.ncas_center=center.center  ";


mysqli_query($connection, $query_center_change2) or die ("Couldn't execute query center change2. $query_center_change2") ; 
					   
					   
$query_center_change3="update sips_phone_bill4
                       set ncas_fund='$fund_change',
                       ncas_rcc='$rcc_change'
                       where id='$center_change_id'	   ";


mysqli_query($connection, $query_center_change3) or die ("Couldn't execute query center change3. $query_center_change3") ; 	
...
echo "<br /><table align='center' border='1' align='left'><tr><th><font color='brown'>$project_name</font></th><th><a href='step_group.php?fyear=$fyear&report_type=reports'>Reports</a><br />$report_reports </th><th><a href='step_group.php?fyear=$fyear&report_type=form&reset=y&fyear=$fyear'>Form</a><br />$report_form</th></tr></table>";
...
$rank=@$rank+1;
$amount=number_format($amount,2);
//if($account=='532822'){$account='532819' ;}
if($account=='532819' and $center_change != 'y'){$t=" bgcolor='salmon' ";}else{$t=" bgcolor='lightgreen' ";}

echo "<tr$t>";
echo "<td>$rank</td>";
echo "<td><a href='step_group.php?project_category=$project_category&project_name=$project_name&step_group=$step_group&fiscal_year=$fiscal_year&start_date=$start_date&end_date=$end_date&report=y&report_type=form&sips_edit=yes&Eid=$id&account=$account&center=$center' target='_blank'>$account</a></td>";
...
//echo "<br />Line504: $report_listing<br />";
