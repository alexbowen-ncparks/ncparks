...
//echo "10<pre>"; print_r($_POST); echo "</pre>"; // exit;
...
// 			$sn=mysqli_real_escape_string($connection,$row['serial_number']);
			$desc=html_entity_decode(htmlspecialchars_decode($row['description']));
			$sn=html_entity_decode(htmlspecialchars_decode($row['serial_number']));
...
if(empty($_SESSION['fixed_assets']['accessPark']))
	...
	$ck_park_array[]=$_SESSION['fixed_assets']['select'];
	}
	else
	{
	$ck_park_array=explode(",",$_SESSION['fixed_assets']['accessPark']);
	}


//echo "91<pre>"; print_r($ck_park_array); echo "</pre>"; // exit;
$sql = "SELECT t1.id as bnc, t1.denr_code_from as code_from, t1.denr_code_to as code_to, t2.asset_number as an, t2.description as 'desc', t2.serial_number as sn, t1.from_date, t1.to_date
...
while($row=mysqli_fetch_assoc($result))
	...
	if($level==1 and in_array($test_from,$ck_park_array)){$bo_not_completed[]=$row;}
	if($level>1){$bo_not_completed[]=$row;}
	}
//echo "$sql<pre>"; print_r($bo_not_completed); echo "</pre>"; // exit;
if(!empty($bo_not_completed))
	{
	echo "<a onclick=\"toggleDisplay('awaiting');\" href=\"javascript:void('')\">
       Awaiting Action</a>
      <div id=\"awaiting\" style=\"display: none\">
	<table border='1'><tr><td>Action ID</td><td>From source to destination</td><td>Asset #</td>
	<td>Description</td><td>Serial #</td><td>Emailed Receiving Park</td><td>Receiving Park Emailed Business Office</td>
	</tr>";
	foreach($bo_not_completed as $k=>$array)
		{
		extract($array);    //$bnc 
		echo "<tr><td align='center'><a href='menu_fixed_assets.php?form_type=change_location&id=$bnc'>$bnc</a></td>
		<td>$code_from to $code_to</td><td>$an</td><td>$desc</td><td>$sn</td><td>$from_date</td><td>$to_date</td></tr>";
		}
	echo "</table>
         </div>";
	}
...
//		echo "$sql<br /><br />"; exit;
	...
	echo "<tr><td>Division:</td><td><input type='text' name='from_division' value='Div. of Parks & Recreation' size='33'></td></tr>";
		if(!empty($from_date))
			...
			{$from_date="";}
	echo "<tr><td>Date:</td><td><input type='text' id='datepicker1' name='from_date' value='$from_date' $ro></td></tr>";
	if(!empty($from_date))
		{
		echo "<tr><td>Signature:</td><td>$from_pasu_name <br /><img src='http://www.dpr.ncparks.gov/photos/$from_sig' height='42'></td></tr>";
		$link="http://www.dpr.ncparks.gov/fixed_assets/change_location.php?id=$id";
	...
	echo "<tr><td>Date:</td><td><input type='text' id='datepicker2' name='to_date' value='$to_date' $ro><br /><font color='red'>by submitting a date you are verifying recent of listed item(s).</font></td></tr>";
		if(!empty($to_date))
			{
		echo "<tr><td>Signature:</td><td>$to_pasu_name <br /><img src='http://www.dpr.ncparks.gov/photos/$to_sig' height='42'></td></tr>";
