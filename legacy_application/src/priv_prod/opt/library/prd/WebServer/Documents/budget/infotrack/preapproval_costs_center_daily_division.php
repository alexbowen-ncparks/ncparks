...
if($level != 1 and $center_code != ''){$where_park=" and park='$center_code'";}
if($level == 1){$where_park=" and park='$concession_location'";}
...
include ("preapproval_report_menu_division_v2.php");

include ("preapproval_widget2_division_v2.php");
...
$query5="select * from purchase_preapproval_tag
where f_year='$f_year'
and parent_record != 'y'
$where_park
...
$query5_yes="select count(id) as 'yes',sum(amount) as 'yes_amount' from purchase_preapproval_tag
where f_year='$f_year'
and parent_record != 'y'
...
$query5_no="select count(id) as 'no',sum(amount) as 'no_amount' from purchase_preapproval_tag
where f_year='$f_year'
and parent_record != 'y'
and record_complete !='y'
$where_park
...
$query6="select sum(amount) as 'amount_parkT' from purchase_preapproval_tag
where f_year='$f_year'
and parent_record != 'y' ;";
...
echo "<form method='post' action='preapproval_costs_center_daily_division.php?f_year=$f_year'>";
if($level==1)
{
...
echo "</tr></table>";
if($level != 1 and $center_code==''){exit;}
