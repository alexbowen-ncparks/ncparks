...
extract($_REQUEST);
if($reset=="Reset"){header("Location: /budget/c/contract_vital.php?dbTable=cid_contract_vitals");exit;}

if($submit!="Update" and $deleteRecord!="Delete" and $addRecord==""){include("../menu.php");}// necessary to allow header() to work

if($u==1){$message="<br>Update Successful.";}
...
if($dbTable==""){$dbTable="cid_contract_vitals";}

$showFields=array("DPR_contract_number","DPR_project_number", "NCAS_center_encumbered","NCAS_account_encumbered","DPR_contract_type", "DPR_contractor_name","DENR_contract_number","PO_number", "DPR_contract_administrator","DPR_comments");
...
header("Location: contract_vital.php?dbTable=$dbTable&u=2");
exit;
}
...
//echo "hello";exit;
header("Location: contract_vital.php?dbTable=$dbTable&DPR_contract_number=$DPR_contract_number&submit=Find&u=1");
exit;
}
...
$result = mysqli_query($connection, $query) or die ("Couldn't execute query Update. $query");
$v=mysql_insert_id();
header("Location: /budget/c/contract_vital.php?dbTable=$dbTable&id=$v&submit=Find");
exit;
}
...
$queryCN = "Select max(DPR_contract_number) + 1 as nextCN from cid_contract_vitals";
$resultCN = mysqli_query($connection, $queryCN) or die ("Couldn't execute query Update. $queryCN");
$rowCN=mysqli_fetch_array($resultCN);extract($rowCN);

// Used \" instead of ' because of lastname or vendor name containing '

echo "<form action=\"contract_vital.php\" method=\"post\">";
//echo "<form action=\"contract_vital.php\"";// Used to debug
...
if($fieldArray[$i]=="DPR_contract_number"){$ex="$nextCN = Next Contract Number";}else{$ex="";}
echo "<tr><td align='right'><b>$fieldArray[$i]</b>:  </td><td>"; pullDown($i); echo "<input type=\"text\" name=\"$fieldArray[$i]\" size=\"$fieldSize[$i]\" value=\"${$fieldArray[$i]}\"> $ex</td></tr>";}
}
...
{extract($row);
$linkCN="<a href='contract_vital.php?DPR_contract_number=$DPR_contract_number&submit=Find'>$DPR_contract_number</a>";
echo "<tr><td align='center'>$linkCN</td><td>$DPR_project_number</td><td>$NCAS_center_encumbered</td><td>$NCAS_account_encumbered</td><td>$DPR_contract_type</td><td>$DPR_contractor_name</td><td>$DENR_contract_number</td><td>$PO_number</td><td>$DPR_contract_administrator</td><td>$DPR_comments</td></tr>";
