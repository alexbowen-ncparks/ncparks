...
$file="year2year_comparison_lytt_drill.php";
...
header('Content-Disposition: attachment; filename=year2year_comparison_lytt_drill.xls');
echo "<html><body>";
...
// ****** Body Query
include("../~f_year.php");
$py1=substr($pf_year,0,2);
$py2=$py1+1;

$prev_start="20".$py1."0701";

$sql="select max(acctdate) as ad from exp_rev";
   $result = @mysqli_query($connection, $sql,$connection);
   $row=mysqli_fetch_array($result);
   $acctdate=$row['ad'];
   
$prev_end=(substr($acctdate,0,4)-1).substr($acctdate,4,4);


$testAccount=substr($account,0,2);
if($testAccount==43){
$query="SELECT  mid(acctdate,5,2) as 'month', mid(acctdate,1,4) as 'year' ,exp_rev.description as 'vendor',  -sum(debit-credit) as 'amount'  FROM exp_rev LEFT JOIN coa ON exp_rev.acct = coa.ncasnum where 1 and center='$center' and ncasnum='$account' and exp_rev.acctdate>='$prev_start' and acctdate <= '$prev_end' 
group by
mid(acctdate,1,4),
mid(acctdate,5,2),
exp_rev.description
