...
//  	echo "<pre>"; print_r($_POST); echo "</pre>";  //exit;
	if(isset($_POST['clemson']))
		...
		if(strpos($_POST['description'],"This item was digitized by Clemson University")>-1)
			...
			$_POST['digital_creator'].="Clemson University";
			}
		}
		...
		$dc_replace=str_replace("Clemson University","",$_POST['digital_creator']);
...
// echo "86<pre>"; print_r($_FILES); echo "</pre>"; // exit;
	if(!empty($_FILES['file']['tmp_name']))
		...
		$year=date("Y");
		$uploaddir = "archive_thumbs/".$year; // make sure www has r/w permissions on this folder
		if (!file_exists($uploaddir)) {mkdir ($uploaddir, 0777);}
		$month=date("m");
		$uploaddir = "archive_thumbs/".$year."/".$month; // make sure www has r/w permissions on this folder
		if (!file_exists($uploaddir)) {mkdir ($uploaddir, 0777);}
		$uploadfile = $uploaddir."/".$file_name;
		move_uploaded_file($temp_name,$uploadfile);// create file on server
		chmod($uploadfile,0777);
		$sql="REPLACE dcr_thumbnails (archive_id,link) "."VALUES ('$id','$uploadfile')";
...
//  echo "<pre>"; print_r($_SESSION); echo "</pre>"; // exit;
 if($_SESSION['photos']['level']<1)
 	{
 	echo "Your access may have timed out. Log back in to The ID.";
 	exit;
 	}
mysqli_select_db($connection, "dpr_system");
$sql="SELECT t1.* , t2.city
FROM parkcode_names_region as t1
left join dprunit_region as t2 on t1.park_code=t2.parkcode"; 
$result = mysqli_query($connection,$sql) or die ("Couldn't execute query 1. $sql");
while($row=mysqli_fetch_assoc($result))
	{
...
echo "<form action='archive_insert.php' method='POST' enctype='multipart/form-data'>";
echo "<table>";
if(!empty($ARRAY))
	{
...
// 		$ARRAY[0]['clemson']="";
		if($fld=="metadata_creator")
			{
			$value=$_SESSION['photos']['name'];
			if($value=="Williams, Veronica")
				{$value="Williams, Denise";}
			}
		if(in_array($fld,$skip)){continue;}
				if(in_array($fld,$restrict_array) and $tempID!="Cucurullo1234"){continue;}
		$rep_fld=$fld;
...
// 					if(empty($t))
// 						{$ck="";} else {$ck="checked";}
					echo "Clemson <input type='checkbox' name='clemson' value=\"x\">";
					}
echo "</th><td$td>";
		...
		if($fld=="subjects")
			...
			<select name='subjects[]' multiple size='$ms'>";
			  foreach($ARRAY_subjects as $k1=>$v1)
					...
					if(in_array($v1,$exp) or $v1=="Parks" or $v1=="North Carolina. Division of State Parks")
// 					if(in_array($v1,$exp))
						{
						$s="selected";
						}else{$s="";}
		...
		if(!in_array($fld,$default_array))
			...
			if($fld=="digital_creator"){$value="North Carolina Division of State Parks";}
		...
		if(!empty($ARRAY_thumbs['link']))
			{
			echo "<tr><td>thumbnail</td><td><img src='$ARRAY_thumbs[link]'>
			<br /><input type='file' name='file'></td></tr>";
			}
			else
			{
			echo "<tr><td>thumbnail</td><td><input type='file' name='file'></td></tr>";
			}
	}
echo "<tr>
