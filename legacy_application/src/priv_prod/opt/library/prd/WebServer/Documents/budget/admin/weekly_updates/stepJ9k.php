...
$queryA="update partf_payments set new_center=center where datenew >= '$start_date' ";
mysqli_query($connection, $queryA) or die ("Couldn't execute query A. $queryA");


$queryB="update partf_fund_trans set new_fund_in=fund_in where fund_in != '' and datenew >= '$start_date' ";
mysqli_query($connection, $queryB) or die ("Couldn't execute query B. $queryB");


$queryC="update partf_fund_trans set new_fund_out=fund_out where fund_out != '' and datenew >= '$start_date' ";
mysqli_query($connection, $queryC) or die ("Couldn't execute query C. $queryC");
...
SELECT new_fund_in as 'center', sum( amount ) AS 'CID_partf_fund_trans_Funds_In_Amount','','' frOM  `partf_fund_trans` WHERE 1 and new_fund_in != '' and datenew <= '$end_date' GROUP BY new_fund_in;
";
...
CID_partf_fund_trans_Funds_Out_Amount,CID_partf_payments_amount)
SELECT new_fund_out as 'center','', sum( amount ) AS 'CID_partf_fund_trans_Funds_Out_Amount',''
FROM `partf_fund_trans`
WHERE 1 and new_fund_out != ''
and datenew <=
'$end_date'
...
CID_partf_fund_trans_Funds_Out_Amount,CID_partf_payments_amount)
SELECT new_center,'','',sum( amount ) AS 'CID_partf_payments_Amount'
FROM partf_payments
WHERE 1 and datenew <= 
'$end_date'
GROUP BY new_center;
...
SELECT proj_in as 'project', partf_projects.new_center, sum( amount ) AS 'fundsin','',''
FROM partf_fund_trans
...
ORDER BY partf_projects.new_center;
";
...
SELECT proj_out AS 'project', partf_projects.new_center, '',sum( amount ) AS 'fundsout',''
FROM partf_fund_trans
...
SELECT charg_proj_num AS 'project', partf_projects.new_center, '','',sum( amount ) AS 'payments'
FROM partf_payments
