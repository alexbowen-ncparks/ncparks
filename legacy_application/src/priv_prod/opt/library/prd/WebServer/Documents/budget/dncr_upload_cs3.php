...
if(empty($submit))
	...
	<body>";
$file_array=array("exp_rev_dncr_osc2_ncx.csv","cab_dpr_temp_ncx.csv","bd725_dpr_temp_pre_ncx.csv");
	
	echo "<form method=post action=dncr_upload_cs3.php enctype='multipart/form-data'>";
...
$query2i="update bd725_dpr_temp_pre_ncx 
...
$query2ia="update bd725_dpr_temp_pre_ncx set total_budget=replace(total_budget,',',''),unallotted=replace(unallotted,',',''),total_allotments=replace(total_allotments,',',''),current=replace(current,',',''),ytd=replace(ytd,',',''),ptd=replace(ptd,',',''),allotment_balance=replace(allotment_balance,',','')
           where 1 ";
...
$query2e="update bd725_dpr_temp_pre_ncx set account=REPLACE(account, '=', '') ";
		  
		  
mysqli_query($connection, $query2e) or die ("Couldn't execute query 2e.  $query2e");


$query2f="update bd725_dpr_temp_pre_ncx set account=REPLACE(account, '\"', '') ";
		  
		  
mysqli_query($connection, $query2f) or die ("Couldn't execute query 2f.  $query2f");


$query2g="update bd725_dpr_temp_pre_ncx set account_descript=REPLACE(account_descript, '=', '') ";
		  
		  
mysqli_query($connection, $query2g) or die ("Couldn't execute query 2g.  $query2g");


$query2h="update bd725_dpr_temp_pre_ncx set account_descript=REPLACE(account_descript, '\"', '') ";
...
///
$query2j="update bd725_dpr_temp_pre_ncx set amount1_neg='yes' where right(total_budget,1)='-' and right(total_budget,2) != '--' ";

		  
mysqli_query($connection, $query2j) or die ("Couldn't execute query 2j.  $query2j ");


$query3j="update bd725_dpr_temp_pre_ncx set amount1_neg='no' where amount1_neg != 'yes' ";

		  
mysqli_query($connection, $query3j) or die ("Couldn't execute query 3j.  $query3j ");


$query4j="update bd725_dpr_temp_pre_ncx set amount1_correct=concat('-',(LEFT(total_budget,LENGTH(total_budget)-1))) where amount1_neg='yes' ";
		  
		  
mysqli_query($connection, $query4j) or die ("Couldn't execute query 4j.  $query4j");


$query5j="update bd725_dpr_temp_pre_ncx set total_budget=amount1_correct where amount1_neg='yes' ";
...
$query2k="update bd725_dpr_temp_pre_ncx set amount2_neg='yes' where right(unallotted,1)='-' and right(unallotted,2) != '--' ";

		  
mysqli_query($connection, $query2k) or die ("Couldn't execute query 2k.  $query2k ");


$query3k="update bd725_dpr_temp_pre_ncx set amount2_neg='no' where amount2_neg != 'yes' ";

		  
mysqli_query($connection, $query3k) or die ("Couldn't execute query 3k.  $query3k ");

$query4k="update bd725_dpr_temp_pre_ncx set amount2_correct=concat('-',(LEFT(unallotted,LENGTH(unallotted)-1))) where amount2_neg='yes' ";
		  
		  
mysqli_query($connection, $query4k) or die ("Couldn't execute query 4k.  $query4k");

$query5k="update bd725_dpr_temp_pre_ncx set unallotted=amount2_correct where amount2_neg='yes' ";
...
$query2L="update bd725_dpr_temp_pre_ncx set amount3_neg='yes' where right(total_allotments,1)='-' and right(total_allotments,2) != '--' ";

		  
mysqli_query($connection, $query2L) or die ("Couldn't execute query 2L.  $query2L ");


$query3L=" update bd725_dpr_temp_pre_ncx set amount3_neg='no' where amount3_neg != 'yes'";

		  
mysqli_query($connection, $query3L) or die ("Couldn't execute query 3L.  $query3L ");

$query4L="update bd725_dpr_temp_pre_ncx set amount3_correct=concat('-',(LEFT(total_allotments,LENGTH(total_allotments)-1))) where amount3_neg='yes' ";
		  
		  
mysqli_query($connection, $query4L) or die ("Couldn't execute query 4L.  $query4L");


$query5L="update bd725_dpr_temp_pre_ncx set total_allotments=amount3_correct where amount3_neg='yes' ";
...
$query2m="update bd725_dpr_temp_pre_ncx set amount4_neg='yes' where right(current,1)='-' and right(current,2) != '--' ";

		  
mysqli_query($connection, $query2m) or die ("Couldn't execute query 2m.  $query2m ");


$query3m=" update bd725_dpr_temp_pre_ncx set amount4_neg='no' where amount4_neg != 'yes'";

		  
mysqli_query($connection, $query3m) or die ("Couldn't execute query 3m.  $query3m ");


$query4m="update bd725_dpr_temp_pre_ncx set amount4_correct=concat('-',(LEFT(current,LENGTH(current)-1))) where amount4_neg='yes' ";
		  
		  
mysqli_query($connection, $query4m) or die ("Couldn't execute query 4m.  $query4m");


$query5m="update bd725_dpr_temp_pre_ncx set current=amount4_correct where amount4_neg='yes' ";
...
$query2n="update bd725_dpr_temp_pre_ncx set amount5_neg='yes' where right(ytd,1)='-' and right(ytd,2) != '--' ";

		  
mysqli_query($connection, $query2n) or die ("Couldn't execute query 2n.  $query2n ");


$query3n=" update bd725_dpr_temp_pre_ncx set amount5_neg='no' where amount5_neg != 'yes'";

		  
mysqli_query($connection, $query3n) or die ("Couldn't execute query 3n.  $query3n ");


$query4n="update bd725_dpr_temp_pre_ncx set amount5_correct=concat('-',(LEFT(ytd,LENGTH(ytd)-1))) where amount5_neg='yes' ";
		  
		  
mysqli_query($connection, $query4n) or die ("Couldn't execute query 4n.  $query4n");


$query5n="update bd725_dpr_temp_pre_ncx set ytd=amount5_correct where amount5_neg='yes' ";
...
$query2p="update bd725_dpr_temp_pre_ncx set amount6_neg='yes' where right(ptd,1)='-' and right(ptd,2) != '--' ";

		  
mysqli_query($connection, $query2p) or die ("Couldn't execute query 2p.  $query2p ");


$query3p=" update bd725_dpr_temp_pre_ncx set amount6_neg='no' where amount6_neg != 'yes'";

		  
mysqli_query($connection, $query3p) or die ("Couldn't execute query 3p.  $query3p ");


$query4p="update bd725_dpr_temp_pre_ncx set amount6_correct=concat('-',(LEFT(ptd,LENGTH(ptd)-1))) where amount6_neg='yes' ";
		  
		  
mysqli_query($connection, $query4p) or die ("Couldn't execute query 4p.  $query4p");


$query5p="update bd725_dpr_temp_pre_ncx set ptd=amount6_correct where amount6_neg='yes' ";
...
$query2r="update bd725_dpr_temp_pre_ncx set amount7_neg='yes' where right(allotment_balance,1)='-' and right(allotment_balance,2) != '--' and allotment_balance != '-' ";

		  
mysqli_query($connection, $query2r) or die ("Couldn't execute query 2r.  $query2r ");


$query3r=" update bd725_dpr_temp_pre_ncx set amount7_neg='no' where amount7_neg != 'yes'";

		  
mysqli_query($connection, $query3r) or die ("Couldn't execute query 3r.  $query3r ");


$query4r="update bd725_dpr_temp_pre_ncx set amount7_correct=concat('-',(LEFT(allotment_balance,LENGTH(allotment_balance)-1))) where amount7_neg='yes' ";
		  
		  
mysqli_query($connection, $query4r) or die ("Couldn't execute query 4r.  $query4r");


$query5r="update bd725_dpr_temp_pre_ncx set allotment_balance=amount7_correct where amount7_neg='yes' ";
...
$query30="update budget.project_steps_detail set status='complete' 
		  ...
		  and step_group='C' and step_num='1h1' ";
