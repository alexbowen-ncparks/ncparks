...
// if(!empty($rep))
// 	{
// 	$filename="warehouse_order_placed";
...
if(empty($rep))
	...
	if(!empty($park_order_list))
		...
		echo "<th colspan='2'><form>View Order Placed for Center: <select name='park_code' onchange=\"this.form.submit()\">
...
// 	echo "<th colspan='4'>Excel <a href='order_placed.php?$var'>export<a/></th>";
	echo "<th>Excel <a href='order_placed.php?$var_csv'>CSV<a/></th>";
		}
...
//	echo "<td><input type='checkbox' name='$name' value=\"x\" $ck></td>";
	foreach($array as $fld=>$value)
		...
		if($fld=="id")
			...
			if(empty($rep)){$value=$temp;}else{continue;}
			}
		if($fld=="product_number")
			{
			$pass_id=$array['id'];
			$temp="<a href='edit_park_purchase.php?id=$pass_id'>$value</a>";
...
if($level<3)
	...
	WHERE 1 order by t1.product_number
	"; 
$result = mysqli_query($connection,$sql) or die ("Couldn't execute query 1. $sql");
while($row=mysqli_fetch_assoc($result))
	{
	@$source_product_number.="\"".$row['product_number']."\",";
	}
	if(empty($rep))
		{
		echo "<tr><td colspan='13'>If needed, a credit can be assigned to this order. Enter the amount to be credited.
		<input type='text' name='credit' value=\"\" size='5'> for PRODUCT NUMBER
		<input type='text' id='credit_product_number' name='credit_product_number' value=\"\" size='10'>
		<input type='hidden' name='pass_id' value=\"$pass_id\"> <input type='submit' name='credit_submit' value=\"Enter Credit\">
		</td>";
		echo "
		<script>
		$(function()
			{
			$( \"#credit_product_number\" ).autocomplete({
			source: [ $source_product_number ]
				});
			});
		</script>";
		echo "<tr><td colspan='13'>
		<textarea name='comments' cols='80' rows='2'></textarea> Enter comments to appear on printed Invoice.
		<input type='hidden' name='complete' value=\"x\"></td>
		</tr>";
