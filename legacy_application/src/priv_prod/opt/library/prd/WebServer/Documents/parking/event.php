...
// echo "<pre>"; print_r($_POST); echo "</pre>";  exit;
	...
	mysqli_select_db($connection,$database); // database 
		...
		if(!EMPTY($_POST['space']))
			{
			$sql="SELECT  all_day from `event_dates`
...
// 	echo "check record $sql<br />$blank_name<br />";   exit;
			...
			$sql="UPDATE event_dates 
			SET all_day=''
			where date_id = '$_POST[pass_edit]' and space='$space' $blank_name
			";  
			$result = @mysqli_query($connection,$sql) or die(mysqli_error($connection));
			
			if(@$_POST['submit_form']=="Release Event Trailer")
				{
				$sql="DELETE FROM event_dates 
				where date_id = '$_POST[pass_edit]' and space='$space'
				";  
				$result = @mysqli_query($connection,$sql) or die(mysqli_error($connection));
// 				echo "The van will now be available for that date.";
header("Location: event.php?cal_type=event");
				exit;
				}
			}
// 		if(!empty($exist))
...
//echo "add update2 $sql<br /><br />"; //exit;
			...
			if($num<1)
			{
			$sql="INSERT ignore INTO event_dates 
...
$sql="SELECT * from event_dates where date_id like '$check_month%'"; //echo "$sql";
$result = @mysqli_query($connection,$sql) or die(mysqli_error($connection));
while($row=mysqli_fetch_assoc($result))
	{
	$ARRAY[]=$row;
...
include("cal_array.php");  // lists the various items being tracked
	...
	foreach($cal_array as $k=>$v)
		{
		if($cal_type=="event" and $k=="event"){$s="selected";}else{$s="";}
...
<td align="center"><font color='green' size='+2'>DPR's Event Trailer</font>
...
for ($i=0; $i<($maxday+$startday); $i++)
	{
		...
		$display="<a href='cal.php?cal_type=event&pass_edit=$edit'>$day</a>";
...
<?php
echo "<table align='center'><tr><td style='color:green; font-weight: bold; font: 1em \"Open Sans\", sans-serif'>Please check with the park that previously reserved the Event trailer or contact the DPR Warehouse to help locate (919-303-7719).<br /><br />(We save fuel and mileage by moving the Event trailer between parks, rather than returning it to one home-base after each reservation.)<br /><br />
Event Trailer Information Packet: <a href='/find/graphics/2016/EventTrailer_Packet_2016.pdf'>download</a></td></tr></table>";
