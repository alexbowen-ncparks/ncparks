...
//$text="Project";pdf_show_xy ($pdf, $text, $col1, $y+$fontSize);
//$text="Number";pdf_show_xy ($pdf, $text, $col1, $y);
...
$fullname=strtolower($arraypark[$projNum]);
$dist=strtolower($arraydist[$projNum]);
$county=strtolower($arraycounty[$projNum]);
if($county=="na" or $county=="NA")
	{$county=strtoupper($county);}
	else
	{$county=ucwords($county);}

$projName=strtolower($arrayprojName[$projNum]);
...
//$text=$projNum;pdf_show_xy ($pdf, $text, $col1+3, $y);

$text=ucwords($fullname);pdf_show_xy ($pdf, $text, $col2-4, $y);
$dc=ucwords($dist.", ").$county;
$text=$dc;pdf_show_xy ($pdf, $text, $col3, $y);

$text=ucwords($projName);pdf_show_xy ($pdf, $text, $col4, $y);
