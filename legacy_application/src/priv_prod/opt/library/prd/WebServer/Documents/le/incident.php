...
function setForm() {
    opener.document.pr63_form.incident_code.value = document.inputForm1.inputField0.value;
    opener.document.pr63_form.incident_name.value = document.inputForm1.inputField1.value;
    self.close();
    return false;
}
...
echo "<tr><td colspan='3'>Incident Codes: <select name='incident_code' onChange=\"MM_jumpMenu('parent',this,0)\"><option value=''></option>";         
		...
		echo "<option $s='incident.php?incident_code=$k'>$k - $v\n";
		}
echo "</select></td></tr>";
		...
		foreach($code_array as $k=>$v)
			...
			if(strpos($k,'XX')>0)
				...
				foreach($legend_array as $k_1=>$v_1)
					...
					echo "<tr>
					<td><a href='incident.php?incident_code=$incident_code&pass_code=$make_code&pass_desc=$v&pass_service=$v_1'>$make_code</a></td><td>$v</td><td>$k_1 $v_1</td>
					</tr>";
					}
				continue;
				}
			...
			if(strpos($k,'2201')>-1)
				...
				echo "<tr>
				<td></td><td>$v</td><td><a href='incident.php?incident_code=$incident_code&pass_code=$k&pass_desc=$v&pass_service_1=$pass_service'>$k</a></td>
					</tr>";
				continue;
				}
			...
			if(strpos($k,'2202')>-1)
				...
				echo "<tr>
				<td></td><td>$v</td><td><a href='incident.php?incident_code=$incident_code&pass_code=$k&pass_desc=$v&pass_service_2=$pass_service'>$k</a></td>
					</tr>";
				continue;
				}
			...
			if(strpos($k,'2306')>-1)
				...
				echo "<tr>
				<td></td><td>$v</td><td><a href='incident.php?incident_code=$incident_code&pass_code=$k&pass_desc=$v&pass_agency_6=$pass_service'>$k</a></td>
					</tr>";
				continue;
				}
			...
			if(strpos($k,'2307')>-1)
				...
				echo "<tr>
				<td></td><td>$v</td><td><a href='incident.php?incident_code=$incident_code&pass_code=$k&pass_desc=$v&pass_agency_7=$pass_service'>$k</a></td>
					</tr>";
				continue;
				}
			...
			echo "<tr><td><a href='incident.php?incident_code=$incident_code&pass_code=$k&pass_desc=$pass_desc'>$k</a></td><td>$v</td></tr>";
			}
	}
