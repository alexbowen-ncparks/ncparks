...
WHERE $where";
//echo "$sql<BR />"; //exit;
$result = mysql_query($sql) or die ("Couldn't execute query SELECT. $sql ".mysql_error());
...
if(empty($SIG_ARRAY))
	{
	echo "No signature was found. 4<br />"; 
	...
	if ($tempID)
		...
		<form method='post' action='/photos/addSig.php' enctype='multipart/form-data' target='_blank'>";
