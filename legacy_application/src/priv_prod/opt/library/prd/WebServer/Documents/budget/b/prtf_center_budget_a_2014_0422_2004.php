...
'approved',sum(pm_allocation) as 'pm_allocation', sum(expenses) as 'expenses', sum(approved-expenses) as 'balancePB', statusper as 'status',statusper_fi_date,manager, cid_project_balances.projnum
...
for($i=0;$i<count($project_center_year_typeArray);$i++)
	...
	if(!@$statusA || in_array($testStatus,$statusA))
		...
		<td align='right'>$b</td>
		<td align='center'>$status_link<br />";
