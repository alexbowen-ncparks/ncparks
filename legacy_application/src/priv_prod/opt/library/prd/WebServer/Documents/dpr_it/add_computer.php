...
// 	echo "<pre>"; print_r($district); echo "</pre>"; // exit;
	...
	$skip=array("submit_form", "select_table", "alt_make", "alt_model", "alt_os", "alt_location");
...
$search_computers_dropdown=array("division","district_section","location","type","os","make","model","computer_status");
...
$alt_value_array=array("make","model","os","location");
echo "<form method='POST' action='add_computer.php'>";
echo "<table><tr><td colspan='2'><h3><font color='#8cd9b3'>Add to the DPR/MNS Inventory of $select_table</h3></td></tr>";
	...
	if(in_array($fld, $search_array))  // search_array created in form_arrays.php
		...
		foreach($drop_down_array as $k=>$v)
			...
			if($fld=="computer_status")
				...
				if(!in_array("deploy",$ARRAY_computer_status))
					{
					// ignored after being used once
					$line.="<option value='deploy'>Deployed</option>\n";
					$ARRAY_computer_status[]="deploy";
					}
				}
		...
		if($fld=="location")
			{
			if(!in_array("NEC",$ARRAY_location))
				{
				// used to add a new location
				// will be ignored once used
				$line.="<option value='NEC'>NEC</option>\n";
				}
