...
//	echo "<pre>"; print_r($_POST); print_r($_FILES); echo "</pre>";  exit;
	if($submit=="Delete")
		{
		$sql="SELECT photo_link FROM family_upload_photo where family_id_photo='$edit'"; //echo "$sql";exit;
		...
		$sql="DELETE FROM family_upload_photo where family_id_photo='$edit'"; //echo "$sql";exit;
		$result = mysqli_query($connection,$sql) or die ("Couldn't execute query Update. $sql ".mysqli_error($connection));
		
		$sql="SELECT file_link FROM family_upload_file where family_id_file='$edit'"; //echo "$sql";exit;
		$result = mysqli_query($connection,$sql) or die ("Couldn't execute query Update. $sql ".mysqli_error($connection));
		if(mysqli_num_rows($result)>0)
			{
			while($row=mysqli_fetch_assoc($result))
		...
		$sql="DELETE FROM family_upload_file where family_id_file='$edit'"; //echo "$sql";exit;
		$result = mysqli_query($connection,$sql) or die ("Couldn't execute query Update. $sql ".mysqli_error($connection));
		
		$sql="DELETE FROM family where id='$edit'"; //echo "$sql";exit;
		$result = mysqli_query($connection,$sql) or die ("Couldn't execute query Update. $sql ".mysqli_error($connection));
...
echo "<tr><th colspan='2'>Post a Note to the State Park Family</th></tr>";
...
if(!empty($_GET["edit"]))
	...
	FROM  family as t1 
	LEFT JOIN family_upload_file as t2 on t1.id=t2.family_id_file
...
//	$value="";
	...
	if($v=="tempID")
		{
		$value=$_SESSION['war']['tempID'];
		$val=substr($_SESSION['war']['tempID'],0,-4);
