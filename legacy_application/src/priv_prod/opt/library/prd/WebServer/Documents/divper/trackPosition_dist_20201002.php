...
if($position!="Park Superintendent" and $level<2 and !in_array($_REQUEST['beacon_num'],$check_supervise))
	{exit;}
...
//	echo "<pre>"; print_r($_POST); echo "</pre>"; // exit;
	...
	header("Location: trackPosition.php?beacon_num=$beacon_num&m=1");
	exit;
	}
...
if($submit=="Enter"){
foreach($_POST as $k=>$v){
if($k!="submit"){$string.="$k='".htmlspecialchars_decode($v)."', ";}
}
$string=trim($string,", ");
...
header("Location: trackPosition.php?posNum=$posNum&m=1");
exit;
}
*/
...
</table><form method='post' action='trackPosition.php'>
$success
<table border='1'><tr><th>BEACON<br>Number</th><th>Park</th><th>District</th><th>CDL Required</th><th>Hiring Manager<br>Email Address</th><th>Classification</th></tr>
<tr>
...
$var=trim($date_bdgt_approv);
if(empty($var)){$f1="";$f2="";}else{$f1="<font color='green'>";$f2="</font>";}
echo "<p>8 -$f1 Budget$f2</p>";
$var=trim($appFromHR);
if(empty($var)){$f1="";$f2="";}else{$f1="<font color='green'>";$f2="</font>";}
echo "<p>9 -$f1 From DNCR HR$f2</p>";
...
if(!isset($date_bdgt_approv)){$date_bdgt_approv="";}
...
<td align='center'>
<input type='text' size='10' name='date_bdgt_approv' value='$date_bdgt_approv'></td>
<td align='center'><input type='text' size='10' name='appFromHR' value='$appFromHR'></td>";

$ckr=""; $ckp="";
$status_array=array("Pending background", "Screening", "Filled", "Recruiting", "Lateral", "Pending Post", "Reclassification", "Resubmission 2nd", "Resubmission 3rd", "Resubmission 4th", "Resubmission 5th");
