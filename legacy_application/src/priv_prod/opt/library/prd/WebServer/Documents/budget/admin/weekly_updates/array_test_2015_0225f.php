...
$textareaFields=array("Fund_Descript");
...
echo "<tr><td colspan='8' align='center'><font color='red'>$num records<br />fund_descript=EAST_MM,NORTH_MM,SOUTH_MM,WEST_MM,STWD_DEMOLITION,STWD_MM_RESERVE,STWD_Exhibit_Maintenance<br />manager=dwayne_parker,pete_mitchell,etc <br />fullname=STONE MOUNTAIN,LAKE NORMAN,STATEWIDE etc.<br />dist=EAST,NORTH,etc.<br />county=moore,nash,etc.<br />section=construction,operations,natural_resources<br />park=CABE,CACR,etc.<br />proj_man=JH(jerry),EL(erin),etc. </font></td></tr>";
echo "<tr>";
...
foreach($ARRAY as $k=>$v)
	...
	echo "<tr$tr class=\"normal\" id=\"row$j\" onclick=\"onRow(this.id)\">";
		foreach($v as $k1=>$v1){// each field, e.g., $tempID=$v[tempID];
			...
			//echo "<td$td><input type='text' value='$v1'></input></td>";}
			  ...
			  {echo "<td$td>$v1<br /><input type='text' value='$v1'></input></td>";}
