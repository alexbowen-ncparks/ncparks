...
include("use_drop_downs.php");  // line 13 of use_drop_downs.php uses $dropdown_file
// echo "<pre>"; print_r($use_dropdowns); echo "</pre>"; // exit;

include("form_arrays.php");   // master file that sets many array attributes

$search_drop_down_flds=array("park_abbreviation");
...
// 	echo "<br />229 $sql<pre>"; print_r($t); echo "</pre>";
$search_radio_flds=array("unit_type");
foreach($search_radio_flds as $k=>$v)
	{
	${$v."_array"}=array();
	$t=array();
	if(in_array($v, $ARRAY_fields))
...
// echo "$sql<pre>"; print_r($unit_type_array); echo "</pre>"; // exit;
$magenta_array=array();
...
// 		echo "<pre>"; print_r($select_array); echo "</pre>"; // exit;
	...
	if(in_array($fld,$radio_yn_array)) // also edit Search section, add_form, and edit_form.php
		{
		$radio_array=$radio_array_form[$fld];  // set in form_arrays.php
		if($fld=="unit_type")
			{
// 			echo "<pre>"; print_r($radio_array); echo "</pre>";
			}
		$line="<td>";
		foreach($radio_array as $k=>$v)
			{
			if($fld=="lease_type" and $v==NULL){continue;}
			$line.="<input type='radio' name='$fld' value=\"$k\">$v";
			}
		$line.="</td>";
		}
...
$required_array=array("park_abbreviation","unit_type","termlength_years","lease_start_date","expires","countyname", "lease_type");
...
// 		$var_id="datepicker1";
	...
	if(in_array($fld,$drop_down))    // $drop_down set in use_drop_downs.php
		...
		if($fld=="park_abbreviation")
			{
			// Need to allow adding ANY unit - not just ones already entered.
			$park_abbreviation_array=array();
			...
			WHERE 1 and t1.park_abbreviation !='' 
			order by t1.park_abbreviation"; 
			//  ECHO "<br />$sql"; //exit;
			$result = mysqli_query($connection,$sql) or die("$sql Error 1#". mysqli_errno($connection) . ": " . mysqli_error($connection));
			while($row=mysqli_fetch_assoc($result))
				{
// 				$park_abbreviation_array[$row['park_id']]=$row;
				$park_abbreviation_array[$row['park_abbreviation']]=$row['park_name'];
				}

				asort($park_abbreviation_array);
			}
// echo "434<pre>"; print_r($park_abbreviation_array); echo "</pre>";
		$temp_fld=$fld;

		$temp_array=$temp_fld."_array";
		$select_array=${$temp_array};  // arrays created in values_?_.php or hard-wired for $park_abbreviation_array
		if(in_array($fld, $drop_down_flip))
			{
			$select_array=array_flip($select_array);
			}
		...
		foreach($select_array as $k=>$v)
			...
			if($fld=="park_abbreviation")
				{
				$line.="<option value='$k' $s>$v - $k</option>";
				}
				else
				{
				$line.="<option value='$v' $s>$v - $k</option>";
	...
	if(in_array($fld,$radio_yn_array))  // also edit Search section, add_form, and edit_form.php a
		{
		$radio_array=$radio_array_form[$fld];  // set in form_arrays.php
		$line="<td>";
// 		echo "<pre>"; print_r($radio_array); echo "</pre>";
		foreach($radio_array as $k=>$v)
			{
			if($fld=="lease_type" and $v==NULL){continue;}
			$line.="<input type='radio' name='$fld' value=\"$k\" $req>$v";
			}
		$line.="</td>";
		}
