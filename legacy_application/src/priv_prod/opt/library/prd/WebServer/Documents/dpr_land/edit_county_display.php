...
$clause_flds=array("county_code", "park_code", "project_status", "CountyName");
$t1_flds=array("county_code"=>"county_id", "project_status"=>"project_status_id");
$t2_flds=array("CountyName"=>"CountyName");
$t3_flds=array("park_code"=>"park_abbreviation","park_code"=>"park_abbreviation");
foreach($clause_flds as $k=>$v)
	{
	if(array_key_exists($v,$_POST))
		...
		if(array_key_exists($v,$t1_flds))
			{
			$t="t1.";
			$v=$t1_flds[$v];
			}
		if(array_key_exists($v,$t2_flds))
			{
			$t="t2.";
			$v=$t2_flds[$v];
			}
		if(array_key_exists($v,$t3_flds))
			{
			$t="t3.";
			$v=$t3_flds[$v];
			}
...
if(empty($ARRAY))
	{ECHO "No asset found for county_code=$county_code."; exit;}
$c=count($ARRAY);

$pass_sort_fld=array("park_code","CountyName");
foreach($ARRAY AS $index=>$array)
	...
	if($index==0)
		...
		foreach($ARRAY[0] AS $fld=>$value)
			...
			if(empty($project_status)){$project_status="";}
			...
			foreach($pass_sort_fld as $k=>$v)
				{
				if(array_key_exists($v,$_POST))
					{
					$val=$_POST[$v];
					$desc.="<input type='hidden' name='$v' value='$val'>";
					}
				}
			$desc.="<input type='hidden' name='sort_by' value='desc'>
			<input type='hidden' name='sort_fld' value='$fld'>
			<input type='hidden' name='county_code' value='$county_code'>
			<input type='hidden' name='project_status' value='$project_status'>
...
echo "<table class='alternate' border='1' cellpadding='3'><tr><td class='head' colspan='$span'>Comparing MySQL table 'a_land_assets' values with those from 'b_SPO_LandAsset_ID' <br />for county code: <strong>$county_code</strong> and a_acreage = b_deedacres [ <strong>$c records</strong> ] ";
echo "</td></tr>";
...
// 			if($level>4){$action_form="edit_form_test.php";}
		...
		if($fld=="notes_2")
			{
			$value="<a onclick=\"toggleDisplay('$index');\" href=\"javascript:void('')\">Show/Hide</a>
<div id=\"$index\" style=\"display: none\">$value</div>";
			}
