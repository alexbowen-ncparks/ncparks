...
$sql="SELECT t1.*, t2.product_title, t1.quantity, t2.current_cost, format(t1.quantity*t1.price,2) as cost
...
order by item_group, sub_group_1, sub_group_2, product_number
";
$pass_sql=$sql;
// echo "$sql"; exit;
$result = mysqli_query($connection, $sql) or die ("Couldn't execute query. $sql ".mysqli_error($connection));
...
// echo "$sql<br /><br />"; //exit;
	...
	if($num_id<1)
		...
		$insert_comments=mysqli_real_escape_string($connection,$invoice_comments);
...
$array_x=array("processed_date"=>"15","invoice_number"=>"83","center"=>"126", "account"=>"175", "sold_by_unit"=>"215", "price"=>"248", "cost"=>"285", "product_number"=>"320", "quantity"=>"355", "product_title"=>"390");
$title_array=array("processed_date"=>"          date","invoice_number"=>"invoice","center"=>"center","product_number"=>"WARE#", "sold_by_unit"=>"unit", "quantity"=>"quantity", "price"=>"price","cost"=>"cost","account"=>"account","product_title"=>"product_title");
$i=0;
$var_page=0;
$max_lines=30;
...
//echo "161<pre>"; print_r($ARRAY); echo "</pre>"; // exit;
...
//	if($index>=22){break;}
	...
	if($i==0)
		...
		$insert_comments=mysqli_real_escape_string($connection, $var_comments);
...
$body="Body=The warehouse has processed the order submitted on $park_ordered_on. You can download the invoice at this link: http://www.dpr.ncparks.gov/ware/$link.";
