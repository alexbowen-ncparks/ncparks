...
if(!empty($oldpassword)){$oldpassword=urldecode($oldpassword);}
session_start();
...
// 		echo "$sql";exit;
		...
		if($oldpassword!=$password){ echo "64 The Username and/or Password you entered is/are not correct! Make sure of your spelling. If the problem persists, send an email to the contact person listed below.<br>";
...
// 			echo "$query"; exit;
			...
			header("Location: https://auth1.dpr.ncparks.gov/dpr_login.php?submit=Change Password&npassword0=$npassword0&npassword1=$npassword1&source=auth&emid=$femid&dbName=$dbName&ftempID=$ftempID");
			exit;
			}
		if(@$var=="nondpr")
			{
// 			echo "<pre>"; print_r($_REQUEST); echo "</pre>"; // exit;
			if(!empty($fpassword)){$npassword0=$fpassword;}
