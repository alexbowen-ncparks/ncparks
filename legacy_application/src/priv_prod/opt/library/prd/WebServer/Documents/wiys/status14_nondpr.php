...
if(@$u==1)
	...
	//echo "$sql";
	$result = @mysql_query($sql, $connection) or die("Error 1#". mysql_errno() );
...
if($sel=="arch"||$sel=="york")
	...
	$resultNonDPR = @mysql_query($sql, $connection) or die("$sql Error 1#". mysql_errno() );
	while($row=MYSQL_FETCH_ARRAY($resultNonDPR))
		{
		extract($row);
...
if($sel || $tempID)
	...
	$dbExplain="";
	mysql_select_db('divper',$connection);
	$sql = "select emplist.currPark,empinfo.Nname,empinfo.Fname,empinfo.Lname, empinfo.tempID,empinfo.emid,empinfo.dbmonth,position.posNum, position.beacon_num, nondpr.Lname as non_Lname
	from empinfo
	LEFT JOIN emplist on emplist.tempID=empinfo.tempID
	LEFT JOIN nondpr on emplist.currpark=nondpr.currPark
	$join2
	$select $order";
		echo "$sql"; //exit;
	$result = @mysql_query($sql, $connection) or die("$sql Error 1#". mysql_errno() );
...
if(@$arrayTempID)
	{
	mysql_select_db('wiys',$connection);
	$today=date('d');
	$checkMonth=date('n');
