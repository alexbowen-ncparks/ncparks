...
// 	$line="<tr><td>$fld</td><td><input type='text' name=\"$fld\" value=\"$value\" size='84'></td></tr>";
	if ($pr) { $line=''; } else { $line="<tr><td>$fld</td><td>$value</td></tr>"; }
	}
if($fld=="Name of Site")
	{
...
// 	$line="<tr><td>$fld</td><td><input type='text' name=\"$fld\" value=\"$value\" size='84'></td></tr>";
	if ($pr) {
	$line="<tr><td><b>Contact:</b> $value</td></tr>";
	} else {
...
// // 	$line="<tr><td>$fld</td><td><input type='text' name=\"$fld\" value=\"$value\" size='84'></td></tr>";
// 	if ($pr) {
// 	$line="<tr><td></td><td><b>$value</b></td></tr>";
// 	} else {
...
// 	$line="<tr><td>$fld</td><td><input type='text' name=\"$fld\" value=\"$value\" size='84'></td></tr>";
	if ($pr) {
	$line="<tr><td><b>Physical Address:</b> $value</b></td></tr>";
	} else {
...
// 	$line="<tr><td>Office Phone</td><td><input type='text' name=\"$fld\" value=\"$value\" size='84'></td></tr>";
	if ($pr) {
	$line="<tr><td><b>Office: </b>$value</td></tr>";
	} else {
...
// 	https://www.ncparks.gov/carolina-beach-state-park/home
	...
	if ($pr) {
	$line="<tr><td><b>URL: </b><a href='$value' target='_blank'>$value</a></td></tr>";
	} else {
...
// 	{
// 	if ($pr) {
// 	$line="<tr><td colspan='2'>Legislative Districts/Names of Representatives & Senators</td></tr><tr><td></td><td>$value</td></tr>";
// 	} else {
...
if($fld=="Acquisition Date by State")
	...
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
	if ($pr) {
	$line="<tr><td colspan='2'><b>Acquisition Date:</b> $value </td></tr>";
	} else {
	...
	if(!empty($park_acres[$park_code]))
		...
		foreach($park_acres[$park_code] as $index=>$array)
			...
			if(empty($sub_classification))
				...
				{
				$temp[$sub_classification]=number_format($system_length_miles,1)." miles";
				}
...
// 	$line.="$value => <b>".number_format($value,0)."</b> acres";
	...
	$line="<tr><td><b>Acres: </b>";

	foreach($temp as $k_a=>$v_a)
		{
		if($k_a=="system_area_acres_land")
			{$v_a=number_format($v_a, 0);}
// 		
		$line.="$k_a = $v_a<br />";
		}
// 	$line.="$value => <b>".number_format($value,0)."</b> acres";
	...
	$line.="<tr><td><b>Acres Comments and any Satellite Areas:</b> $var_ac</td></tr>";
...
// 		echo"v=$value";
	...
	if ($pr) {
	$line="<tr><td colspan='2'><b>Structures: </b>$total_fac $value</td></tr>";
	} else {
...
if($fld=="Description of Site")
	...
	if ($pr) {
	$line="<tr><td><b>Description of Site</b> $value</td></tr>";
	} else {
...
// 
// 	if ($pr) {
// 	$line="<tr><td colspan='2'>$fld</td></tr><tr><td></td><td>$value</td></tr>";
// 	} else {
...
// 	$line="<tr><td>$fld</td><td><textarea name=\"$fld\" cols='123' rows='1'>$value</textarea></td></tr>";
	...
	if ($pr) {
	$line="<tr><td><b>Site Activities:</b> $value</b></td></tr>";
	} else {
...
if($fld=="Educational Services")
	...
	if ($pr) {
	$line="<tr><td><b>Educational Services:</b> $value</td></tr>";
	} else {
...
if($fld=="Outreach Efforts")
	...
	if ($pr) {
	$line="<tr><td><b>Outreach Efforts:</b> $value</td></tr>";
	} else {
...
if($fld=="Bond Project")
	...
	if ($pr) {	
	$line="<tr><td><b>Bond Project:</b> $value</td></tr>\n";
	} else {
...
// 
// 	if ($pr) {
// 	$line="<tr><td></td><td width='1024'><b>Park History:</b> ".nl2br($value)."</td></tr>";
// 	} else {
...
// 
// 	if ($pr) {
// 	$line="<tr><td>$fld"."(s)</td><td>$value</td></tr>";
// 	} else {
...
// 	echo "<pre>"; print_r($park_year_attend['FALA']); echo "</pre>"; // exit;
	if(!empty($park_year_attend[$park_code]))
		...
		if ($pr) {
		$line="<tr><td><b>FY19-20 Visitation:</b>";
		...
		$line="<tr><td><b>$fld</b> (Fiscal Year)</td>";
// 		foreach($park_year_attend[$park_code] as $ky=>$kv)
// 			{
...
if($fld=="Admission Fees")
	...
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
	if ($pr) {
	$line="<tr><td><b>Admission Fees</b> $value</td></tr>\n";
	} else {
...
// 		{$value=$ARRAY[$fld];}
// 	if ($pr) {
// 	$line="<tr><td colspan='3'>$fld</td></tr><tr><td></td><td>$value</td></tr>";
// 	} else {
...
if($fld=="Other Fees")
	...
	if ($pr) {
	$line="<tr><td><b>Other Fees:</b> $value</td></tr>\n";
	} else {
...
if($fld=="Hours of Operations")
	...
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
	if ($pr) {
	$line="<tr><td><b>Hours of Operation: </b></td></tr><tr><td>".nl2br($value)."</td></tr>\n";
	} else {
	...
	if(!empty($temp_personnel))
		...
		foreach($temp_personnel as $p_k=>$array)
			...
			if($p_k==6009)
				...
				if(!empty($Appropriated))
					{
					$tot_temp_app+=$Appropriated;
					$val_p="Temporary Appropriated=".$Appropriated;
					$tot_per+=$Appropriated;
					}
				if(!empty($Receipts))
					{
					$tot_temp_rec+=$Receipts;
...
// 	$line="<tr><td>$fld</td></tr><tr><td></td><td><textarea name=\"$fld\" cols='73' rows='4'>$value</textarea></td></tr>";
	...
	if ($pr) {
	$line="<tr><td colspan='3'><b>Positions as of November 2019 </b><br>Total=$tot_per</td></tr><tr>
	<td>Permanent Appropriated = $tot_perm_app <br />
	Permanent Receipted = $tot_perm_rec <br />
	Temporary Appropriated = $tot_temp_app <br />
	Temporary Receipted =$tot_temp_rec</td></tr>";
	} else {
	$line="<tr><td colspan='2'><b><font color='green'>*$fld</font></b> (db=divper.BO149) Total=$tot_per<br />
...
// 	$line="<tr><td colspan='5'><font color='green'>*$fld</font> (query in dpr_system/fiscal_year_budget.php)</td></tr><tr><td></td><td><textarea name=\"$fld\" cols='53' rows='3' $RO>$value</textarea></td></tr>";
	...
	if ($pr) {
	$line="<tr><td colspan='2'><b>Fiscal Year 2019-20</b></td></tr>
	<tr><td>
	<table>
...
if($fld=="Volunteer Hours")
	...
	if(!empty($temp_vol_hours))
		{$value=number_format($temp_vol_hours[$park_code],0);}
	if($level<3){$RO="readonly";}else{$RO="";}
...
// 	$num_hours=number_format($value,0);
	...
	$line="<tr><td>
	<table><tr></td>
	<b>Public / Private Partnership Percentages FY 2019-20</b></td></tr>
	<tr><td>Valuation = $" . number_format($value_dollars, 2) . "<br />$24.72 per hr. for NC from 
	<a href='https://independentsector.org/resource/vovt_details/'>https://independentsector.org/resource/vovt_details/</a>
	</td></tr></table></td></tr>";
	} else {
	$line="<tr><td colspan='2'><b><font color='green'>*$fld</font> for $fy</b> (db=park_use.vol_stats)</td></tr><tr><td></td><td>$num_hours Hours for a Valuation=$" . number_format($value_dollars, 2) . "<br />$24.72 per hr. for NC from <a href='https://independentsector.org/resource/vovt_details/'>https://independentsector.org/resource/vovt_details/</a></td></tr>";
	}
...
// 	$value=$park_contact[$park_code]['office_hours'];
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
	if ($pr) {
	$line="<tr><td><b>Future Needs: </b>$value</td></tr>\n";
	} else {
...
// 	$value=$park_contact[$park_code]['office_hours'];
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
	if ($pr) {
	$line="<tr><td><b>Private Support:</b> $value</td></tr>\n";
	} else {
	$line="<tr><td><font color='green'>*$fld</font></td></tr><tr><td></td><td><textarea name=\"$fld\" cols='123' rows='4'>$value</textarea></td></tr>";
	}
	}
...
if($fld=="Restrictions or Limitations on the use of Site")
	...
	if ($pr) {
	$line="<tr><td><b>Restrictions or Limitations on the use of Site:</b> $value</td></tr>\n";
	} else {
...
if($fld=="Friends Group")
	...
	if ($pr) {
	$line="<tr><td><b>Friends Group:</b> $value</td></tr></table>\n";
	} else {
