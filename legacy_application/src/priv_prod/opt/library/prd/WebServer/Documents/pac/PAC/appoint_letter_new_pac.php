...
// Letter
$sql="SELECT letter_body as letter_body_text from pac_letter_new_pac";
...
LEFT JOIN emplist as t2 on t1.tempID=t2.tempID
LEFT JOIN position as t3 on t2.beacon_num=t3.beacon_num
where t2.currPark='$temp_parkcode' and t3.posTitle='Park Superintendent'
order by o_chart"; //echo "$sql";exit;
$result = mysql_query($sql) or die ("c=$connection Couldn't execute query. $sql");
...
$result = mysql_query($sql) or die ("c=$connection Couldn't execute query. $sql"); //echo "$sql";exit;
$row=mysql_fetch_assoc($result); extract($row);
...
if($personInfo['pac_reappoint_date']){$appointType="reappoint";}else{$appointType="appoint";}
$text=str_replace("*appointType*",$appointType,$letter_body_text);
...
LEFT JOIN emplist as t2 on t1.tempID=t2.tempID
LEFT JOIN position as t3 on t2.beacon_num=t3.beacon_num
where  t3.posTitle='Parks & Rec Division Dir'";
$result = mysql_query($sql) or die ("c=$connection Couldn't execute query. $sql"); 
$row=mysql_fetch_assoc($result); extract($row);
...
pdf_show_xy($pdf, $chop, 50, $new_y-62);
