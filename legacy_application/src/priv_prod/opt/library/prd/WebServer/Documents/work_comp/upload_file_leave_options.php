...
if ($_FILES['files']['error'][0] == 0)
	...
	$sql="REPLACE form_leave_options_upload (wc_id,file_name) "."VALUES ('$wc_id','$file_name')";
...
//	echo "$sql <br />";exit;
	...
	// remove any existing file for form19
			$sql = "SELECT file_link from form_leave_options_upload  where wc_id='$wc_id'";
	...
	$year=date("Y");
	$ts=time();
	$uploaddir = "upload_form_leave_options/".$year; // make sure www has r/w permissions on this folder

	//    echo "$uploaddir"; exit;
	if (!file_exists($uploaddir)) {mkdir ($uploaddir, 0777);}

	$new_name="upload_form_leave_options_".$wc_id."_".$ts;
	...
	$path_to_file="/opt/library/prd/WebServer/Documents/work_comp/".$uploadfile;
		...
		if(file_exists($path_to_file))
			{
			$sql = "UPDATE form_leave_options_upload set file_link='$uploadfile' where id='$id'";
			$result = @mysqli_query($connection, $sql) or die("$sql Error 1#". mysqli_errno($connection) . ": " . mysqli_error($connection));
			}
			else
