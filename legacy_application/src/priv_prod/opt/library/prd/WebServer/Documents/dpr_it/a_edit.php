...
$skip=array("id","submit_form", "select_table", "computers", "printers", "subnets", "switches", "previous_comments", "comments_computers", "comments_printers", "comments_subnets", "comments_switches","ports","file_upload");
...
$skip=array("select_table","submit_form", "computers", "printers", "subnets", "switches", "previous_comments", "comments_computers", "comments_printers", "comments_subnets","comments_switches","alt_os","ports"); $equal_array=array("id","district_section","region_section","location","type","os", "make", "model", "fas", "site_id", "printer", "site_name", "current_service_provider", "vlan", "computer_status", "printer_status"); // in edit.php and search_form.php
$temp=array();
FOREACH($_POST as $fld=>$value)
	{
	if(in_array($fld,$skip)){continue;}
...
if(mysqli_num_rows($result)<1)
	...
	// echo "<pre>"; print_r($computer_history_array); echo "</pre>"; // exit;
		$skip=array("id","division","district_section","link");
		...
		$search_subnets_dropdown=array("region_section", "site_id","gateway", "site_name", "current_service_provider", "location", "type", "vlan");
$search_switches_dropdown=array("region_section","host_name", "location", "make", "model", "site_id", "os");
...
// also in computer_history.php
	$ARRAY_computer_status=array("bwo"=>"Being Worked On","rec_p"=>"Received at Park","sent_r"=>"Sent to Raleigh","rec_r"=>"Received in Raleigh","sent_p"=>"Sent to Park","surp_tobe"=>"To be Surplused","surp_process"=>"Surplus Process","surplus"=>"Surplused");
	$ARRAY_printer_status=array("bwo"=>"Being Worked On","rec_p"=>"Received at Park","sent_r"=>"Sent to Raleigh","rec_r"=>"Received in Raleigh","sent_p"=>"Sent to Park","surp_tobe"=>"To be Surplused","surp_process"=>"Surplus Process","surplus"=>"Surplused");
	if($level<3)
		{
		$ARRAY_computer_status=array("rec_p"=>"Received at Park","sent_r"=>"Sent to Raleigh","surp_tobe"=>"To be Surplused");
		$ARRAY_printer_status=array("rec_p"=>"Received at Park","sent_r"=>"Sent to Raleigh","surp_tobe"=>"To be Surplused");
		}
		...
		foreach($array as $fld=>$value)
			...
			if(in_array($fld, $search_array) and !in_array($fld, $readonly_array))
				...
				if($fld=="location")
					...
					$line.="<option value='SURPLUS' $s>SURPLUS</option>\n";
					}
				$line.="</select>";
