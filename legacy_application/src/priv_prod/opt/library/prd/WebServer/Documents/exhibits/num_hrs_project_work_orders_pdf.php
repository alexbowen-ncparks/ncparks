...
$maxD=max($ARRAY);
//echo "<pre>";  print_r($spread);print_r($array2);echo "</pre>"; // exit;

/*
foreach($spread as $k=>$v)
	{
	$data[]=array("edition"=>$k,"volume"=>$v);
	}
...
$text="Number of work orders by hours reported.";
	pdf_show_xy ($pdf, $text,100,50);
...
foreach($spread as $num_hours=>$num)
	{
//	$a=$array['volume'];
//	$b=$array['edition'];
$a=$num_hours;
...
//	$xx=$x_legend[$row['section']]-110;
	$xx=$xx+$barW+11;
		pdf_show_xy ($pdf, number_format($numA,2),$xx, $a+95);
		pdf_show_xy ($pdf, $b,$xx+1, $a+85);

$link="http://nature123.net/work_order/r_num_hours_work_order.php?hours=$num_hours&submit=Enter";
			$starting_xpos = $xx-2;
			$starting_ypos = $a+83;
			pdf_add_weblink($pdf, $starting_xpos, $starting_ypos, $starting_xpos + 16, $starting_ypos + 11, $link);
