...
// 				}
		...
		//		$val=$_POST['budget_hrs_b']*$_POST['avg_rate'];
			...
			$clause=rtrim($clause,",");
			$sql = "Update seasonal_payroll_next set $clause
			WHERE  beacon_posnum='$beacon_posnum'"; //echo "$sql"; exit;
			$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql ");
			
			$sql = "Update employ_position set position_title='$position_title', pay_rate='$pay_rate'
			WHERE  beacon_num='$beacon_posnum'"; //echo "$sql"; exit;
...
$skip=array("submit");
		...
		if(in_array($fld,$skip)){continue;}
			...
			$clause=rtrim($clause,",");
			$sql = "REPLACE seasonal_payroll_next set $clause"; 
	//		echo "$sql"; exit;
			$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql ");
...
// echo "<pre>"; print_r($title_array); echo "</pre>";	
...
// WHERE  t1.beacon_posnum='$beacon_posnum'"; //echo "$sql"; //exit;
		...
		if($k=="datebegin"){$datebegin=$v;}
			...
			if($k=="osbm_title")
				...
				foreach($title_array as $k1=>$v1){
					if($val==$v1){$s="selected";}else{$s="";}
					$v.="<option value='$v1' $s>$v1</option>";
					}
