...
foreach($unitArray as $k=>$v){
	if($unit_type==$v){$s="selected";}else{$s="value";}
		echo "<option $s='summary_print.php?unit_type=$v'>$v</option>";
       }
	if($unit_type=="all"){$s="selected";}else{$s="value";}
		echo "<option $s='summary_print.php?unit_type=all'>All</option>";
   echo "</select></td></tr></table></form>";


	
if(empty($unit_type)){exit;}
...
while($row=mysqli_fetch_assoc($result)){$printThese[]=$row['parkcode'];}
...
{
if($unit_type=="State Natural Area"){$unit_type="Natur";}
	foreach($parkCodeName as $k=>$v){
		if($k AND !in_array($k,$skip)){
...
if($level>4){$file="print_pdf.php";}else{$file="print_pdf.php";}
