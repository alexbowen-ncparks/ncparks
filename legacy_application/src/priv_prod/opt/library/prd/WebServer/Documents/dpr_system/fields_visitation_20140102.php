...
if($fld=="Full-time (2008-09)")
	...
	$var=@$year_park_staff['2009'][$park_code]['FT N-FLSAOT Perm']+@$year_park_staff['2009'][$park_code]['FT S-FLSAOT Perm'];
	$value=$var;
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
...
$fld="Full-time (2009-10)";
	$var=@$year_park_staff['2010'][$park_code]['FT N-FLSAOT Perm']+@$year_park_staff['2010'][$park_code]['FT S-FLSAOT Perm'];
	$value=$var;
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
...
$fld="Full-time (2010-11)";
	$var=@$year_park_staff['2011'][$park_code]['FT N-FLSAOT Perm']+@$year_park_staff['2011'][$park_code]['FT S-FLSAOT Perm'];
	$value=$var;
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
...
$fld="Full-time (2011-12)";
	$var=@$year_park_staff['2012'][$park_code]['FT N-FLSAOT Perm']+@$year_park_staff['2012'][$park_code]['FT S-FLSAOT Perm'];
	$value=$var;
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
...
$fld="Full-time (2012-13)";
	$var=@$year_park_staff['2013'][$park_code]['FT N-FLSAOT Perm']+@$year_park_staff['2013'][$park_code]['FT S-FLSAOT Perm'];
	$value=$var;
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
...
if($fld=="Future Capital Needs")
	{
	$var_new_fac=@$park_nsl[$park_code]['new_fac'];
	$var_renov=@$park_nsl[$park_code]['renov'];
	$var_land=@$park_nsl[$park_code]['land'];
	$var_ann_main=@$park_nsl[$park_code]['ann_main'];
	$var="Future Needs from Park Master Plan: New Facilities $var_new_fac; Renovations $var_renov; Land Acquisition $var_land";
	$mod=str_replace("$","",$var_ann_main);
	$mod=str_replace(",","",$mod);
	$vam="$".number_format(($mod * .03),0);
	$var.="\n\nRoutine Annual Maintenance: $vam (3% of DOI structure replacement value of $var_ann_main; does not include roads, trails, utilities, and unroofed structures)";
	$value=$var;
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
		
	$line.="<table><tr><td>Future Needs:</td><td><textarea name=\"$fld\" cols='123' rows='3'>$value</textarea></td></tr>";
...
$fld="Private Support";
	...
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
	$line.="<tr><td>Private Support</td><td><textarea name=\"$fld\" cols='123' rows='3'>$value</textarea></td></tr>";
...
$fld="Restrictions or Limitations on the use of Site";
	...
	if(!empty($ARRAY[$fld]))
		{$value=$ARRAY[$fld];}
	$line.="<tr><td>Restrictions or Limitations on the use of Site</td><td><textarea name=\"$fld\" cols='123' rows='3'>$value</textarea></td></tr>";
	$line.="</table>";
	}
?>