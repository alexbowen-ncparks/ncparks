...
// 	echo "<pre>"; print_r($_POST); echo "</pre>";  exit;
	if(!$seidA=="")
		...
		for($i=0;$i<count($seidA);$i++)
			...
			`park`='$reasonA[$i]', `park_reg`='$reasonA[$i]',`posType`='$posTypeA[$i]',`section`='$sectionA[$i]',`fund`='$fundA[$i]',`fund_source`='$fund_sourceA[$i]' WHERE seid='$seidA[$i]'";
...
if($val > -1)
	...
	header("Location: form.php?park=$park&message=$message");
	exit;
	} // end Add
...
//	$testPark=$_SESSION['parkS'];
	...
	$sql = "SELECT * From position WHERE park_reg='$park' AND markDel = '' ORDER by current_salary desc";
// 	echo "$sql"; exit;
	$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. ");
