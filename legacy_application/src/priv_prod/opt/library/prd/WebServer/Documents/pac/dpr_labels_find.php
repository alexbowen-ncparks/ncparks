...
echo "<html><head><title>PAC</title>";
if($rep=="")
	{
...
//	if($passQuery){$arraySet=urldecode($passQuery);}
	...
	if(in_array("PAC",$add_cat) OR in_array("PAC_FORMER",$add_cat) OR in_array("PAC_nomin",$add_cat) OR in_array("dist",$fieldArray))
		{
		$orderBy="order by dist_approve desc,park,t1.affiliation_code,Last_name";
		}
		else{
...
$sql="SELECT person_id as id,affiliation_code as code from labels_affiliation";
	$result = mysqli_query($connection,$sql) or die ("Couldn't execute query line 108. $sql");
...
// Use GROUP_CONCAT instead
	...
	if($_SESSION['pac']['level']>4 AND $rep=="")
		{
//		echo "$sql<br /><br />";
		//exit;
		}
	
	$result = mysqli_query($connection,$sql) or die ("Couldn't execute query line 135. $sql");
...
// ****************************************	
	if($num==1)
		...
		$result = mysqli_query($connection,$sql) or die ("Couldn't execute query line 159. $sql");
		$num=mysqli_num_rows($result);
...
//			echo "<pre>"; print_r($passCode); echo "</pre>"; // exit;
		echo "<form action='dpr_labels_update.php' method='POST'><table border='1' cellpadding='5'>";
			...
			if(in_array("PAC",$passCode))
				{
				$letterCode="appoint";	if($pac_reappoint_date){$appointType="Reappointment";}else{$appointType="Appointment";}
				echo "<tr><td>Produce PAC <a href='PAC/appoint_letter.php?id=$id&parkcode=$park'>$appointType</a> Letter<br />
				<font size='-2'>Doesn't work in IE. Use any other browser.</font></td><td align='right'></td>
				<td>PAC <a href='PAC/agenda_letter.php?parkcode=$park'>Agenda</a> Letter</td><td align='right'>";
				}
			
			if(in_array("PAC_nomin",$passCode))
				{	if($pac_reappoint_date){$appointType="Reappointment";}else{$appointType="Appointment";}
		...
		echo "<td>
		<form action='dpr_labels_find.php'>
		...
		if($test=="park" || $test=="pass")
			...
			$result = mysqli_query($connection,$sql) or die ("Couldn't execute query line 263. $sql");
			$num=mysqli_num_rows($result);
...
echo "<tr><td><form action='home.php?submit_label=Home+Page'>
		<input type='submit' name='submit' value='Home'  style=\"background-color:#E9967A\"></form></td>";

include("dpr_labels_form.php");

echo "<tr>
<td align='right'><input type='submit' name='submit_label' value='Find' style=\"background-color:lightgreen;width:65;height:35\"></td></form>

<td align='right'><form action='home.php'>
<input type='submit' name='submit' value='PAC Home Page'></form></td>

<td align='right'><form action='dpr_labels_add.php'>
<input type='submit' name='submit_label' value='Go to Add'></form></td>";
