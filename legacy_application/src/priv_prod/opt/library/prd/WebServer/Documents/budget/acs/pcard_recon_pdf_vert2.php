...
$header_array=array("item_purchased","vendor_name","company","ncasnum","center","amount");
...
// 				pdf_show_boxed ($pdf, $text, 150,255,400,20,"left","");
				...
				$x=40; $y=230;$text="Justification:";
					pdf_show_xy ($pdf, $text, $x ,$y-70);
					$date=$array[$num]['report_date'];
					
					$text="Report date: Week ending $date";
					pdf_show_xy ($pdf, $text, $x+65 ,$y-70);
						
					$text="Reconciler: $cashier_name   $cashier_date";					
					pdf_show_xy ($pdf, $text, $x+65 ,$y-80);
				
					$text="Approver: $manager_name   $manager_date";					
					pdf_show_xy ($pdf, $text, $x+350 ,$y-80);
				
					$format_image="jpeg";
					//$load_image="Dodd3454.jpg";
					$load_image=$cashier_sig_location;
					$img_size=getimagesize($load_image);
			        $height=$img_size[1];
			

				 
				 $scale="scale $cashier_scale";
				 $image = PDF_load_image($pdf,$format_image,$load_image,"");
					//pdf_show_xy ($pdf, $text, $x+65 ,$y-20);
		            PDF_fit_image($pdf,$image,$x+60,$y-130,$scale);
					...
					$format_image2="jpeg";
					//$load_image="Dodd3454.jpg";
					$load_image2=$manager_sig_location;
					$img_size2=getimagesize($load_image2);
			        $height=$img_size2[1];
					/*
				  ...
				  */
				  ...
				  //$scale="scale .25";
				    ...
				    $scale="scale $manager_scale";
		            $image2 = PDF_load_image($pdf,$format_image2,$load_image2,"");
					//pdf_show_xy ($pdf, $text, $x+65 ,$y-20);
		            PDF_fit_image($pdf,$image2,$x+350,$y-130,$scale);
		            pdf_close_image($pdf, $image2);
	               ...
	               $fontSize=8;
	                pdf_setfont ($pdf, $times, $fontSize);
					pdf_show_xy ($pdf, $text, 105 ,$y-137);		
					
				pdf_rect($pdf, 100, 100, 450, 45); // rect.
 					 pdf_stroke($pdf);
	...
	pdf_setfont ($pdf, $times, $fontSize);					
			$text="All purchases made with the State Purchasing Card are Sales Tax Exempt. If additional lines are needed, use another Pcard Reconciling Form to continue listing purchases.  Complete and submit this form along with the receipts within 5 - 7 business days from the date of purchase.";
				pdf_show_boxed ($pdf, $text, 100 ,60,450,30,"left","");
...
// 				pdf_show_boxed ($pdf, $text, 100 ,95,450,20,"left","");
	...
	pdf_show_boxed ($pdf, $text, 100 ,15,450,40,"left","");		
...
// $body_array=array("item_purchased"=>"30","company"=>"195","ncasnum"=>"280","center"=>"356","vendor_name"=>"435","amount"=>"510");
$body_array=array("item_purchased"=>"30","vendor_name"=>"195","company"=>"380","ncasnum"=>"416","center"=>"475","amount"=>"525");
...
//  	echo "<pre>"; print_r($value_array); echo "</pre>";  exit;
...
// 					$text=substr($text,0,110);
			...
			// if($value_array[$key]=="2010 Dodge Dakota-PJ5621-FAS#187604-Serpentine Belt Replacement Diagnostic/Repairs-PA#44136")  // used for testing
// 				{
// 				echo "$text<pre>"; print_r($value_array); echo "</pre>$item_lines";  exit;
// 				}				
				
		// Draw vert line
		if($key=="amount" and $item_lines>1)
				{$var_height=0;$var_bottom=25;}
				else
				{
				$var_height=10;
				if(@$cutRow==0){$var_height=9;}
				$var_bottom=3;
				}
...
$filename=$parkcode."_report_".$report_type."_".$xtnd_start.".pdf";
