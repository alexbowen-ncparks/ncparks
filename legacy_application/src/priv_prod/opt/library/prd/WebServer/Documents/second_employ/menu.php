...
extract($_REQUEST);
@$level=$_SESSION['second_employ']['level'];
@$tempID=$_SESSION['second_employ']['tempID'];

if($level<3)
	{
	@$pass_park=$_SESSION['second_employ']['select'];
	}

if($level<1){echo "You do not have access to this database.<br /><br />If you were previously logged in but get this message, it may be because your session timed out. Try logging into Secondary Employment again."; exit;}
...
<?php
if(!isset($tab)){$tab="";}
echo "<title>Secondary Employment Authorization</title>";
@include("/opt/library/prd/WebServer/Documents/css/TDnull.php");
	
//	echo "<pre>"; print_r($_SERVER); echo "</pre>"; // exit;
echo "<body><div align='center'>";

if($_SERVER['PHP_SELF']=="/second_employ/menu.php")
	{
	echo "<img src=\"/inc/css/images/dpr_1.jpg\"><br /><font color='brown'>Welcome to the</font><br />";
	}

$color_array=array("Track a Request"=>"lightgreen","Secondary Employment Policy"=>"yellow","Instructions"=>"#C6E2FF","Submit a Request"=>"#FFE4C4","Ramps"=>"#DDA0DD","Swim Lines"=>"#FFB6C1");
...
//$menu_array['Secondary Employment Policy']="http://portal.ncdenr.org/group/ooc/";

$menu_array['Instructions']="/find/forum.php";

if($level>2)
	{$menu_array['Summary Report']=$path."summary_report.php";}


echo "<table><tr><td align='center' colspan='10'><font color='brown'>NC DPR Secondary Employment Website</font></td></tr><tr>";
foreach($menu_array as $k=>$v)
	{
		$color=@$color_array[$k];
		if($k=="Secondary Employment Policy"||$k=="Instructions")
			{$target=" target='_blank'";}else{$target="";}
		echo "<td><form action='$v' $target>";
		if($k=="Instructions")
			{
			echo "<input type='hidden' name='searchterm' value='secondary employment'>";
			echo "<input type='submit' name='submit' value='Instructions'";
			echo " style=\"background-color:$color\"></form>";
			}
		else
			{
			echo "<input type='hidden' name='database' value='second_employ'>";
			echo "<input type='submit' name='submit' value='$k'";
			echo " style=\"background-color:$color\"></form>";
			}
		echo "</td>";
	}
