...
$sql="SELECT add1,add2,city, zip,ophone,email from dprunit where parkcode='$parkcode'";
$result = mysql_query($sql) or die ("Couldn't execute query. $sql ".mysql_error());
...
$result1 = mysql_query($sql) or die ("c=$connection Couldn't execute query. $sql ".mysql_error());
...
PDF_fit_image($pdf, $im, 72, 657, "scale 0.65");
PDF_close_image($pdf, $im);

// ************* DPR Logo
$im = PDF_load_image($pdf, "auto", "DPR logo.tiff", "");
PDF_fit_image($pdf, $im, 464, 30, "scale .10");
PDF_close_image($pdf, $im);
*/
...
LEFT JOIN labels_affiliation as t2 on t2.person_id=labels.id
where labels.id='$id'";
$result1 = mysql_query($sql) or die ("Couldn't execute query. $sql ".mysql_error());
...
$pi="Dear ".$personInfo['suffix']." ".$personInfo['Last_name'].":";
pdf_show_xy($pdf, $pi, 50, $piY-60);

//echo "$agenda_body"; exit;
// *********** Body of Letter ****************************
...
PDF_show_boxed ($pdf,$agenda_body,50,50,480,500,"left","");
