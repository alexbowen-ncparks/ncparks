...
//echo "<br />filepath=$file_path";//exit;	
       ...
       // print_r(fgetcsv($handle));
        ...
        mysqli_query($connection, "TRUNCATE TABLE `crs_tdrr_cc_import` ") or die(mysqli_error()); 
        ...
        while (($data = fgetcsv($handle, 10000, ',')) !== FALSE) {  
            ...
            $payment_type = $data[5]; 
            $product_id = $data[6]; 
			$product_id=mysqli_real_escape_string($product_id);
            $product_name = $data[7]; 
...
$sql="INSERT INTO crs_tdrr_cc_import 
 set `transaction_date`='$transaction_date', `revenue_location_id`='$revenue_location_id', 
 `revenue_location_name`='$revenue_location_name',`transaction_location_id`='$transaction_location_id',`transaction_location_name`='$transaction_location_name',`payment_type`='$payment_type',`product_id`='$product_id',`product_name`='$product_name',`amount`='$amount',`account_id`='$account_id',`account_name`='$account_name',`batch_deposit_date`='$batch_deposit_date',`batch_id`='$batch_id',`deposit_id`='$deposit_id',`revenue_note`='$revenue_note' ";  
 
            mysqli_query($connection, $sql) or (mysqli_query($connection, "ROLLBACK") and die(mysqli_error() . " - $sql"));  
        }  
      
	  
$query0="update crs_tdrr_cc_import
         set revenue_type=mid(account_id,1,4),company_type='1601',revenue_code=mid(account_id,11,9)
         where 1 ";		 
...
$query1="truncate table crs_tdrr_cc";

$result1 = mysqli_query($connection, $query1) or die ("Couldn't execute query 1.  $query1");

$query2="insert into crs_tdrr_cc(
         transaction_date,
		 revenue_location_id,
...
$query17="truncate table crs_tdrr_cc_adj;";
...
//echo "<tableUploaded Deposit Total equals: $upload_total";  exit;



header("location: bank_deposits_menu_cc_test.php?step=1a");
