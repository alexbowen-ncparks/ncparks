...
$query3a="select max(system_entry_date) as 'last_update', max(cvip_id) as 'last_cdcs' from energy1 where ncas_account='532230' ";
...
echo "<form method='post' action='water_cdcs_import.php?f_year=$f_year&egroup=$egroup&report=$report&valid_account=$valid_account&last_cdcs=$last_cdcs'>";
...
echo "<br />";
echo "<table><tr><th>Recordss: $num3</th>";
...
echo "</tr>";
echo  "<form method='post' autocomplete='off' action='water_cdcs_update.php'>";
...
//These individual variables are the names of the fields queried from MySQL
extract($row3);
if($ncas_account=='532230'){$egroup='water';}
...
$rate=round(($ncas_invoice_amount/($energy_quantity/1000)),2);
...
echo  "<td>$ncas_invoice_number<br />$datesql2 ($invoice_month$date_sql_year)<br />$system_entry_date<br />$account_number<br /><a href='/budget/energy/energy_reporting.php' onclick=\"return popitup('/budget/energy/energy_reporting.php?f_year=$f_year&center_code=$center_code&egroup=$egroup&report=accounts')\">Valid Accounts</a></td>";
