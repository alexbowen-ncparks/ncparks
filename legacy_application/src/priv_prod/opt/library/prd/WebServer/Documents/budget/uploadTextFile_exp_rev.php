...
if($m<7){$beginFiscalYear=$y0."0701";$fy=$y0sub.$y1sub;}
else
{$beginFiscalYear=$y1."0701";$fy=$y1sub.$y2sub;}

session_start();
$_SESSION[budget][fy]=$fy;
$sql =  "DELETE FROM  `exp_rev` where acctdate >= '$beginFiscalYear'";
...
INSERT INTO exp_rev (invoice,pe,description,debit,credit,sys,acct,center,fund,acctdate,f_year)
...
// we need to re-reset
if($into_table=="exp_rev"){$into_table="exp_rev_fyear";}
...
if($into_table=="cip_fund_bal"){echo "<br><br>Click <a href='clean_cip_fund_balance.php'>link</a> to cleanup the imported data.
<BR><BR>
...
if($into_table=="partf_fund_trans"){echo "<br><br>Click <a href='clean_partf_fund_trans.php'>link</a> to cleanup the imported data.
<BR><BR>
1. --.<br>
2. --.<br>
3. --.";
...
echo "<br><br>Imported records have been parsed and cleaned:
<BR><BR>
1. Update records by finding appropriate ACCT number and adding it to each record.<br>
2. Delete ALL records where INVOICE is blank.<br>
3. Delete ALL records where DEBIT AND CREDIT are zero.<br>
4. Update records by parsing out center, fund and date from fld1.<br>
5. Update records by removing quotes from acct, center,acctdate.<br>
6. Convert American date to database date.<br>
6. Set fld1 to blank so this script only runs on newly imported records next time.<br>
7. OPTIMIZE  TABLE exp_rev";
*/
}
