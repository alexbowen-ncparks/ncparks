...
$var="funding";
	...
	$sql="SELECT t1.funding_id
	from land_asset_funding_junction as t1
	...
	if(mysqli_num_rows($result)>0)
		...
		while($row=mysqli_fetch_assoc($result))
			{
			extract($row);
// $t1_fields from lin 14
			$sql="SELECT $t1_fields, t2.funder_name, funder_abbreviation
			from $var as t1
			left join funder as t2 on t1.funder_id=t2.funder_id
			WHERE t1.funding_id='$funding_id'"; //ECHO "$sql<br />"; //exit;
		...
		{	//$sql<br />
		$message= "No Fundng record was found for land_assets_id $land_assets_id.";
		}
