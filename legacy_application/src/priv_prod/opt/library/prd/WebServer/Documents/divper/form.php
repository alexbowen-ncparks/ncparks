...
$sql = "SELECT DISTINCT park from position where 1 order by park"; //echo "$sql"; exit;
$result = mysqli_query($connection,$sql) or die ("Couldn't execute query.");
while($row=mysqli_fetch_assoc($result))
	{
	$parkCode[]=$row['park'];
	}
// if(empty($parkCode['BOCR'])){$parkCode['BOCR']="BOCR";}
// if(empty($parkCode['PIVI'])){$parkCode['PIVI']="PIVI";}
...
// 	echo "<pre>"; print_r($_POST); echo "</pre>";  exit;
	if(!$seidA=="")
		...
		for($i=0;$i<count($seidA);$i++)
			...
			`park`='$reasonA[$i]', `park_reg`='$reasonA[$i]', `code`='$reasonA[$i]', `code_reg`='$reasonA[$i]', `park`='$reasonA[$i]', `program_code`='$reasonA[$i]',  `program_code_reg`='$reasonA[$i]',  `posType`='$posTypeA[$i]',`section`='$sectionA[$i]',`fund`='$fundA[$i]',`fund_source`='$fund_sourceA[$i]' WHERE seid='$seidA[$i]'";
		...
		header("Location: form.php?park=$parkPass&message=$message");
		exit;
			}// end if !$seid
	} // end Update
...
if($val > -1)
	...
	$sql = "INSERT INTO position SET `park`='$park', `park_reg`='$park',`posNum`='$posNum',`beacon_num`='$beacon_num', `posTitle`='$posTitle',`posTitle_reg`='$posTitle',`rcc`='$rcc',`current_salary`='$current_salary',`salary_grade`='$salary_grade',`exempt`='$exempt',`section`='$sectionPass',`posType`='$posType',`fund`='$fund',`fund_source`='$fund_source'";
	$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql".mysqli_error($connection));
	$message="Addition successful.";
	$sql = "INSERT IGNORE INTO vacant SET `beacon_num`='$beacon_num'";
	$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. $sql".mysqli_error($connection));
	$link="form.php?park=$park";
	echo "<br /><a href='$link'>Return</a> to $park.";
	exit;
	} // end Add
...
// 	echo "$level";
	...
	if($_SESSION['divper']['level'] > 2)
		...
		echo "<table><form method='post' action='form.php'><tr>
...
$sql = "SELECT * From position WHERE park='$park' AND markDel = '' ORDER by current_salary desc";
	$result = mysqli_query($connection,$sql) or die ("Couldn't execute query. ");
	echo "<font color='red'>$message</font>
	<hr><form name='newEvent' method='post' action='form.php'>
