...
FROM `$dbTable`";$result = mysql_query($sql); if($result){$row=mysql_fetch_array($result);extract($row);
...
}else{$dbVal=addslashes($passVal[$k]);}
...
//$result = mysql_query($sql1) or die ("Couldn't execute query. sql1 $sql1");
...
$where $group_by";
$result2 = mysql_query($sql2);$row2=mysql_fetch_array($result2);extract($row2);

//echo "$sql2<br>";exit;
}

if(!$passSQL){$sql1 = "SELECT $from $where $g limit $limit";}
$result = mysql_query($sql1) or die ("Couldn't execute query. sql1 $sql1");
