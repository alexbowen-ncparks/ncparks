...
//	$ex=explode(": Total",$bill_txt);
	$ex=explode(":Bill Number    ",$bill_txt);
//	echo "<pre>"; print_r($ex[0]); echo "</pre>"; exit;

// Get Total of Bill
$grand1=explode("Total.........................",$ex[0]);
	//echo "<pre>"; print_r($grand1); echo "</pre>"; exit;
$grand2=explode(":",$grand1[1]);
$grand_total_format=trim($grand2[0]); $grand_total=trim($grand_total_format,"$");
	//	echo "gt0=$grand_total_format"; //exit;
$grand_total=str_replace(",","",$grand_total); //echo "gt1=$grand_total";//exit;
...
$var1=explode($grand_total_format,$bill_txt);
//	echo "<pre>var1 $grand_total "; print_r($var1); echo "</pre>";  exit;
	
// $var1[0] is summary of all DPR charged numbers
$var2=explode("Employee Summary",$var1[1]); 
$var2[0]="";

//	echo "<pre>var2"; print_r($var2); echo "</pre>";  exit;
...
foreach($var4 as $k=>$v){
	...
	$var7=str_replace(":                                                                                                                                  :","",implode("",$var6));  // remove :bunch of spaces:
	
	$var7=str_replace("
\r\n","",$var7);// remove paragraph symbol
	//echo "<pre>"; print_r($var7); echo "</pre>"; exit;
	
	$var7=str_replace(":\r","\r",$var7);// remove different line feed
	$var7=str_replace("\r","",$var7);// remove different line feed
	$var7=str_replace("\n:",":",$var7);// remove different line feed
	$var7=str_replace(" : "," 
",$var7);// remove different line fedd
	$var7=str_replace(" 
"," 
 :",$var7);// remove different line fedd
...
$fldArray=array("`phone_number`","`local`","`ism`","`voice`","`wan`","`sna`","`p2p`","`dialup`","`long_distance`","`virtual`","`one800`","`calling_card`","`cellular`","`nchih`","`misc_passthru`","`erate`","`lan`","`total`");
...
if($t>0){
	$sql="INSERT INTO charges SET `phone_number`='WAN $k', `total`='$t'";}
	// echo "$sql";exit;
				$result=mysql_query($sql);
...
// right(t1.Mphone,8) as 
$sql="SELECT t1.Fname,t1.Lname, Mphone
...
order by total desc, erate desc, misc_passthru desc, local desc, long_distance desc";
...
echo "<tr$tr>";
	...
	$person=$phoneArray[$v['phone_number']];
		$pn=$v['phone_number'];
		$check="<a href='phone_detail.php?phone_number=$pn&person=$person&billing_period=$billing_period&open_file=$open_file' target='_blank'>[ $var ]</a>";
		$var=$check;
		}
