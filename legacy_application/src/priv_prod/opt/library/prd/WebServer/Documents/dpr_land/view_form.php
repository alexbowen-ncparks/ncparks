...
// "balance"=>"Balance", 
$abstract_table_array=array("land_owner"=>"Owner Contact", "land_assets"=>"Data", "deed_history"=>"Deed History", "proposed_funding"=>"Proposed Funding", "funding"=>"Funding", "spo_milestones"=>"Milestones", "transactions"=>"Transactions", "fas_data"=>"FAS_Data", "documents"=>"Documents", "photos"=>"Photos", "land_notes"=>"Notes","staff_contact"=>"Staff", "associated_files"=>"Associated Files" );

// "balance"=>"Balance", 
$abstract_edit_array=array("land_owner"=>"land_owner_id", "land_assets"=>"land_assets_id", "deed_history"=>"deed_history_id", "proposed_funding"=>"proposed_funding_id", "funding"=>"funding_id", "spo_milestones"=>"milestones_id", "transactions"=>"transactions_id", "fas_data"=>"fas_data_id", "documents"=>"documents_id", "photos"=>"photos_id", "land_notes"=>"land_notes_id", "staff_contact"=>"staff_contact_id", "associated_files"=>"associated_files_id Files" );
...
// 	IF($edit==1 and $var=="land_assets" and in_array($_SESSION['dpr_land']['tempID'],$edit_array))
// 		{
// 		echo "<form action='update_land_assets.php' method='POST'>";
// 		}
	echo "<table class='table' border='1' cellpadding='5'><tr><td colspan='2'></td></tr>";
	foreach($ARRAY AS $index=>$array)
		{
		$item=$index+1;
		$tab_name=$abstract_table_array[$var];
		$edit_table_name=$var;
		$edit_field=$abstract_edit_array[$var];
		$edit_record=$array[$edit_field];
		$action_form="edit_form.php";
		echo "<tr><td bgcolor='yellow'><strong>$tab_name</strong> Item $item</td>
		<td colspan='3' ><form action='$action_form' method='post' target='_blank'>
			<input type='hidden' name='select_table' value='$edit_table_name'>
			<input type='hidden' name='table_id' value='$edit_record'>
			<input type='submit' name='submit_admin' value='Edit $edit_record' style=\"background-color: #e6ccff;\">
			</form>
		...
		IF($edit==1 and $var=="land_assets" and in_array($_SESSION['dpr_land']['tempID'],$edit_array))
				{
				if($fld=="land_assets_id"){$ro="readonly";}else{$ro="";}
				$var_display="<td><input type='text' name='$fld' value=\"$value\" $ro></td>";
//  		echo "<pre>"; print_r($project_status_array); echo "</pre>"; // exit;
// arrays created in sql_land_assets.php
...
// 		IF($edit==1 and $pass_var=="land_assets" and in_array($_SESSION['dpr_land']['tempID'],$edit_array))
// 			{
// 			echo "<tr><td colspan='3' align='center'><input type='submit' name='submit_form' value=\"Update\"></td></tr></table><table>
// 			</form>";
// 			}
		}
// 	$limit_fields_array=array("land_assets","land_owner","transactions");
	$limit_fields_array=array();
	if(empty($show_all) and in_array($pass_var,$limit_fields_array))
		{	
		echo "<tr><td colspan='2' align='center'><form method='POST' action='view_form.php'>
