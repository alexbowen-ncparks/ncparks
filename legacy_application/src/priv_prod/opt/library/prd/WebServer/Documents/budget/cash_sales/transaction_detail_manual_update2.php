...
//$check_name2=addslashes($check_name2);
//$check_name2=(urldecode($check_name2));
// echo "<br />Line 83: check_name2=$check_name2<br />";
...
//KLUDGE to replace html special characters with the ampersand.  TBass 4/11/19

if($level>3){$query3="update crs_tdrr_division_history_parks_manual set check_name=replace(check_name,'&amp;','&') where id='$id[$j]' ";}
if($level>3){$result3=mysqli_query($connection, $query3) or die ("Couldn't execute query 3. $query3");}

if($level>3){$query3a="update crs_tdrr_division_history_parks_manual set comment=replace(comment,'&amp;','&') where id='$id[$j]' ";}
if($level>3){$result3a=mysqli_query($connection, $query3a) or die ("Couldn't execute query 3a. $query3a");}

if($level>3){$query3b="update crs_tdrr_division_history_parks_manual set comment=replace(comment,'amp;','') where id='$id[$j]' ";}
if($level>3){$result3b=mysqli_query($connection, $query3b) or die ("Couldn't execute query 3b. $query3b");}
