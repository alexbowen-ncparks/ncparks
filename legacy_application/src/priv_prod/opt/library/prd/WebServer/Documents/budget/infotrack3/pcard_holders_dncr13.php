...
foreach($_REQUEST as $k=>$v)
	...
	//echo "<tr>";
		...
		foreach($v as $k1=>$v1){// each field, e.g., $tempID=$v[tempID];
			...
			//echo "<tr><td>$k1</td><td><input type='text' name='$k1' value='$v1'></td></tr>";
			$query="update pcard_holders_dncr2 set $k1='$v1' where id='$id' ";
			echo "query=$query<br />";
			//$result = mysqli_query($connection, $query) or die ("Couldn't execute query.  $query");	
...
//echo "<form action='pcard_holders_dncr10.php'>";
...
$count=count($fieldNames);
echo "<form method='post' action='pcard_holders_dncr13.php'>";
echo "<table border='1' cellpadding='2'>";
foreach($ARRAY as $k=>$v)
	{// each row
...
foreach($ARRAY as $k=>$v)
	...
	echo "<tr>";
		...
		foreach($v as $k1=>$v1){// each field, e.g., $tempID=$v[tempID];
			...
			if(in_array($k1,$LinkFields)){echo "<td>$v1 <a href='pcard_holders_dncr13.php?edit=y&id=$v1'>Edit</a></td>";} else  {echo "<td>$v1</td>";}
