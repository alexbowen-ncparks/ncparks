...
{
if($center_code!='')
{$query5="select * from energy_report_water_cost where 1
and f_year='$f_year' and park='$center_code'
order by f_year,park,water_account_number
";
$query5a="select sum(total_cost_dollars) as 'yearly_cost',
                sum(total_usage_gal) as 'yearly_usage',
				((sum(total_cost_dollars))/(sum(total_usage_gal)/1000)) as 'yearly_rate'
				from energy_report_water_cost where 1
                 and f_year='$f_year' and park='$center_code' ";
...
$query5a="select sum(total_cost_dollars) as 'yearly_cost',
                sum(total_usage_gal) as 'yearly_usage',
				(sum(total_cost_dollars)/sum(total_usage_gal)) as 'yearly_rate'
				from energy_report_water_cost where 1
...
echo "<table border='1'><tr><th>Yearly<br />Cost</th><th>Yearly<br />Usage</th><th>Rate</th></tr>
             <tr><td>$yearly_cost</td><td align='center'>$yearly_usage<br />gal</td><td>$yearly_rate/(1000gal)</td></tr>
	  </table>";
...
// echo "<td><a href='energy_reporting.php?f_year=$f_year&park=$park&center=$center'>$f_year</a></td>";
  ...
  echo "   <td>$f_year</td>
           ...
           <td>$park</td>		   
           <td>$rank</td>		   
           <td><a href='/budget/energy/energy_reporting.php?f_year=$f_year&egroup=water&report=cdcs&valid_account=y&center_code=$park&account_number=$water_account_number' target='_blank'>$water_account_number</a></td>		   
           <td>$building_name</td>                   
           <td>$july</td>                   
           <td>$august</td>                   
           <td>$september</td>                   
