...
echo  "<td>$amount</td>";
echo  "<td>$datenew</td>";
echo "<form method='post' action='stepJ9v_match.php'>"; 
...
echo "<input type='hidden' name='project_category' value='FMS'>
...
echo "<table>";
echo "<tr>";
echo "<td><form method=post='post' action='stepJ9v_update.php'>
     ...
     <input type='hidden' name='step' value='$step'>
     <input type='hidden' name='step_num' value='$step_num'>";	 
     //echo "<input type='submit' name='submit' value='caa_match'>";
     //echo "<input type='submit' name='submit' value='ca_match'>";
     echo "<input type='submit' name='submit' value='center_match'></form></td>";	 
...
if($posted_lookup == 'y')
{
$query2="select vendorname as 'vendor_name',invoice as 'ncas_invoice_number',amount as 'ncas_invoice_amount',
         datenew as 'system_entry_date',proj_num as 'project_number',xtid as 'id'
		 from partf_payments
		 where center='$center' and datenew < '$start_date'
		 order by system_entry_date desc         
";
}
...
if($posted_lookup != 'y')
{
echo "<H2 ALIGN=LEFT><mark><font color='blue'>SECTION 2-Code Sheets for Center: $center</font></mark></H2>";
echo "<H2 ALIGN=LEFT><font color=blue><a href='stepJ9v_update.php?xtid=$xtid&center=$center&project_category=$project_category&project_name=$project_name&fiscal_year=$fiscal_year&start_date=$start_date&end_date=$end_date&step_group=$step_group&step=$step&step_num=$step_num&submit=$submit&posted_lookup=y'>Lookup previous Posted Payments for Center-$center</a></font></H2>";
}


if($posted_lookup == 'y')
{
echo "<H2 ALIGN=LEFT><mark><font color='blue'>SECTION 2-Previous Posted Payments for Center: $center</font></mark></H2>";
echo "<H2 ALIGN=LEFT><font color=blue><a href='stepJ9v_update.php?xtid=$xtid&center=$center&project_category=$project_category&project_name=$project_name&fiscal_year=$fiscal_year&start_date=$start_date&end_date=$end_date&step_group=$step_group&step=$step&step_num=$step_num&submit=$submit'>Lookup Code Sheets for Center-$center</a></font></H2>";
}
