...
$text="DPR";
pdf_show_xy ($pdf, $text, 72 ,$ver-11);
// last_name, first_name
$text=strtoupper($last_name.", ".$first_name);
pdf_show_xy ($pdf, $text, 163 ,$ver-11);

pdf_setlinewidth($pdf,4);
pdf_rect($pdf, 65, $ver-18, 63, 28); // division
  pdf_stroke($pdf);
pdf_rect($pdf, 155, $ver-18, 400, 28); // name
  pdf_stroke($pdf);

$fontSize=8;
pdf_setfont ($pdf, $times, $fontSize);
$text="DIVISION";
pdf_show_xy ($pdf, $text, 77 ,$ver-30);
// last_name, first_name
$text="CARDHOLDER NAME (LAST, FIRST)";
pdf_show_xy ($pdf, $text, 200 ,$ver-30);
...
pdf_rect($pdf, 25, $ver-424, 565, 440); // page rect
  pdf_stroke($pdf);
  
pdf_moveto($pdf, 25, PAGE_HEIGHT-95);
		pdf_lineto($pdf, 590, PAGE_HEIGHT-95);
		pdf_stroke($pdf);
...
pdf_setfont ($pdf, $times, $fontSize);
			...
			$text="*Do Not Use this form to reconcile purchases to Account Number 532711 - 532729 and 532930. A valid, approved Travel Reimbursement form should be used for these types fo reconciliations.*";
				pdf_show_boxed ($pdf, $text, 150,275,400,20,"left","");
				
				$x=40; $y=250;$text="Justification:";
					pdf_show_xy ($pdf, $text, $x ,$y);
					pdf_rect($pdf, 100, 220, 450, 40); // rect.
 					 pdf_stroke($pdf);
					
				$x=40; $y=200;$text="Supervisor Signature";
					pdf_show_xy ($pdf, $text, $x ,$y);
...
pdf_setfont ($pdf, $times, $fontSize);					
	...
	pdf_show_boxed ($pdf, $text, 100 ,70,450,40,"left","");		
