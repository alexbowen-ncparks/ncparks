...
/*
if($beacnum=='65027688' or $temp_id='Owen1111')
{
echo "<pre>"; print_r($_SESSION);
...
// added on 11/21/2015
{
$sql = "SELECT new_rcc as 'rcc' From center where parkCode='$parkcode' and new_fund='1680'";
// echo "sql=$sql<br />";
$result = mysqli_query($connection, $sql) or die ("Couldn't execute query. $sql");
...
if($beacnum=='60033226'){echo "parkRCC=$parkRCC[$parkcode]";}

// deleted on 11/21/2015
/*
{
...
// added on 11/21/2015
if(!@$ncas_rcc){$ncas_rcc=@$parkRCC[$parkcode];}
...
/*
if($beacnum=='60033242' or $beacnum=='60033093') //rebecca owen,val mitchener
{
...
//if($beacnum!='60033242' and $beacnum!='60033093') //rebecca owen,val mitchener
{
echo "<tr><td>vendor_number</td><td><input type='button' value='Get Vendor Info' onclick=\"return popitup('vendor.php?parkcode=$parkcode')\"> <input name='vendor_number' type='text' value='$vendor_number' size='40' readonly='readonly'> New Vendor <input type='checkbox' name='new_vendor' value='x'$nvCK>
</td></tr>";
}
...
if($beacnum!='60033242' and $beacnum!='60033093') //rebecca owen,val mitchener
{
echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; group_number <input type='text' name='group_number' value='$group_number' size='5' readonly='readonly'></td></tr>";
}




echo "<tr><td>vendor_address</td><td><textarea name='vendor_address' cols='45' rows='3' >$vendor_address</textarea>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; pay_entity <input type='text' name='pay_entity' value='$pay_entity' size='5' ></td></tr>";
...
//echo "<br />temp_id=$temp_id<br />";
//Jennifer Goss or Becky Owen
//if($beacnum=='60032787' or $beacnum=='65027688' or $temp_id=='Owen1111')
{
echo "
<tr><td>
...
echo "<table>
<tr>
<td><input type=\"button\" value=\"PARTF Project Info\" onClick=\"return popitup('partf.php?parkcode=$parkcode')\"></td>
...
// ! Jennifer Goss and ! Becky Owen

/*
if($beacnum!='60032787' and $beacnum!='65027688' and $temp_id!='Owen1111')
{
echo "
<tr><td>
 &nbsp;&nbsp; <input type=\"button\" value=\"View POs\" onClick=\"return popitup('po.php?parkcode=$parkcode&po_number=$ncas_po_number')\">
...
echo "<table>
<tr><td><font color='red'>Per Budget Office: CI Project Funds<br />unavailable to pay Invoices effective 5/21/21</font><br /><font class='cartRow'>Project Button removed</font></td>";
//<td><input type=\"button\" value=\"PARTF Project Info\" onClick=\"return popitup('partf.php?parkcode=$parkcode')\"></td>

echo "<td>project_number <input name='project_number' type='text' value='$project_number' size='8' READONLY> <font size='-2'>Project number must be entered using Project Info button.</font>
<br>company <input name='ncas_company' type='text' value='$ncas_company' size='8' READONLY>
<br>budget_code <input name='ncas_budget_code' type='text' value='$ncas_budget_code' size='8' READONLY>";
}
...
$query1 = "SELECT center_desc as 'center_desc' From center where parkCode='$parkcode' and new_fund='1680'";
echo "query1=$query1";
$result1 = mysqli_query($connection, $query1) or die ("Couldn't execute query 1.  $query1");

$row1=mysqli_fetch_array($result1);
extract($row1);//brings back max (end_date) as $end_date
echo "center_desc=$center_desc";
...
<tr><td align='right'>remit_code_Customer#</td><td><textarea name='ncas_remit_code' cols='55' rows='2'>$ncas_remit_code</textarea>&nbsp;&nbsp;&nbsp;accrual_code <input type='text' name='ncas_accrual_code' value='$ncas_accrual_code' size='4'></td></tr>";
if($ncas_remit_park=='')
{
echo "<tr><td align='right'>remit_code_Park</td><td><textarea name='ncas_remit_park' cols='55' rows='2'>$center_desc</textarea></td></tr>";
}

if($ncas_remit_park!='')
{
echo "<tr><td align='right'>remit_code_Park</td><td><textarea name='ncas_remit_park' cols='55' rows='2'>$ncas_remit_park</textarea></td></tr>";
}




echo "<tr><td align='right'>comments</td><td><textarea name='comments' cols='55' rows='2'>$comments</textarea>&nbsp;&nbsp;&nbsp; ";
if($level > 2)
{
...
echo "<table>";
// echo "<pre>"; print_r($receive); echo "</pre>"; // exit;
...
for($j=0;$j<count($receive);$j++)
	{
	if($receive[$j]==$passReceived_by){$s="selected";}else{$s="";}
		 echo "<option value='$receive[$j]' $s>$receive[$j]\n";
	}
...
for($j=0;$j<$k;$j++)
	{
	if($prepare[$j]==$passPrepared_by){$s="selected";}else{$s="";}
		 echo "<option value='$prepare[$j]' $s>$prepare[$j]\n";
	}
...
if(@$submit_acs=="Submit"||$formType=="Update")
	...
	if($project_number!="")
		{
		echo "<form action='/budget/acs/acs_ci_pdf.php'>";
		}
		else
		{
	...
	<input type='submit' name='submit_acs' value='Preview'> Click browser's back button after Preview.</form></td>";
	
	if($level>0)
		{
	echo "<td><form action='/budget/acs/acs_pdf_dncr.php'>
	<input type='hidden' name='m' value='invoices'>
	<input type='hidden' name='vendor_name' value='$vendor_nameURL'>
	<input type='hidden' name='due_date' value='$due_date'>
	<input type='hidden' name='prepared_by' value='$passPrepared_by'>
	<input type='submit' name='submit_acs_dncr' value='Preview DNCR'><br />Click browser's back button after Preview DNCR.</form></td></td>";
		}
