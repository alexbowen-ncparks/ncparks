...
{
$query5="select vmc_posted5_v2.park,vmc_posted5_v2.center,vmc_posted5_v2.f_year,sum(jul) as 'jul',
sum(aug) as 'aug',sum(sep) as 'sep',sum(oct) as 'oct',sum(nov) as 'nov',sum(dece) as 'dece',sum(jan) as 'jan',sum(feb) as 'feb',sum(mar) as 'mar',sum(apr) as 'apr',sum(may) as 'may',sum(jun) as 'jun',sum(total) as 'total'
from vmc_posted5_v2
left join center on vmc_posted5_v2.center=center.center
where 1
and dist='north' and fund='1280' and actcenteryn='y'
and center.parkcode != 'mtst' 
and center.parkcode != 'harp' 
and f_year='$f_year' 
group by vmc_posted5_v2.center,vmc_posted5_v2.f_year
order by vmc_posted5_v2.park;";
}
*/
//6/30/14
/*
...
*/
if($e_group=='electricity' and $report=='accounts'){include "electricity_accounts.php" };
if($level>2)
{
...
"<tr$t>";
 ...
 /*	   
     ...
     echo "<td><a href='.php?f_year=$f_year&park=$park&center=$center'>$park</a></td>
           <td>$center</td>		   
           <td>$f_year</td>		   
           <td>$jul</td>		   
           <td>$aug</td>		   
