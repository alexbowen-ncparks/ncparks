...
if(mysqli_num_rows($result)<1)
	{
	echo "No items marked for Surplus at $limit_level.";
	exit;
	}
while ($row=mysqli_fetch_assoc($result))
	{
...
$sql="SELECT t1.park as dist, t1.working_title, concat(t3.Fname, if(Nname!='', concat(' (',Nname,') '), ' '),t3.Lname) as name, t3.email
	...
	left join emplist as t2 on t1.beacon_num=t2.beacon_num
	left join empinfo as t3 on t3.emid=t2.emid
	where t1.posTitle='Accounting Clerk V'
	";
	$result = mysqli_query($connection,$sql) or die ("Couldn't execute query 1. $sql ".mysqli_error($connection));
	while ($row=mysqli_fetch_assoc($result))
		{
		$ACCO_array[]=$row['name'];
		$ACCO_email=$row['email'];
		}
//echo "<pre>"; print_r($chop_array); echo "</pre>"; // exit;
...
foreach($approve_array as $k=>$v)
	...
	if(@$_POST['approve_level']==$v)
		...
		{$ck="";}
	if($_SESSION['fixed_assets']['level']<=$k){continue;}
...
$center_code=substr($_POST['single_location'],3,4);
$sql="SELECT center as center_dpr
	from budget.center
	where parkCode='$center_code'
	";
...
if(!isset($pasu_date))
	...
	$var_sub="subject=$email_location items to be Surplused";
$site_link=urlencode("http://www.dpr.ncparks.gov/fixed_assets/surplus_equip_form.php?location=$email_location&act=review");
...
if($level==3 or $level>3)
	...
	if(!isset($chop_date))
		{$chop_date="0000-00-00";}
		
		
	if(!isset($ACCO_email) or $chop_date=="0000-00-00")
		{$email_ACCO="";}
		else
		{
		$var_sub="subject=$email_location items to be Surplused";
		...
		$db_link=urlencode("http://www.dpr.ncparks.gov/fixed_assets/index.html");
		$var_sub.="&body=Follow this link to the site for review: $site_link You may have to login first. $db_link";
		$email_ACCO="email <a href=\"mailto:$ACCO_email?".$var_sub."\">Bus. Office</a>";
		}
