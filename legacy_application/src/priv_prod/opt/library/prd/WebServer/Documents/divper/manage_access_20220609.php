...
if($_SESSION['parkS'] !="" or $park != "")
	...
	@$ln=$Lname;
	$sql = "SELECT B0149.position_desc as BEACON_title, emplist.*,empinfo.Fname,empinfo.Lname,empinfo.Nname,position.posTitle, position.beacon_num
	From empinfo 
...
// 	echo "$sql";
	...
	if(@$emid!="" && @$Lname=="")
		{
		$sql = "SELECT B0149.position_desc as BEACON_title, emplist.*,empinfo.Fname,empinfo.Lname,empinfo.Nname,position.posTitle
		From empinfo 
		left join emplist on emplist.emid=empinfo.emid
		left join position on emplist.beacon_num=position.beacon_num
	left join B0149 on B0149.position=position.beacon_num
...
if(empty($_REQUEST))
	...
	// Name comes from B0149
	// Code is made up for use here.
	$beacon_title_match=array("Administrative Specialist"=>"OA","Maintenance/Construction Supervisor"=>"MCSU","Parks Superintendent"=>"PASU","Park Ranger"=>"PARA");
...
if(!empty($temp))
	{$new_temp=implode(", ",$temp);}else{$new_temp="";}
		...
		echo "</tr><tr><td>BEACON_title <b>$BEACON_title</b> <a onclick=\"toggleDisplay('$emid');\" href=\"javascript:void('')\">$match</a>
<div id=\"$emid\" style=\"display: none\">$new_temp</div></td>";
		...
		<td>Itineray Print <input type='text' size='10' name='itinerary' value='$itinerary'></td><td>Supervise Positions: <textarea name='supervise' cols='43' rows='1'>$supervise</textarea></td>
		<td><input type='hidden' name='park' value='$currPark'>
		<input type='hidden' name='emid' value='$emid'>
		<input type='submit' name='Submit' value='Update'>
		</td></form><td><form method='post' action='manage_access.php'>
		<input type='hidden' name='emid' value='$emid'>
