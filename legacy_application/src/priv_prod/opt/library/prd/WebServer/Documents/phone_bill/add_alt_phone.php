...
$database="divper";
include("../../include/auth.inc"); //echo "c=$connection";
$database="phone_bill";
...
if(@$park=="")
	...
	echo "<form action='add_alt_phone.php' method='POST' target='_blank'>
	<table align='center' cellpadding='7'>

	<tr><td>Pick a Park: 
	<select name='park'><option selected=''></option>";
	$pa=array_unique($park_array);
	foreach($pa as $k=>$v)
		{
...
if(@$_POST['alt_lines']!="")
	...
	//	echo "<pre>";print_r($_POST);echo "</pre>";
		...
		//exit;
		 $result = MYSQL_QUERY($sql,$connection);
//	echo "hello";exit;
		header("Location: add_alt_phone.php?park=$park");
		exit;
	}
...
$sql="SELECT *
	...
	while($row=mysql_fetch_assoc($result))
		{
		extract($row);
		echo "<a href='update_alt_phone.php?pn=$alt_lines&park=$park'>$alt_lines</a> => $location<br />";
		}
