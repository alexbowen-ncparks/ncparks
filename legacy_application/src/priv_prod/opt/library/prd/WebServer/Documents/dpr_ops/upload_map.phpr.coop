...
*/
	...
	if($error==0){
			$temp_name=$_FILES['map']['tmp_name'];
			if($temp_name==""){continue;}
			$name=$_FILES['map']['name'];
			$name=str_replace("'","",$name);
			$name=addslashes($name);
		extract($_POST);
		$form_name="DPR_map_".$proj_number."_".time();
		$e=explode(".",$_FILES['map']['name']);
		$ext=array_pop($e);
		$file_name = $form_name.".".$ext;

//echo " $file_name"; exit;

		$uploaddir = "upload_map"; // make sure www has r/w permissions on this folder
		...
		$sql="INSERT INTO upload_map SET proj_id='$proj_id', proj_number='$proj_number', file_name='$name', link='$uploadfile' ";
