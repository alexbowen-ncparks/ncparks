<?php
//echo "<pre>";print_r($_REQUEST);echo "</pre>";//exit;
session_start();
//echo "<pre>";print_r($_SESSION);echo "</pre>";exit;
$level=$_SESSION['budget']['level'];
$posTitle=$_SESSION['budget']['position'];
$tempid=$_SESSION['budget']['tempID'];
//echo $tempid;
extract($_REQUEST);
//echo "<pre>";print_r($_REQUEST);"</pre>";exit;

$database="budget";
$db="budget";
include("/opt/library/prd/WebServer/include/iConnect.inc"); // connection parameters
mysqli_select_db($connection, $database); // database
include("../../../../include/activity.php");// database connection parameters
//echo "submit1=$submit1";echo "submit2=$submit2";exit;

//changed on 5/14/20-TBass
/*
$query1="update pcard_extract_worksheet,pcard_unreconciled
set pcard_extract_worksheet.pcard_num=pcard_unreconciled.pcard_num,
pcard_extract_worksheet.pcard_user=pcard_unreconciled.last_name,
pcard_extract_worksheet.pcard_vendor=pcard_unreconciled.vendor_name,
pcard_extract_worksheet.pcard_transdate=pcard_unreconciled.trans_date
where pcard_extract_worksheet.pcard_trans_id=pcard_unreconciled.transid_new
and pcard_unreconciled.transid_date_count='1' and pcard_extract_worksheet.denr_paid='n';";
*/

$query1="update pcard_extract_worksheet,pcard_unreconciled
set pcard_extract_worksheet.pcard_num=pcard_unreconciled.pcard_num,
pcard_extract_worksheet.pcard_user=pcard_unreconciled.last_name,
pcard_extract_worksheet.pcard_vendor=pcard_unreconciled.vendor_name,
pcard_extract_worksheet.pcard_transdate=pcard_unreconciled.trans_date
where pcard_extract_worksheet.pcard_trans_id=pcard_unreconciled.transid_new
and pcard_extract_worksheet.center=pcard_unreconciled.center
and pcard_extract_worksheet.acct=pcard_unreconciled.ncasnum
and pcard_extract_worksheet.debit_credit=pcard_unreconciled.amount
and pcard_unreconciled.ncas_yn='n' ";


mysqli_query($connection, $query1) or die ("Couldn't execute query 1. $query1");

/*
$query2="update pcard_extract_worksheet,pcard_users
set pcard_extract_worksheet.pcard_user=pcard_users.last_name
where pcard_extract_worksheet.pcard_num=pcard_users.card_number 
and pcard_extract_worksheet.denr_paid='n' 
and pcard_users.card_number != '' ;";
mysqli_query($connection, $query2) or die ("Couldn't execute query 2. $query2");
*

/*
$query2="update pcard_extract_worksheet,pcard_holders_dncr2
set pcard_extract_worksheet.pcard_user=pcard_holders_dncr2.last_name
where pcard_extract_worksheet.pcard_num=pcard_holders_dncr2.pcard_num 
and pcard_extract_worksheet.denr_paid='n' ;";
mysqli_query($connection, $query2) or die ("Couldn't execute query 2. $query2");
*/








$query23a="update budget.project_steps_detail set status='complete' where project_category='$project_category'
         and project_name='$project_name' and step_group='$step_group' and step_num='$step_num' ";
			 
mysqli_query($connection, $query23a) or die ("Couldn't execute query 23a.  $query23a");

$query24="select * from budget.project_steps_detail
         where project_category='$project_category' and project_name='$project_name'
		 and step_group='$step_group'  and status='pending' "; 

$result24=mysqli_query($connection, $query24) or die ("Couldn't execute query 24.  $query24");

$num24=mysqli_num_rows($result24);

//echo "pending_items=$num4";exit;

//if($num4==0){echo "done"}; if ($num4!=0){echo "$num4 pending items"}; exit;
if($num24==0)

{$query25="update budget.project_steps set status='complete' where project_category='$project_category'
         and project_name='$project_name' and step_group='$step_group' ";
mysqli_query($connection, $query25) or die ("Couldn't execute query 25.  $query25");}
////mysql_close();

if($num24==0)

{header("location: main.php?project_category=$project_category&project_name=$project_name ");}

if($num24!=0)

{header("location: step_group.php?project_category=$project_category&project_name=$project_name&step_group=$step_group&fiscal_year=$fiscal_year&start_date=$start_date&end_date=$end_date");}




?>

























