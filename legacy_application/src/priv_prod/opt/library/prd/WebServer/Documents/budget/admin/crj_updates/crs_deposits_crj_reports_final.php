...
if($cashier_overshort_comment=='' and 
   $manager_overshort_comment=='' and 
   $fs_approver_overshort_comment=='' and 
   $accountant_comment=='')
  {
    $overshort_comments='NO';
  }
else
  {
    $overshort_comments='YES';
  }
...
extract($row12a3);
if($vendor_fee_adjustment and 
   $deposit_id=='243571880')
{
...
/* 2022-02-24: CCOOPER - A Tony Beacon position-specific access/functionality, I will comment out for when new Accountant is hired
 if($beacnum=='60032793')
 {
 $pid='72';
...
//echo "deposits=$deposits";
 {
    ...
    echo "<table align='center'>
            <tr>
              <th><a href='/budget/menu1314.php'>
                <img height='50' width='50' src='/budget/infotrack/icon_photos/home1.png' alt='picture of home'></img></a>
              </th>
              <th><a href='/budget/admin/crj_updates/bank_deposits_menu_division_final.php?menu_id=a&menu_selected=y'>
                  <img height='50' width='50' src='/budget/infotrack/icon_photos/bank.jpg' alt='picture of bank'></img></a>
              </th>
              <th>Cash Receipts Journal (2.14)- ORMS Deposit ID
              </th>
              <td>$GCN <a href=crs_deposits_crj_reports_final.php?deposit_id=$deposit_id&GC=n&dncr=$dncr><font $shade_deposit_id>$deposit_id</font></a>";
    ...
    if($deposit_id_first4=='ADMI')
    {
        echo "<br /><a href='/budget/cash_sales/bank_deposit_step2_reload_deposit_slip.php?deposit_id=$deposit_id'>Re-Load Deposit Slip</a>";
    }
        echo "</td>";
...
//echo "<br />dncr_gift_edit=$dncr_gift_edit<br />";
   ...
   if($deposits>=1)
   {
     echo "<td>$GCY<a href=crs_deposits_crj_reports_final.php?deposit_id=$deposit_id&GC=y&dncr=$dncr><font $shade_deposit_id_GC>$deposit_id_GC</font></a>
           </td>";
   }
           "</tr>   
        </table>";
...
while ($row12=mysqli_fetch_array($result12))
	...
	if($ncas_account=='000300000'){$account_name="Raleigh Refund by Check";}
...
/* 2022-02-24: CCOOPER - clean up code format, give A Boggus, C Williams, and R Gooding access to the Bank Deposit functionality that T Dodd and H Rumble have - when the park does not enter comments, they still can */ 
if($overshort_comments=='NO')
{
  if($beacnum=='60032781' or 
     $beacnum=='60036015' or
     $beacnum=='60033242' or
     $beacnum=='65032827' or
     $beacnum=='60032997')
    {
        echo "<br />Comments: <a href='overshort_comments.php?id=$id' target='_blank'>$overshort_comments</a>";
    }
  if($beacnum!='60032781' and 
...
//echo "<br />crj_prepared_by=$crj_prepared_by<br />";
$database="divper";
//include("/opt/library/prd/WebServer/include/iConnect.inc"); // connection parameters
mysqli_select_db($connection, $database); // database
//echo "cashier=$cashier<br />";
...
/* 2022-02-24: CCOOPER - adding A Boggus, C Williams, and R Gooding to the Business office duties */
if($cashier=='Rumble2030'){$crj_prepared_by="Heide Rumble";}
if($cashier=='Boggus9933'){$crj_prepared_by="Angela Boggus";}
if($cashier=='Williams6429'){$crj_prepared_by="Carmen Williams";}
if($cashier=='Gooding5256'){$crj_prepared_by="Rachel Gooding";}
...
mysqli_select_db($connection, $database); // database
	...
	//echo "sql3=$sql3<br />";
	$result3 = mysqli_query($connection, $sql3) or die ("Couldn't execute query3. $sql3<br />$database ".mysqli_error($connection));
...
$result5 = mysqli_query($connection, $sql5) or die ("Couldn't execute query5. $sql5 $database");
