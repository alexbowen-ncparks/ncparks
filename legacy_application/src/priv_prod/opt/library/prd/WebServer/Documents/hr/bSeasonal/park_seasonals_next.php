...
bl(print_r($_SESSION, true));
bl(print_r($_REQUEST, true));
bl(print_r($_POST, true));; 
...
// the little block at the top left of the page
if($level>3)
	{
...
// list of parks (center code)

if($level<2)
	{
	
	$parkList=explode(",",$_SESSION['hr']['accessPark']);
	bl(print_r($parkList, true));
	if($parkList[0]!="")
		{
		...
		echo "<td><form><select name=\"center_code_1\" onChange=\"MM_jumpMenu('parent',this,0)\"><option selected=''>Select Park</option>";
			foreach($parkList as $k=>$v)
				{
				$con1="park_seasonals_next.php?center_code=$v";
...
// menu "Form to Request NEXT cycle"
if(@$rep==""){include("menu_next.php");}



// the menu strip with center code, lock, unlock, etc.
...
if(@$rep=="")
	...
	if($level>1)
		{
			...
			foreach($center_code_array as $k=>$v)
				...
				echo "<option $s='park_seasonals_next.php?center_code=$v'>$v</option>";
				  }
		   echo "</form></td>";	
		}
		...
		if($center_code=="All")
			{
			echo "<td><a href='park_seasonals_next.php?center_code=All&rep=excel'>Excel</a> export</td>";
			}
	...
	{
	  echo "<td><FORM>
	<INPUT TYPE=\"BUTTON\" VALUE=\"LOCK\" ONCLICK=\"window.location.href='park_seasonals_next.php?lock=y'\"> $lock_date
	</FORM></td>
	<td><FORM>
	<INPUT TYPE=\"BUTTON\" VALUE=\"UNLOCK\" ONCLICK=\"window.location.href='park_seasonals_next.php?lock=n'\"> 
	</FORM>
	</td>";
	
	if($level>4)
		{echo "
		<td><FORM action='park_seasonals_next.php'>
		Start: 
...
,t1.start_date
,t1.budget_hrs_a
,t1.budget_hrs_a_other
,t1.budget_weeks_a
,t1.budget_weeks_a_other
,t1.month_11
,t1.aca
,t1.avg_rate_new
,t1.six_month
,t1.estimate
,t1.budget_dollars_a
,t1.budget_dollars_a_estimate_temp
,t1.budget_dollars_a_estimate_seasonal
,t1.district
,t1.park_comments
,t1.park_approve
,t2.position_desc as HR_position_title";
...
// ************** Records for top table - Positions made avail for hiring by Park
$date_array=array();
$sql = "SELECT $flds_1
...
// ***** Park selected table *********
$skip=array("district","park_approve","budget_hrs_a_other","budget_weeks_a_other");
...
if(@$date_array)
	...
	if(@$rep=="")
		{
echo "<form method='POST' name='frm_0' action='park_seasonals_hours_next.php' onsubmit=\"return checkJustification($pass_count)\">";
		}
	
	echo "<table border='1' cellpadding='3' align='center'>";
	
	// Header row of html table
 ...
 <tr>
  ...
  <h2>
   <a href='instructions.php?s=$new_request_date&e=$new_request_date_end' target='_blank'>Instructions
   </a>
   </h2>
  </td>
 <td colspan='10'>
  ...
  Actual request period is 
  <font color='green' size='+2'>$new_request_date
  </font> to 
  <font color='red' size='+2'>$new_request_date_end
  </font>
 </td>
</tr>
...
<tr>
 <td colspan='3' align='center'>
  <input name=\"btn\" type=\"button\" onclick=\"CheckAll_id($pass_count)\" value=\"Check All\">
  <input name=\"btn\" type=\"button\" onclick=\"UncheckAll_id($pass_count)\" value=\"Uncheck All\">
 </td>
 ";
...
$required_array=array("start_date","budget_hrs_a", "budget_weeks_a",  "month_11","park_comments","estimate");
$pass_header="";
	foreach($date_array[0] as $fld=>$val)
		...
		if($fld=="budget_dollars_a_estimate_temp")
			{
			$header_fld="Dollars (Estimate Temp)";
			}
		if($fld=="budget_dollars_a_estimate_seasonal")
			{
			$header_fld="Dollars (Estimate Seasonal)";
			}
		...
		if($fld=="six_month")
			{
			$header_fld="Temp <font color='magenta'>(6-11 mo)</font> or Seasonal <font color='green'>(less than 6 months)</font>";
			}

		// place estimate column HERE ***********************************
		if($fld=="estimate")
			{
			$header_fld="Estimate $$ for Seasonal vs. Temp";
			}
		...
		if($fld=="park_comments")
			...
			$header_fld.=" <a href='park_seasonals_next.php?center_code=$center_code&sort=$s'>sort</a>";
			}
	...
	foreach($date_array as $number=>$fields)
		...
		foreach($fields as $fld_name=>$value)
			...
			if($fld_name=="budget_hrs_a")
				...
				if(@$rep=="")
					{
					$budget_hrs_a_other=$fields['budget_hrs_a_other'];
					$value="<font size='-2'>peak</font><input type='text' name='budget_hrs_a[$beacon_number]' value='$budget_hrs_a' size='4' >
					<br /><font size='-2'>other</font><input type='text' name='budget_hrs_a_other[$beacon_number]' value='$budget_hrs_a_other' size='4' >";
					}
				else
					{
					$value=$budget_hrs_a;
			...
			if($fld_name=="budget_weeks_a")
				...
				@$totAmount_a_other+=$budget_weeks_a_other*$date_array[$number]['budget_hrs_a_other']*$date_array[$number]['avg_rate_new'];
				if(@$rep=="")
					{
					$budget_weeks_a_other=$fields['budget_weeks_a_other'];
					$value="<font size='-2'>peak</font><input type='text' name='budget_weeks_a[$beacon_number]' value='$budget_weeks_a' size='4' >
					<br /><font size='-2'>other</font><input type='text' name='budget_weeks_a_other[$beacon_number]' value='$budget_weeks_a_other' size='4' >";
					}
				else
			...
			if($fld_name=="six_month")
				{
				if(@$rep=="")
					{
					if($value=="seasonal")
						{
						$ck_y="checked";$ck_n=""; $color_y="black";$color_n="black"; $td=" bgcolor='green'";
						$disabled="";
						}
					else
						{$ck_y="";$ck_n="checked"; $color_y="black";$color_n="black";$td=" bgcolor='magenta'";
						$disabled="disabled";
						if ($level >= 6) { $disabled=""; }
						}
					$value="<input type='radio' $disabled name='six_month[$beacon_number]' value='seasonal' $ck_y><font color='$color_y'>Seas.</font><br />";
					$value.="<input type='radio' name='six_month[$beacon_number]' value='temp' $ck_n><font color='$color_n'>Temp</font>";
					}
					
				}		

			if($fld_name=="estimate")
				{
				if(@$rep=="")
					{
					if($value=="seasonal")
					...
					$value="<input type='radio' name='estimate[$beacon_number]' value='seasonal' $ck_y><font color='$color_y'>Seas.</font><br />";
					$value.="<input type='radio' name='estimate[$beacon_number]' value='temp' $ck_n><font color='$color_n'>Temp</font>";
					}
...
// 				$tot_hours=$half_2_hrs + $half_3_hrs;
			...
			if($fld_name=="budget_dollars_a")
				{
				$value_a=$date_array[$number]['budget_weeks_a']*$date_array[$number]['budget_hrs_a']*$date_array[$number]['avg_rate_new'];
				$value_a_other=$date_array[$number]['budget_weeks_a_other']*$date_array[$number]['budget_hrs_a_other']*$date_array[$number]['avg_rate_new'];
				$value_a_format=number_format($value_a,2);
				$value_a_other_format=number_format($value_a_other,2);
				$tot=$value_a+$value_a_other;
				$park_total+=$tot;
				$value=$value_a_format."<br />".$value_a_other_format."<br />
...
// 				$value=$value_a."<br />".$value_b."<br />".$value_c;
			...
			if($fld_name=="budget_dollars_a_estimate_seasonal")
				{
				if ($fields['estimate']=="seasonal") {
					$value_a_seasonal=$date_array[$number]['budget_weeks_a']*$date_array[$number]['budget_hrs_a']*$date_array[$number]['avg_rate_new'];
					$value_a_seasonal_other=$date_array[$number]['budget_weeks_a_other']*$date_array[$number]['budget_hrs_a_other']*$date_array[$number]['avg_rate_new'];
					$value_a_seasonal_format=number_format($value_a_seasonal,2);
					$value_a_seasonal_other_format=number_format($value_a_seasonal_other,2);
					$tot_seasonal=$value_a_seasonal+$value_a_seasonal_other;
					$park_total_seasonal+=$tot_seasonal;
					$value=$value_a_seasonal_format."<br />".$value_a_seasonal_other_format."<br />
					Total: ".number_format($tot_seasonal,2);
					} else {
					$value_a_seasonal=0;
					$value_a_seasonal_other=0;
					$value_a_seasonal_format=number_format($value_a_seasonal,2);
					$value_a_seasonal_other_format=number_format($value_a_seasonal_other,2);
					$tot_seasonal=$value_a_seasonal+$value_a_seasonal_other;
					$park_total_seasonal+=$tot_seasonal;
			...
			if($fld_name=="budget_dollars_a_estimate_temp")
				{
				if ($fields['estimate']=="temp") {
					$value_a_temp=$date_array[$number]['budget_weeks_a']*$date_array[$number]['budget_hrs_a']*$date_array[$number]['avg_rate_new'];
					$value_a_temp_other=$date_array[$number]['budget_weeks_a_other']*$date_array[$number]['budget_hrs_a_other']*$date_array[$number]['avg_rate_new'];
					$value_a_temp_format=number_format($value_a_temp,2);
					$value_a_temp_other_format=number_format($value_a_temp_other,2);
					$tot_temp=$value_a_temp+$value_a_temp_other;
					$park_total_temp+=$tot_temp;
					$value=$value_a_temp_format."<br />".$value_a_temp_other_format."<br />
					Total: ".number_format($tot_temp,2);
					} else {
					$value_a_temp=0;
					$value_a_temp_other=0;
					$value_a_temp_format=number_format($value_a_temp,2);
					$value_a_temp_other_format=number_format($value_a_temp_other,2);
					$tot_temp=$value_a_temp+$value_a_temp_other;
					$park_total_temp+=$tot_temp;
...
// 			$totAmount=number_format(($totAmount_a+$totAmount_a_other),2);
...
// 		$taa=number_format($totAmount_a,2);
		$taa=number_format($park_total,2);
		$taa_temp=number_format($park_total_temp,2);
		$taa_seasonal=number_format($park_total_seasonal,2);
		// $tab=number_format($totAmount_b,2);
// 		$tac=number_format($totAmount_c,2);
		echo "<td colspan='15' align='right'>$taa requested<br>";
		echo "$taa_temp (temp estimate)<br>";
		echo "$taa_seasonal (seasonal estimate)</td>";
		// echo "<td colspan='3'>
// 		$taa + $tab + $tac= 
// 		$$totAmount
// 		</td>";
		...
		mysqli_select_db($connection,$database);
			...
			//echo "$sql<br /><br />";
				...
				if(!empty($email))
					...
					$to=rtrim($to,",");
					$body="body=We are requesting approval of $num0 seasonal positions at a cost of $$totAmount.%0A%0AIf you are already logged into the HR database, clicking this link will take you to the listing of requested positions:%0Ahttp://www.dpr.ncparks.gov/hr/bSeasonal/park_seasonals_next.php?center_code=$center_code";
	...
	{
	echo "<font color='red'>No positions have been selected for this park. </font><font color='green'>To select the positions to be filled, click Check All and then click Submit at bottom of page.</font>";
	}
