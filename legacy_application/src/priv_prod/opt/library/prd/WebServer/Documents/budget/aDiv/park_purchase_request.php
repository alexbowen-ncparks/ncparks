...
extract($_REQUEST);
//$purchase_description_Edit=urldecode($purchase_description_Edit);
//$purchase_description_Edit=stripslashes($purchase_description_Edit);

//echo "justification_Edit=$justification_Edit<br />";
//echo "<pre>"; print_r($_SESSION); echo "</pre>"; // exit;
$park4vehicles=$_SESSION['budget']['select'];
$level=$_SESSION['budget']['level'];
$beacnum=$_SESSION['budget']['beacon_num'];
if($beacnum=='60032852'){$level='2';} //william davis (CLNE level 1)
...
foreach($_REQUEST as $k => $v)
	{
	if($v and $k!="PHPSESSID" and $k!="del")
		{@$varQuery.=$k."=".$v."&";}
	}
$passQuery=$varQuery;
   $varQuery.="rep=excel";    
...
//Fiscal Year is primarily based on $f_year VALUE coming from /budget/~f_year.php  which doesn't re-set Fiscal Year until August.  7/9/19
//$f_year manually set to 1920 (below)  7/9/19
//$f_year='1920';


// query to bring back correct fiscal year

$query20a="select f_year
...
$where1="where 1 and dist='$distCode' and section='operations' and new_fund='1680'";

if($rep==""){
$sql="SELECT section,parkcode,new_center as varCenter from center $where1 order by section,parkcode,center";
...
if(empty($report_date)){$where = "where 1"; $report_date="";}
...
$headerArray=array("user_id","system_entry_date","f_year","purchaser","location","pay_center","purchase_description","unit_quantity","unit_cost","requested_amount","purchase_type","purchase_date","justification");
...
// MOJE=1680547   NERI=1680549
if($center=="1680547" OR $center=="1680549"){$edit=1;}
}
...
for($j=0;$j<count($headerArray);$j++){
	...
	if(in_array($headerArray[$j],$pullDownFlds)){
	$addToHeader="";
	if($headerArray[$j]=="purchase_type"){
		$addToHeader="<font color='red'> required</font>";}
		
		
	
echo "<tr><td>$headerArray[$j]$addToHeader</td><td align='left'>";
if($headerArray[$j]=="category"){
...
//if($headerArray[$j]=="purchase_type"){$menuArray=array("","service_contracts"=>"service_contracts","purchase4resale"=>"purchase4resale","mission_critical"=>"mission_critical","emergency"=>"emergency");}
if($headerArray[$j]=="purchase_type"){$menuArray=array("","purchase4resale"=>"purchase4resale","mission_critical"=>"mission_critical","emergency"=>"emergency");}
...
$sql="SELECT section,parkcode,new_center as varCenter from center where (fund='1280'  OR fund='2803') order by section,parkcode,center";
//echo "Line509: sql=$sql";
$result = mysqli_query($connection, $sql) or die ("Couldn't execute query 1. $sql");
$menuArray[]="";
while ($row=mysqli_fetch_array($result)){
...
if($headerArray[$j]=="purchaser"||$headerArray[$j]=="pay_center"||$headerArray[$j]=="location"){
		...
		if($headerArray[$j]=="pay_center"){$var=$_SESSION['budget']['centerSess_new'];}
...
unset($menuArray);}// end Pull-downs
			...
			if($headerArray[$j]=="purchase_description")
			{
			if($level<2)
				{				
				$textlink="<br /><a href='vehicle_info.php?park=$park4vehicles' target='_blank'>View Vehicles</a>";
				}
				$purchase_description_Edit=urldecode($purchase_description_Edit);
                $purchase_description_Edit=stripslashes($purchase_description_Edit);
				
				
				echo "<tr><td>$headerArray[$j]</td><td align='left'><textarea name='$headerArray[$j]' cols='50' rows='4' $ro>$purchase_description_Edit</textarea>$textlink</td></tr>";
				}
			
             if($headerArray[$j]=="justification")
			{
			
				$justification_Edit=urldecode($justification_Edit);
                $justification_Edit=stripslashes($justification_Edit);
				
				
				echo "<tr><td>$headerArray[$j]</td><td align='left'><textarea name='$headerArray[$j]' cols='50' rows='4' $ro>$justification_Edit</textarea></td></tr>";
				}


			
			if($headerArray[$j]!="purchase_description" and $headerArray[$j]!="justification")
			{
...
$footer="<tr><td colspan='7'> Email DPR Budget Office with any problems encountered regarding financial or accounting issues.<br /> Email DPR <a href='mailto:database.support@ncparks.gov'>database.support@ncparks.gov</a> regarding any issues with the web database application.</td></tr>";
